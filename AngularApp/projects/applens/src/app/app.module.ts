...
export class ValidResourceResolver implements Resolve<void> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<any> {
    return this._http.get<any>('assets/enabledResourceTypes.json').pipe(
      map((response) => {
        let routePath = route.pathFromRoot
          .map((v) => v.url.map((segment) => segment.toString()).join('/'))
          .join('/');
        if (routePath.toLowerCase().includes('/stamps/')) {
          let enabledResourceTypes = <ResourceServiceInputs[]>(
            response.enabledResourceTypes
          );
          let matchingResourceInputs = enabledResourceTypes.find(
            (t) => t.resourceType.toLowerCase() === 'stamps'
          );
      ...
      children: [
        {
          ...
          children: [
            ...
            },
            {
              ...
              loadChildren: () =>
                import('./modules/containerapp/containerapp.module').then(
                  (m) => m.ContainerAppModule
                )
            },
            {
              path: 'staticwebapps/:staticwebapp',
              loadChildren: () =>
                import('./modules/staticwebapp/staticwebapp.module').then(
                  (m) => m.StaticWebAppModule
                )
            },
            {
              path: 'stampfinder/:stampName',
              loadChildren: () =>
                import('./modules/stamp/stamp.module').then(
                  (m) => m.StampModule
                )
            },
            {
              path: 'icm',
              loadChildren: () =>
                import('./modules/incidentassist/incidentassist.module').then(
                  (m) => m.IncidentAssistModule
                )
            },
            {
              path: 'surveys/:caseId',
            ...
            },
            {
              ...
              loadChildren: () =>
                import('./modules/ase/ase.module').then((m) => m.AseModule)
            },
            {
              path: 'subscriptions/:subscriptionId/resourceGroups/:resourceGroup/:resourceTypeName/:resourceName',
              redirectTo:
                'subscriptions/:subscriptionId/resourceGroups/:resourceGroup/providers/Microsoft.Web/:resourceTypeName/:resourceName'
            },
            {
            ...
            //For '/resourceGroup' case insensitive
            {
              path: 'subscriptions/:subscriptionId/resourcegroups/:resourceGroup/providers/:provider/:resourceTypeName/:resourceName',
              redirectTo:
                'subscriptions/:subscriptionId/resourceGroups/:resourceGroup/providers/:provider/:resourceTypeName/:resourceName'
            },
            {
            ...
            },
            {
              path: 'caseCleansing',
              loadChildren: () =>
                import('./modules/casecleansing/casecleansing.module').then(
                  (m) => m.CasecleansingModule
                )
            }
          ]
        }
...
@NgModule({
  ...
  providers: [
    ...
    ValidResourceResolver,
    {
      ...
      useExisting: ApplensAppinsightsTelemetryService
    },
    AdalService,
    {
