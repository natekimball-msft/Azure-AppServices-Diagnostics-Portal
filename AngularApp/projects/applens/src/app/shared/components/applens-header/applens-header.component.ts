...
export class ApplensHeaderComponent implements OnInit {
  ...
  choiceGroupOptions: IChoiceGroupOption[] = [
    {
      ...
      text: 'Smart Grouping',
      onClick: () => {
        this.smartViewChecked = true;
        this.selectedKey = 'smarter';
      }
    },
    {
      key: 'waterfall',
      text: 'Waterfall',
      onClick: () => {
        this.smartViewChecked = false;
        this.selectedKey = 'waterfall';
      }
    }
  ];
  ...
  constructor(
    ...
    private _themeService: ApplensThemeService,
    private _detectorControlService: DetectorControlService,
    @Optional() public _searchService?: SearchService,
    @Optional() private _applensGlobal?: ApplensGlobal
  ) {}

  ngOnInit() {
    const alias = this._adalService.userInfo.profile
    ...
    if (this._adalService.userInfo.profile) {
      ...
      this.userNameInitial = `${givenName
        .charAt(0)
        .toLocaleUpperCase()}${familyName.charAt(0).toLocaleUpperCase()}`;
    }
    ...
    this._userSettingService.getUserSetting().subscribe((userSettings) => {
      this.expandCheckCard = userSettings
        ? userSettings.expandAnalysisCheckCard
        : false;
      this.darkThemeChecked =
        userSettings && userSettings.theme.toLowerCase() == 'dark'
          ? true
          : false;
      this.smartViewChecked =
        userSettings && userSettings.viewMode.toLowerCase() == 'smarter'
          ? true
          : false;
      this.selectedKey =
        userSettings && userSettings.viewMode.toLowerCase() == 'smarter'
  ...
  toggleExpandCheckCard(event: { checked: boolean }) {
    ...
    this.userSettingChanged =
      this.expandAnalysisChanged || this.themeChanged || this.viewModeChanged;
  ...
  private updateUserSettingsFromPanel() {
    ...
    this._userSettingService
      .updateUserPanelSetting(updatedSettings)
      .subscribe();
