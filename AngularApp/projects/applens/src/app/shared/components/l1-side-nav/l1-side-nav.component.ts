...
export class L1SideNavComponent implements OnInit {
    ...
    subItems: [
      {
        ...
        click: () => {
          this._applensGlobal.openL2SideNavSubject.next(
            L2SideNavType.Develop_Detectors
          );
        }
      },
      {
  ...
  dialogSubText: string =
    'You’ll lose access to current resource’s data. Are you sure to select a new resource?';
  ...
  ngOnInit() {
    ...
    this._diagnosticApiService
      ...
      .subscribe((isDetectorDevelopmentEnabled) => {
  ...
  getCurrentHighLightItem(): L1SideNavItemType {
    ...
    if (
      childRoute &&
      (childRoute.snapshot.params['analysisId'] ||
        childRoute.snapshot.params['detector'])
    ) {
  ...
  private navigateTo(path: string) {
    if (this._activatedRoute.parent) {
      ...
      const queryParams = UriUtilities.removeChildDetectorStartAndEndTime(
