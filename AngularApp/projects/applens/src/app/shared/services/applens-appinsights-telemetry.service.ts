...
export class ApplensAppinsightsTelemetryService implements ITelemetryProvider {
  ...
  public initialize() {
    if (!this.appInsights) {
      const appInsightsRequest = this._backendApi
      ...
      const envConfigRequest = this._backendApi
        .get<string>('api/appsettings/APPLENS_ENVIRONMENT')
        .pipe(
          map((value: string) => {
      ...
      const hostnameConfigRequest = this._backendApi
        .get<string>(`api/appsettings/APPLENS_HOST`)
        .pipe(
          map((value: string) => {
