...
export class StaticWebAppService extends ResourceService {
  ...
  public startInitializationObservable() {
    this._initialized = this._observerApiService
      .getStaticWebApp(this._armResource.resourceName)
      .pipe(
        map((observerResponse: ObserverStaticWebAppResponse) => {
          this._observerResource = this._staticWebAppObject =
            this.getStaticWebAppFromObserverResponse(observerResponse);
  ...
  public getCurrentResource(): Observable<any> {
    return this._currentResource.pipe(
      map((currentResource) => {
  ...
  private getStaticWebAppFromObserverResponse(
    observerResponse: ObserverStaticWebAppResponse
  ): ObserverStaticWebAppInfo {
    const response = observerResponse.details.find(
      (staticWebApp) =>
        staticWebApp.SubscriptionName.toLowerCase() ===
          this._armResource.subscriptionId.toLowerCase() &&
        staticWebApp.ResourceGroupName.toLowerCase() ===
