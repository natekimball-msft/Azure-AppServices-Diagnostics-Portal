...
export class SiteService extends ResourceService {
  private _currentResource: BehaviorSubject<ObserverSiteInfo> =
  ...
  public startInitializationObservable() {
    this._initialized = this._observerApiService
      .getSite(this._armResource.resourceName)
      .pipe(
        ...
        flatMap((site) => {
          return this._observerApiService
            .getSiteSku(site.InternalStampName, site.SiteName)
            .pipe(
              map((siteSku: any) => {
  ...
  public restartSiteFromUri(
    resourceUri: string
  ): Observable<HttpResponse<any>> {
    return null;
  }

  public updateSettingsFromUri(
    resourceUri: string,
    body: any
  ): Observable<any> {
    return null;
  ...
  private getSiteFromObserverResponse(
    observerResponse: ObserverSiteResponse
  ): ObserverSiteInfo {
    return observerResponse.details.find(
      (site) =>
        site.Subscription.toLowerCase() ===
          this._armResource.subscriptionId.toLowerCase() &&
        site.ResourceGroupName.toLowerCase() ===
  ...
  public updatePesIdAndImgSrc() {
    if (
      this._siteObject.Kind &&
      this._siteObject.Kind.toString().toLowerCase().indexOf('workflowapp') !==
        -1
    ) {
      this.pesId = '17378';
      ...
      this._siteObject.Kind &&
      this._siteObject.Kind.toString().toLowerCase().indexOf('functionapp') !==
        -1
    ) {
      this.pesId = '16072';
      ...
      this._siteObject.IsLinux != undefined &&
      this._siteObject.IsLinux
    ) {
      this.pesId = '16170';
  ...
  private getSiteASPAndSKu(siteSku: ObserverSiteSku): string {
    const priceTire = SkuUtilities.getPriceTireBySkuAndSize(
      siteSku.sku.toString(),
      siteSku.current_worker_size
    );
