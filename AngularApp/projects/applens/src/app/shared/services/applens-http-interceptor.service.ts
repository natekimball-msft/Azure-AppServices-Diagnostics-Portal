  ...
  raiseAlert(event) {
    ...
    let alertInfo: AlertInfo = {
      ...
      confirmationOptions: [
        ...
        { label: 'No, take me back', value: 'no' }
      ],
  ...
  intercept(
    ...
    return next.handle(req).pipe(
      ...
      catchError((error: HttpErrorResponse) => {
        ...
        if (
          error.status === 403 &&
          error.url.includes('api/invoke') &&
          errorObj.Status == UserAccessStatus.ResourceNotRelatedToCase
        ) {
          ...
          if (
            errorObj.DetailText &&
            errorObj.DetailText.includes('the token is expired')
          ) {
            this._adalService.clearCache();
            return this._adalService
              .acquireToken(this._adalService.config.clientId)
              .pipe(
