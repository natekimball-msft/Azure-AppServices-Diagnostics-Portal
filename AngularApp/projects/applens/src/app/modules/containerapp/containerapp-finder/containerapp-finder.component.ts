...
export class ContainerAppFinderComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._observerService.getContainerApp(this.siteName).subscribe(
      (observerContainerAppResponse) => {
        if (
          observerContainerAppResponse.details.toString() ==
          'Unable to fetch data from Observer API : GetContainerApp'
        ) {
      ...
      (error: Response) => {
        this.error =
          error.status == 404
            ? `Container App with the name ${this.siteName} was not found`
            : `There was an error trying to find container app ${this.siteName}`;
        this.loading = false;
      }
    );
  }
  ...
  navigateToSite(matchingSite: ObserverContainerAppInfo) {
    let resourceArray: string[] = [
      ...
      'providers',
      matchingSite.IsInAppNamespace ? 'Microsoft.App' : 'Microsoft.Web',
      'containerApps',
      matchingSite.ContainerAppName
    ];
