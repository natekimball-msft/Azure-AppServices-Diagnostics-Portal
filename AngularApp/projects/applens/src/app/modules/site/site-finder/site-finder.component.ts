...
export class SiteFinderComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._observerService.getSite(this.site).subscribe(
      (observerSiteResponse) => {
        if (
          observerSiteResponse.details.toString() ==
          'Unable to fetch data from Observer API : GetAdminSite'
        ) {
      ...
      (error: Response) => {
        this.error =
          error.status == 404
            ? `App ${this.site} was not found`
            : `There was an error trying to find app ${this.site}`;
        this.loading = false;
      }
    );
  }
