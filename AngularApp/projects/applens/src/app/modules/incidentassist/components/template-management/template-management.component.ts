...
export class TemplateManagementComponent implements OnInit {
  ...
  retryMessage: string =
    'Please try again. If the error persists, please contact AppLens team.';
  userId: string = null;
  loaderMessage = null;
  ...
  narrowTextFieldStyles: Partial<ITextFieldStyles> = {
  ...
  constructor(
    private _incidentAssistanceService: IncidentAssistanceService,
  ...
  ngOnInit() {
    ...
    this._telemetryService.logPageView(
      TelemetryEventNames.ICMTemplateManagementPage,
      { userId: this.userId }
    );
    this.pageLoading = true;
    this.displayLoader = true;
    this._incidentAssistanceService.isIncidentAssistanceEnabled().subscribe(
      (res) => {
        ...
        if (this.isEnabled) {
          ...
          this._telemetryService.logEvent(
            TelemetryEventNames.ICMTemplateManagementPage,
            { IsEnabled: this.isEnabled.toString() }
          );
          this.alternateContent =
            'Incident assistance feature is currently disabled in AppLens';
  ...
  getOnboardedTeams() {
    this.pageLoading = true;
    this.displayLoader = true;
    this._incidentAssistanceService.getOnboardedTeams().subscribe(
      (res) => {
        this.displayLoader = false;
        ...
        if (onboardedTeams.length > 0) {
            ...
            if (team.type.length > 0) {
              team.type.forEach((type) => {
                this.fabDropdownOptions.push({
                  key: team.teamId + '++' + type.toString(),
                  text: team.teamName + ' (' + type.toString() + ')'
                });
                this.allOnboardedTeams.push({
                  teamId: team.teamId,
              ...
              this.fabDropdownOptions.push({
                key: team.teamId + '++' + team.type[0].toString(),
                text: team.teamName + ' (' + team.type[0].toString() + ')'
              });
              this.allOnboardedTeams.push({
                teamId: team.teamId,
                teamName: team.teamName,
                incidentType: team.type[0]
              });
            }
          ...
          this.pageLoading = false;
          this.alternateContent =
            "You do not have access to any AppLens ICM Automation team templates. Please contact your team's AppLens ICM Assistance admin.";
        }
      },
      (err) => {
        this.displayLoader = false;
  ...
  selectDropdownKey(e: { option: IDropdownOption; index: number }) {
    ...
    const selectedTeam = this.allOnboardedTeams.find(
      (team) => team.teamId === teamId && team.incidentType === incidentType
    );
  ...
  getTeamTemplate() {
    this.resetGlobals();
    this.displayLoader = true;
    this._incidentAssistanceService
      .getTeamTemplate(
        this.selectedTeam.teamId,
        this.selectedTeam.incidentType.toString()
      )
      .subscribe(
        (res) => {
          this.displayLoader = false;
          ...
          this._telemetryService.logEvent(
            TelemetryEventNames.ICMTeamTemplateLoaded,
            {
              TeamId: this.selectedTeam.teamId,
              IncidentType: this.selectedTeam.incidentType.toString(),
              userId: this.userId
            }
          );
        },
        (err) => {
          ...
          this.templateLoadError =
            err.error.length < 100
              ? err.error
              : `Failed to load the template. ${err.error}`;
        }
      );
  }
  ...
  onTestIncidentClick() {
    ...
    this._incidentAssistanceService.testTemplateWithIncident(body).subscribe(
      (res) => {
        this.testingIncidentLoader = false;
      ...
      (err) => {
        this.testingIncidentLoader = false;
        this.testIncidentError =
          err && err.error && err.error.length < 200
            ? err.error
            : 'An error occurred';
        this.updateButtonDisabled = false;
      }
      ...
      this._incidentAssistanceService
        ...
        .subscribe(
          (res) => {
            ...
            this._telemetryService.logEvent(
              TelemetryEventNames.ICMTeamTemplateUpdate,
              {
                TeamId: this.selectedTeam.teamId,
                IncidentType: this.selectedTeam.incidentType.toString(),
                Status: 'success',
    ...
    this._incidentAssistanceService
      ...
      .pipe(
        ...
        catchError((err) => {
          ...
          this.teamTemplateAuthors = null;
          this.templateAuthorsErrorMessage =
            'Error retrieving template authors.';
          this.templateAuthorsSuccessMessage = '';
          return of(null);
        })
      )
      .subscribe((res) => {
        setTimeout(() => {
          this.ngxSmartModalService
            .getModal('icmTemplatePermissionsModal')
            .open();
        }, 100);
      });
  }
  ...
  updatePermissions() {
    if (
      ...
      this.templateAuthors !== this.teamTemplateAuthors
    ) {
      this._incidentAssistanceService
        .updateTeamTemplateAuthors(
          this.selectedTeam.teamId,
          this.selectedTeam.incidentType.toString(),
          { authors: this.templateAuthors }
        )
        .subscribe(
          (data: any) => {
            ...
            this.templateAuthorsErrorMessage = '';
            this.templateAuthorsSuccessMessage =
              'Template authoring permissions updated successfully.';
          },
          (err) => {
            this.templateAuthorsSuccessMessage = '';
            this.templateAuthorsErrorMessage =
              'Error updating template authoring permissions. Please try again.';
            return of(null);
          }
        );
    }
    ...
    this._incidentAssistanceService
      ...
      .subscribe(
        ...
        (err) => {
          this.displayLoader = false;
          this.showAvailableValidations = true;
          this.availableValidationsError =
            'Error retrieving available validations. Please try again.';
        }
      );
  }
