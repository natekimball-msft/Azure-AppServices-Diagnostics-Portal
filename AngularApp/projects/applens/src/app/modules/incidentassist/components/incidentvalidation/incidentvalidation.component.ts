...
export class IncidentValidationComponent implements OnInit {
    ...
    routeMessageType: string = "none";
    userId: string = null;
    solutions: any = null;
    showSolutions: boolean = false;
    panelType = Number(String(PanelType.custom));
    loaderMessage = null;
    validateRouteButtonDisabled: boolean = false;
    incidentRouteValidationStatus: boolean = false;
    ...
    refreshButtonStatus() {
        ...
        this.validateRouteButtonDisabled = this.incidentRouteValidationStatus;
    }
        ...
        if (!this.updateButtonDisabled) {
            ...
            this._incidentAssistanceService.updateIncident(body).subscribe(res => {
                ...
                if (result.updationStatus) {
                    ...
                    this.incidentRouteValidationStatus = false;
  ...
  onValidateRouteClick() {
      if (!this.validateRouteButtonDisabled) {
          this.resetGlobals();
          this.displayLoader = true;
          this.loaderMessage = "Finding incident route";
          this._incidentAssistanceService.validateIncidentRoute(this.incidentId).subscribe(res => {
              ...
              if (result && result.length > 0) {
                  ...
                  this.routeMessage = `Incident Route validation has failed: ${result}`;
