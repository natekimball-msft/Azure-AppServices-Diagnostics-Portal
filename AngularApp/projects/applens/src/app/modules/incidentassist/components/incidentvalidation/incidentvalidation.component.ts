...
export class IncidentValidationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._telemetryService.logPageView(
      TelemetryEventNames.IncidentAssistancePage,
      { IncidentId: this.incidentId, userId: this.userId }
    );
    if (this.incidentId && this.incidentId.length > 0) {
      ...
      this.alternateContent =
        'A valid incident id was not provided in the url path.';
    }
  }
  ...
  refreshButtonStatus() {
    this.validationButtonDisabled =
      this.incidentValidationStatus ||
      this.incidentInfo.validationResults.every((x) => x.value == x.oldValue);
    this.updateButtonDisabled =
      !this.incidentValidationStatus || this.updatedSuccessfully;
  }
  ...
  setIncidentInfo(payload) {
    if (
      ...
      payload.validationResults.length > 0
    ) {
  ...
  getIncidentData() {
    this.displayLoader = true;
    this._incidentAssistanceService.getIncident(this.incidentId).subscribe(
      (res) => {
        this.displayLoader = false;
        var result = JSON.parse(res.body);
        this.pageLoading = false;
        this.setIncidentInfo(result);
        this.incidentValidationStatus = result.validationResults.every(
          (x) => x.validationStatus
        );
        this._telemetryService.logEvent(
          TelemetryEventNames.IncidentAssistanceLoaded,
          {
            IncidentId: this.incidentInfo.incidentId,
            ValidationStatus: this.incidentValidationStatus.toString(),
            userId: this.userId
          }
        );
        if (!this.incidentValidationStatus) {
          this.footerMessage =
            "Some validations have failed. Please correct the info and click on 'Check Validation' button.";
  ...
  onSubmit() {
    if (!this.validationButtonDisabled) {
      var body = {
        IncidentId: this.incidentInfo.incidentId,
        ValidationResults: this.incidentInfo.validationResults.map((x) => {
      ...
      setTimeout(() => {
        if (this.loaderMessage == 'Checking validations') {
          this.loaderMessage = 'Running Diagnostics';
        }
      }, 3000);
      this._incidentAssistanceService.validateIncident(body).subscribe(
        (res) => {
          ...
          if (result.validationStatus) {
            ...
            this.incidentInfo.validationResults.forEach((x) => {
              x.validationStatus = true;
              x.oldValue = x.value;
            });
          ...
          this._telemetryService.logEvent(
            TelemetryEventNames.IncidentValidationCheck,
            {
              IncidentId: this.incidentInfo.incidentId,
              ValidationStatus: this.incidentValidationStatus.toString(),
              Status: 'success',
  ...
  onUpdateClick() {
    if (!this.updateButtonDisabled) {
      ...
      this.resetGlobals();
      this.displayLoader = true;
      this._incidentAssistanceService.updateIncident(body).subscribe(
        (res) => {
          this.displayLoader = false;
          var result = JSON.parse(res.body);
          if (result.updationStatus) {
            ...
            this.footerMessage =
              'Incident has been updated successfully and will be investigated by the team.';
            this.footerMessageType = 'success';
          } else {
          ...
          this._telemetryService.logEvent(
            ...
            {
              IncidentId: this.incidentInfo.incidentId,
              UpdationStatus: this.updatedSuccessfully.toString(),
              Status: 'success',
              userId: this.userId
            }
          );
