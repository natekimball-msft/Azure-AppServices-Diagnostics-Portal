...
export class InitResolver implements Resolve<Observable<ResourceInfo>> {
  ...
  resolve(
    ...
    return this._resourceService.waitForInitialization().pipe(
      map((resourceInfo) => {
        const queryParams = this._userSettingService.excludeQueryParams(
          route.queryParams
        );
        recentResource = {
      ...
      mergeMap((resourceInfo) => {
        return this._userSettingService.getUserSetting().pipe(
          take(1),
      ...
      mergeMap((resourceInfo) => {
        return this._userSettingService.updateLandingInfo(recentResource).pipe(
          catchError((_) => of(null)),
          map((_) => resourceInfo)
        );
      })
...
export const DashboardModuleRoutes: ModuleWithProviders<DashboardModule> =
...
@NgModule({
  ...
  declarations: [
    ...
    SearchTermAdditionComponent,
    SearchMenuPipe,
    TabDataComponent,
    TabDevelopComponent,
    TabCommonComponent,
    TabDataSourcesComponent,
    TabMonitoringComponent,
    TabMonitoringDevelopComponent,
    TabAnalyticsDevelopComponent,
    TabAnalyticsDashboardComponent,
    GistComponent,
    TabGistCommonComponent,
    TabGistDevelopComponent,
    TabChangelistComponent,
    GistChangelistComponent,
    TabAnalysisComponent,
    CategoryPageComponent,
    SupportTopicPageComponent,
    SelfHelpContentComponent,
    UserDetectorsComponent,
    FormatResourceNamePipe,
    Sort,
    ...
    L2SideNavComponent,
    UserActivePullrequestsComponent,
    FavoriteDetectorsComponent,
    ApplensDocsComponent,
    ApplensDocSectionComponent,
