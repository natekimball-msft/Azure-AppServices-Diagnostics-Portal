...
import { TelemetryService, ChatUIContextService, TelemetryEventNames, ChatMessage, OpenAIChatComponent } from 'diagnostic-data';
...
export class CommunicationToolkitComponent implements OnInit {
  ...
  copyChatGPTClicked(){
    ...
    this._clipboard.copyAsHtml(this.OpenAICHATComponentRef._chatContextService.messageStore[id][length-1].displayMessage);

  }

  clearChatClicked(){
    var id = this.OpenAICHATComponentRef.chatIdentifier;
    this.OpenAICHATComponentRef._chatContextService.messageStore[id] = [];
  }
