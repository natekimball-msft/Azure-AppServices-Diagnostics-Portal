...
export class UserDetectorsComponent implements OnInit {
    ...
    private getSupportTopicName(supportTopicIds: SupportTopic[]): string {
        ...
        supportTopicIds.forEach(t => {
            ...
            if(topic) {
                ...
                const l3NameSet = l2ToL3Map.has(l2Name) ? l2ToL3Map.get(l2Name) : new Set<string>();
    ...
    private convertSupportTopicNameSetToString(l2ToL3Map : Map<string, Set<string>>): string {
        ...
        for (const l2Name of l2SupportTopicNames) {
            ...
            const l3NamesTemplate = `<ul>${l3Names.map(l3Name => `<li>${l3Name}</li>`).join("")}</ul>`;
