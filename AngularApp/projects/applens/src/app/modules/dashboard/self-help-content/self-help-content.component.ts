...
export class SelfHelpContentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.supportTopicId =
      this._activatedRoute.snapshot.params['supportTopicId'];
    this.selfHelpPath = this._supportTopicService.getSelfHelpPath();

    this._diagnosticService
      .getSelfHelpContent(this.pesId, this.supportTopicId, this.selfHelpPath)
      .subscribe((res: string) => {
        this.selfHelpContent =
          res == undefined || res === ''
            ? 'No self help documentation is found'
            : res;
      });
  }
