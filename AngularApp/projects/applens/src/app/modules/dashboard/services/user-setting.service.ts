...
import {
  ...
  UserPanelSetting,
  UserSetting
} from '../../../shared/models/user-setting';
import { DiagnosticApiService } from '../../../shared/services/diagnostic-api.service';
import { Params } from '@angular/router';

@Injectable()
export class UserSettingService {
  private _userSettingSubject: BehaviorSubject<UserSetting> =
    new BehaviorSubject(null);
  currentTheme: string = 'light';
  currentViewMode: string = 'smarter';
  currentThemeSub: BehaviorSubject<string> = new BehaviorSubject<string>(
    'light'
  );
  currentViewModeSub: BehaviorSubject<string> = new BehaviorSubject<string>(
    'smarter'
  );
  ...
  private get _userId() {
    const alias =
      !!this._adalService.userInfo.profile &&
  ...
  getUserSetting(invalidateCache = false): Observable<UserSetting> {
    ...
    return this._diagnosticApiService
      .getUserSetting(this._userId, invalidateCache)
      .pipe(
        map((userSetting) => {
  ...
  getExpandAnalysisCheckCard() {
    return this.getUserSetting().pipe(
      map((userSetting) => userSetting.expandAnalysisCheckCard)
    );
  }
  ...
  isWaterfallViewMode() {
    return this.getUserSetting().pipe(
      map((userSetting) => {
        return userSetting.viewMode == 'waterfall';
      })
    );
  }
  ...
  excludeQueryParams(qp: Params) {
    ...
    keys.forEach((x) => {
      if (!this.queryParamsToExclude.includes(x)) {
        newParams[x] = qp[x];
      }
    });
    return newParams;
  ...
  private addRecentResource(
    newResource: RecentResource,
    userSetting: UserSetting
  ): RecentResource[] {
    const res = [...this._userSetting.resources];
    const index = userSetting.resources.findIndex(
      (resource) =>
        resource.resourceUri.toLowerCase() ===
        newResource.resourceUri.toLowerCase()
    );
    if (index >= 0) {
      res.splice(index, 1);
  ...
  updateUserPanelSetting(
    ...
    return this._diagnosticApiService
      .updateUserPanelSetting(panelSetting, this._userId)
      .pipe(
        map((userSetting) => {
          this._userSetting = userSetting;
          return userSetting;
  ...
  updateLandingInfo(resource: RecentResource): Observable<UserSetting> {
    const updatedResources = this.addRecentResource(
      resource,
      this._userSetting
    );
    const info: LandingInfo = {
      ...
      defaultServiceType: this.defaultServiceType
        ...
        : this._userSetting.defaultServiceType
    };
    return this._diagnosticApiService
      .updateUserLandingInfo(info, this._userId)
      .pipe(
        map((userSetting) => {
          this._userSetting = userSetting;
          return userSetting;
  ...
  removeFavoriteDetector(detectorId: string): Observable<FavoriteDetectors> {
    return this._diagnosticApiService
      .removeFavoriteDetector(detectorId, this._userId)
      .pipe(
        map((userSetting) => {
          this._userSetting = userSetting;
  ...
  ): Observable<FavoriteDetectors> {
    return this._diagnosticApiService
      .addFavoriteDetector(detectorId, detectorProp, this._userId)
      .pipe(
        map((userSetting) => {
          this._userSetting = userSetting;
          return userSetting.favoriteDetectors;
