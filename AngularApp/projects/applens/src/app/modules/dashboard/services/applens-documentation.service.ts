...
export class ApplensDocumentationService {
  ...
  public getDocsRepoSettings(): Observable<DocumentationRepoSettings> {
    let repoRootObservable = this._diagnosticApi.getAppSetting(
      'SystemInvokers:DocumentationRoot'
    );
    let stagingBranchObservable = this._diagnosticApi.getAppSetting(
      'SystemInvokers:DocumentationStagingBranch'
    );
    let resourceObservable = this._diagnosticApi.getAppSetting(
      'SystemInvokers:ResourceIDString'
    );
    ...
    return forkJoin([
      ...
      map((repoSettings) => {
        ...
        return new DocumentationRepoSettings(
          docsRepoRoot,
          docStagingBranch,
          docsResource,
          isStaging
