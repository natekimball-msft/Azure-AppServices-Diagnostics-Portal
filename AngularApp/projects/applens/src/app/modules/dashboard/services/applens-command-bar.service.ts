...
export class ApplensCommandBarService {
  constructor(
    ...
    private _applensDiagnosticService: ApplensDiagnosticService,
      ...
      flatMap((canAdd) => {
        if (canAdd) {
          return this._userSettingService
            .addFavoriteDetector(detectorId, { type: detectorType })
            .pipe(map((_) => successfulMessage));
        } else {
  ...
  private canAddFavoriteDetector(
    detectors: DetectorMetaData[],
    favoriteDetectors: FavoriteDetectors,
    maxNumber: number
  ) {
    const allFavoriteDetectors = Object.keys(favoriteDetectors);
    const filteredFavoriteDetectors = allFavoriteDetectors.filter(
      (favoriteDetector) =>
        detectors.findIndex(
          (d) => d.id.toLowerCase() === favoriteDetector.toLowerCase()
        ) > -1
    );
  ...
  public removeFavoriteDetector(detectorId: string): Observable<string> {
    ...
    const errorMessage =
      'Some issue happened while unpinning, Please try again later';
