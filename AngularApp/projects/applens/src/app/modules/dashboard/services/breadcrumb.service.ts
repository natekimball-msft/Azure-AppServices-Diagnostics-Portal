...
export class BreadcrumbService {
  ...
  public breadcrumbSubject: BehaviorSubject<BreadcrumbNavigationItem[]> =
    new BehaviorSubject([]);
  ...
  public updateBreadCrumbSubject(breadcrumbItem: BreadcrumbNavigationItem) {
    ...
    if (breadcrumbList.findIndex((b) => b.name === breadcrumbItem.name) > -1)
      return;
  ...
  public navigate(item: BreadcrumbNavigationItem) {
    ...
    if (itemIndex === 0) {
      ...
      routingParams =
        copiedList[itemIndex].queryParams != undefined
          ? copiedList[itemIndex].queryParams
          : routingParams;
    ...
    if (!!routingParams['startTime'] && !!routingParams['endTime']) {
      this._detectorControl.setCustomStartEnd(
        routingParams['startTime'],
        routingParams['endTime'],
        'BreadCrumbControl'
      );
