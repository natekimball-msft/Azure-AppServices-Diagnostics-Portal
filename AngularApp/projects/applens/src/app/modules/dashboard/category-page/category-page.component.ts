...
export class CategoryPageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const supportTopicImage = this._supportTopicService
      .getCategoryImage(this.categoryName)
      .pipe(
        map((iconString) => {
    ...
    const allDetectorsList = this._diagnosticService.getDetectors().pipe(
      map((detectors: DetectorMetaData[]) => {
        this.allDetectors = detectors;
        // Get all the detectors with support topics
        var detectorsWithSupportTopics = detectors.filter(
          (detector) =>
            detector.category &&
            detector.category.toLowerCase() ===
              this.categoryName.toLowerCase() &&
            detector.supportTopicList &&
            detector.supportTopicList.length > 0
        );
        ...
        detectorsWithSupportTopics.forEach((detector) => {
          ...
          detector.supportTopicList.forEach((supportTopic) => {
            this.supportTopicIdMapping.push({
        ...
        authors.forEach((author) => {
          if (
            author &&
            author.length > 0 &&
            !this.authorsList.find(
              (existingAuthor) => existingAuthor === author
            )
          ) {
    ...
    const publicDetectors = this._diagnosticService
      .getDetectors('', false)
      .pipe(
        map((publicDetectors: DetectorMetaData[]) => {
          this.publicDetectorsList = publicDetectors.filter(
            (detector) =>
              detector.category &&
              detector.category.toLowerCase() ===
                this.categoryName.toLowerCase()
    ...
    forkJoin(supportTopicImage, allDetectorsList, publicDetectors).subscribe(
      (res) => {
        ...
        this.publicDetectorsList.forEach((detector) => {
          ...
          var publicDetectorWithPII = this.allDetectors.find(
            (detectorWithPII) => detectorWithPII.id === detectorId
          );
          if (
            publicDetectorWithPII &&
            !this.filterdDetectors.find((d) => d.id === detectorId)
          ) {
          ...
          this._diagnosticService.getUsers(body).subscribe((userImages) => {
            ...
            this.filterdDetectors.forEach((detector) => {
              this._supportTopicService
                .getCategoryImage(detector.name)
                .subscribe((iconString) => {
                  let onClick = () => {
                    this._telemetryService.logEvent(
                      TelemetryEventNames.DetectorCardClicked,
                  ...
                  if (detector.author != undefined) {
                    ...
                    const separators = [' ', ',', ';', ':'];
                    let detectorAuthors = authors
                      .split(new RegExp(separators.join('|'), 'g'))
                      .filter((author) => author != '');
                    detectorAuthors.forEach((author) => {
                      if (
                        !this.filterdDetectorAuthors.find(
                          (existingAuthor) => existingAuthor === author
                        )
                      ) {
                      ...
                      detectorUsersImages[author] =
                        this.userImages.hasOwnProperty(author)
                          ? this.userImages[author]
                          : undefined;
                    });
                  }
                  ...
                  let detectorItem = new DetectorItem(
                    detector.name,
            ...
            this._telemetryService.logPageView(
              TelemetryEventNames.CategoryPageLoaded,
              {
                numDetectors: this.filterdDetectors.length.toString(),
