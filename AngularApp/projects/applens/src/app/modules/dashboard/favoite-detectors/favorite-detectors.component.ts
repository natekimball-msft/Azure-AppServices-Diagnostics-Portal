...
export class FavoriteDetectorsComponent implements OnInit {
  ...
  ngOnInit() {
    this._applensDiagnosticService.getDetectors().subscribe((detectors) => {
      ...
      this._userSettingService.getUserSetting().subscribe((userSetting) => {
        ...
        this.favoriteDetectorsForDisplay =
          this.filterDetectors(favoriteDetectors);
      });
    });
  }
  ...
  public navigate(detector: DetectorMetaData) {
    this._telemetryService.logEvent(
      ...
      { detectorId: detector.id, location: 'OverViewPage' }
    );
    if (detector.type === DetectorType.Detector) {
      this._router.navigate([`./detectors/${detector.id}`], {
        relativeTo: this._activatedRoute
      });
    } else {
      this._router.navigate([`./analysis/${detector.id}`], {
        relativeTo: this._activatedRoute
      });
    }
  }
  ...
  public removeDetector(e: Event, detector: DetectorMetaData) {
    ...
    this._userSettingService.removeFavoriteDetector(detector.id).subscribe(
      (_) => {
  ...
  private filterDetectors(
    ...
    const detectors = this.allDetectors.filter((detector) => {
      return (
        ...
        favoriteDetectors[detector.id].type === detector.type
      );
    });
