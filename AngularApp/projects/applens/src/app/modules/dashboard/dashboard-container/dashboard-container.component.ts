...
import {
  AppLensCloudRegion,
  AppLensCloudRegionUtility
} from '../../../shared/utilities/applens-cloud-region-utility';
...
export class DashboardContainerComponent implements OnInit {
  ...
  constructor(
    ...
    private _applensDiagnosticApiService: ApplensDiagnosticService,
    ...
    let caseNumber = this._activatedRoute.snapshot.queryParams['caseNumber']
      ...
      ? this._activatedRoute.snapshot.queryParams['srId']
      : null;
    this.detector = this._activatedRoute.snapshot.queryParams['detector']
      ? this._activatedRoute.snapshot.queryParams['detector']
      : null;
    if (caseNumber && AppLensCloudRegionUtility.getASCCloudSpecificBaseUri()) {
      this.ascResourceExplorerLink =
        AppLensCloudRegionUtility.getASCCloudSpecificBaseUri() +
        '/resourceexplorerv2?srId=' +
        caseNumber;
    }
  }
  ...
  ngOnInit() {
    ...
    this.showMetrics = !(
      this._resourceService.overviewPageMetricsId == undefined ||
      this._resourceService.overviewPageMetricsId == ''
    );
    ...
    this.resourceReady.subscribe((resource) => {
      if (resource) {
        ...
        if (
          ...
          if (resource['IsXenon']) {
            this._resourceService.imgSrc =
              this._resourceService.altIcons['Xenon'];
          }
        } else if (
          serviceInputs.resourceType.toString().toLowerCase() ===
          ...
          this.observerLink =
            'https://wawsobserver.azurewebsites.windows.net/staticwebapps/' +
            this.resource['DefaultHostname'];
        }
    ...
    this._observerService
      .getSiteSku(this.resource.InternalStampName, this.resource.SiteName)
      .subscribe((siteSku) => {
        if (siteSku) {
    ...
    this.displayDownloadReportButton =
      this.checkIsWindowsApp() || this.checkIsLinuxApp();

    // Logging telemetry for Download Report button
    const dRBDEventProperties = {
      ResiliencyScoreButtonDisplayed:
        this.displayDownloadReportButton.toString(),
      Subscription: this.subscriptionId,
      Platform:
        this.siteSku.is_linux != undefined
          ? !this.siteSku.is_linux
            ? 'Windows'
            : 'Linux'
          : '',
      AppType:
        this.siteSku.kind != undefined
          ? this.siteSku.kind.toLowerCase() === 'app'
            ? 'WebApp'
            : this.siteSku.kind.toString()
          : '',
      resourceSku:
        this.siteSku.sku != undefined ? this.siteSku.sku.toString() : '',
      ReportType: 'ResiliencyScore'
    };
    this._telemetryService.logEvent(
      TelemetryEventNames.DownloadReportButtonDisplayed,
      dRBDEventProperties
    );
    ...
    //
    this._http.get<any>('assets/vfs_fonts.json').subscribe((data: any) => {
      this.vfsFonts = data;
    });
  }
  ...
  private checkIsWindowsApp() {
    ...
    return (
      this.siteSku &&
      this.getAppType(this.siteSku.kind.toLowerCase()) === 'WebApp' &&
      !this.siteSku.is_linux &&
      _sku >= 8
    ); //Only for Web App (Windows) in Standard or higher SKU
  }
  ...
  private checkIsLinuxApp() {
    ...
    return (
      this.siteSku &&
      this.getAppType(this.siteSku.kind.toLowerCase()) === 'LinuxApp' &&
      this.siteSku.is_linux &&
      _sku >= 8
    ); //Only for Web App (Linux) in Standard or higher SKU
  }
  ...
  updateDownloadReportId() {
    ...
    if (btns && btns.length > 0) {
      ...
      if (
        ...
        downloadButtonIndex < btns.length &&
        btns[downloadButtonIndex]
      ) {
  ...
  updateAscLink() {
    if (this.ascResourceExplorerLink && this.resource != null) {
      this.resource[
        'ASCLink'
      ] = `<a href="${this.ascResourceExplorerLink}" target="_blank">Resource Explorer<i class="hyper-link-icon ml-1" aria-hidden="true"></i></a>`;
  ...
  downloadReport() {
    ...
    this._telemetryService.logEvent(
      TelemetryEventNames.DownloadReportButtonClicked,
      rSEventProperties
    );
    ...
    this._applensDiagnosticApiService
      .getDetector(
        'ResiliencyScore',
      ...
      .subscribe(
        (httpResponse: DetectorResponse) => {
          ...
          if (this.downloadReportFileName == undefined) {
            ...
            this.downloadReportFileName = `ResiliencyReport-${
            ...
            ResiliencyScoreReportHelper.generateResiliencyReport(
              httpResponse.dataset[0].table,
              `${this.downloadReportFileName}_(cached)`,
              this.generatedOn,
              this.vfsFonts
            );
          }
  ...
  coachMarkViewed() {
    ...
    try {
      ...
      loggingError.message =
        'Error accessing localStorage. Most likely accessed via in InPrivate or Incognito mode';
      loggingError.stack = error;
