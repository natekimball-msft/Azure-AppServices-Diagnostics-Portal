...
export class TabAnalysisComponent implements OnInit {
  ...
  constructor(
    ...
    this._activatedRoute.paramMap.subscribe((params) => {
      ...
      this._diagnosticService
        .getDetectorMetaDataById(this.analysisId)
        .subscribe((metaData) => {
      ...
      this._applensCommandBarService
        ...
        .subscribe((userSetting) => {
          if (userSetting && userSetting.favoriteDetectors) {
            const favoriteDetectorIds = Object.keys(
              userSetting.favoriteDetectors
            );
            this.pinnedDetector =
              favoriteDetectorIds.findIndex(
                (d) => d.toLowerCase() === this.analysisId.toLowerCase()
              ) > -1;
          }
        });
    });
  ...
  onDowntimeChanged(event: DownTime) {
    ...
    if (
      ...
      this._activatedRoute.firstChild == null ||
      !this._activatedRoute.firstChild.snapshot.paramMap.has('detector') ||
      this._activatedRoute.firstChild.snapshot.paramMap.get('detector').length <
        1
    ) {
      this._router.navigate([`./`], {
        relativeTo: this._activatedRoute,
        queryParams: {
          startTimeChildDetector: event.StartTime.format(this.stringFormat),
          endTimeChildDetector: event.EndTime.format(this.stringFormat)
        },
  ...
  emailToAuthor() {
    this._applensCommandBarService
      .getDetectorMeatData(this.analysisId)
      .subscribe((metaData) => {
  ...
  addOrRemoveDetector() {
    ...
    if (this.pinnedDetector) {
      this._telemetryService.logEvent(
        ...
        { detectorId: this.analysisId, location: 'CommandBar' }
      );
  ...
  private addFavoriteDetector() {
    this._applensCommandBarService
      .addFavoriteDetector(this.analysisId, DetectorType.Analysis)
      .subscribe(
        (message) => {
  ...
  private removeFavoriteDetector() {
    this._applensCommandBarService
      .removeFavoriteDetector(this.analysisId)
      .subscribe(
        (message) => {
          this.setPanelStatusAndMessage(HealthStatus.Success, message);
        },
