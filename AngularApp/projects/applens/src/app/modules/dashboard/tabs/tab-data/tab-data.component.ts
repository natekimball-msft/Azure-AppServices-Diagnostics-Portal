...
export class TabDataComponent implements OnInit {
    ...
    this._route.queryParams.subscribe((queryParams: Params) => {
      if (queryParams.detectorQueryParams != undefined) {
        this._detectorControlService.setDetectorQueryParams(
          queryParams.detectorQueryParams
        );
      } else {
        this._detectorControlService.setDetectorQueryParams('');
      }
      ...
      this.detector = this._route.snapshot.params['detector']
        ? this._route.snapshot.params['detector']
        : null;
      this._telemetryService.logEvent(
        TelemetryEventNames.DownloadReportButtonDisplayed,
        {
          DownloadReportButtonDisplayed:
            this.displayDownloadReportButton.toString(),
          'Detector:': this.detector,
          SubscriptionId: this._route.parent.snapshot.params['subscriptionid'],
          Resource: this.resource.SiteName
        }
      );
    });
  ...
  refresh() {
    ...
    this._diagnosticApiService
      .getDetectorMetaDataById(this.detector)
      .subscribe((metaData) => {
        if (metaData) {
    ...
    this._applensCommandBarService.getUserSetting().subscribe((userSetting) => {
      if (userSetting && userSetting.favoriteDetectors) {
        ...
        this.pinnedDetector =
          favoriteDetectorIds.findIndex(
            (d) => d.toLowerCase() === this.detector.toLowerCase()
          ) > -1;
      }
    });
  ...
  emailToAuthor() {
    this._applensCommandBarService
      .getDetectorMeatData(this.detector)
      .subscribe((metaData) => {
  ...
  addOrRemoveDetector() {
    ...
    if (this.pinnedDetector) {
      this._telemetryService.logEvent(
        TelemetryEventNames.FavoriteDetectorRemoved,
        { detectorId: this.detector, location: 'CommandBar' }
      );
      this.removeFavoriteDetector();
    } else {
      this._telemetryService.logEvent(
        TelemetryEventNames.FavoriteDetectorAdded,
        { detectorId: this.detector, location: 'CommandBar' }
      );
  ...
  private addFavoriteDetector() {
    ...
    const detectorType = this.detectorMetaData
      ? this.detectorMetaData.type
      : DetectorType.Detector;

    this._applensCommandBarService
      .addFavoriteDetector(this.detector, detectorType)
      .subscribe(
        (message) => {
  ...
  private removeFavoriteDetector() {
    this._applensCommandBarService
      .removeFavoriteDetector(this.detector)
      .subscribe(
        (message) => {
          this.setPanelStatusAndMessage(HealthStatus.Success, message);
        },
