...
import {
  SupportTopicItem,
  SupportTopicResult
} from '../resource-home/resource-home.component';
...
export class SupportTopicPageComponent implements OnInit {
  ...
  ngOnInit() {
    this.supportTopicName =
      this._activatedRoute.snapshot.params['supportTopic'];
    this._supportTopicService
      .getCategoryImage(this.supportTopicName)
    ...
    this._diagnosticService
      .getDetectors()
      .subscribe((detectors: DetectorMetaData[]) => {
        ...
        this._supportTopicService
          .getSupportTopics()
          .subscribe((allSupportTopics: SupportTopicResult[]) => {
            this.supportTopicsLoaded = true;
            this._telemetryService.logPageView(
              TelemetryEventNames.SupportTopicsLoaded,
              {}
            );
            let filteredSupportTopics = allSupportTopics.filter(
              (supportTopic) =>
                supportTopic.supportTopicL2Name === this.supportTopicName
            );

            filteredSupportTopics.forEach((sup: SupportTopicResult) => {
              this._supportTopicService
                .getCategoryImage(sup.supportTopicL3Name)
                .subscribe((iconString) => {
                  let icon = iconString;
                  let matchingDetector = this.supportTopicIdMapping.find(
                    (st) => st.supportTopic.id === sup.supportTopicId
                  );
                  ...
                  let item = new SupportTopicItem(
                    sup.supportTopicL2Name,
                    sup.productId,
                    matchingDetectorType,
                    sup.supportTopicId,
                    sup.supportTopicL3Name,
                    sup.supportTopicPath,
                    icon,
                    [],
                    matchingDetectorId,
                    matchingDetectorName,
                    matchingDetectorInternalOnly
                  );
                  if (
                    !this.supportTopics.find(
                      (st) => st.supportTopicL3Name === sup.supportTopicL3Name
                    )
                  ) {
  ...
  navigateToContent(supportTopic: SupportTopicItem) {
    if (
      supportTopic.detectorId != undefined &&
      supportTopic.detectorId !== ''
    ) {
      ...
      this.navigateTo(
        `../../pesId/${supportTopic.pesId}/supportTopics/${supportTopic.supportTopicId}`
      );
    }
  }
}
