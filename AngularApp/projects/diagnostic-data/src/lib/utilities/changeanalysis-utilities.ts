...
export class ChangeAnalysisUtilities {
  ...
  static readonly changeAnalysisSupportedResources = [
    'Microsoft.Sql',
    'Microsoft.Storage',
    'Microsoft.Web'
  ];
  ...
  public static prepareValuesForDiffView(diffvalue: any): DiffEditorModel {
    try {
      ...
      if (isString(diffvalue)) {
        ...
        if (
          jsonObject.hasOwnProperty('content') &&
      ...
      if (diffvalue instanceof Object || diffvalue instanceof Array) {
        if (
          diffvalue.hasOwnProperty('content') &&
          diffvalue['content'] != null
        ) {
          return {
  ...
  public static getImgPathForResource(searchResourceType: string): string {
    ...
    let resource = azureIconsList.find(
      (element) =>
        element.resourceType.toLowerCase() == searchResourceType.toLowerCase()
    );
    return resource
      ? resource.imgPath
      : ChangeAnalysisUtilities.basePath + 'AzureResource.png';
  }
  ...
  public static isResourceProviderSupported(provider: string): boolean {
    return (
      ChangeAnalysisUtilities.changeAnalysisSupportedResources.indexOf(
        provider
      ) >= 0
    );
  }
