...
export class UriUtilities {
  ...
  static buildSlotLink(
    resourceUri: string,
    isTargetingPreview: boolean
  ): string {
    const path = UriUtilities.getPathByResourceType(resourceUri);
    return `https://portal.azure.com/?websitesextension_ext=asd.ispreview%3D${isTargetingPreview}#@microsoft.onmicrosoft.com/resource/${StringUtilities.TrimBoth(
      resourceUri,
      '/'
    )}/${path}`;
  }
  ...
  private static getPathByResourceType(resourceUri: string): string {
    ...
    const type =
      `${resourceDescriptor.provider}/${resourceDescriptor.type}`.toLowerCase();
  ...
  static removeQueryParams(
    ...
    for (const key of allQueryParamKeys) {
      if (
        removeQueryList.findIndex(
          (q) => q.toLowerCase() === key.toLowerCase()
        ) === -1
      ) {
  ...
  static removeChildDetectorStartAndEndTime(allQueryParams: {
    [key: string]: any;
  }) {
    return UriUtilities.removeQueryParams(allQueryParams, [
  ...
  static addAdditionalQueryParams(
