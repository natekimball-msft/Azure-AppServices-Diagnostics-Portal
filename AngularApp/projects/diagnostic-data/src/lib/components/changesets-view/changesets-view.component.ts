...
@Component({
  ...
  styleUrls: [
    './changesets-view.component.scss',
    '../insights/insights.component.scss'
  ]
})
export class ChangesetsViewComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  private readonly noScanMsg: string =
    'No recent scans were performed on this web app.';
  ...
  constructor(
    ...
    protected changeAnalysisService: ChangeAnalysisService
  ) {
  ...
  private parseData(data: DataTableResponseObject) {
    ...
    if (rows.length > 0 && rows[0].length > 0) {
      ...
      this.changeSetText =
        batchChanges.length === 1
          ? `1 change detected`
          : `${batchChanges.length} changes have been detected`;
      this.changeSetText =
        resourceName != ''
          ? this.changeSetText + ` for ${resourceName}`
          : this.changeSetText;
      ...
      if (isAppService) {
        ...
        this.scanDate =
          rows[0][6] != ''
            ? 'Changes were last scanned on ' +
              moment.utc(rows[0][6]).format('YYYY-MM-DD HH:mm')
            : this.noScanMsg +
              ' Please enable Change Analysis using Change Analysis Settings.';
      } else {
        this.scanDate =
          rows[0][6] != ''
            ? 'Changes were last scanned on ' +
              moment.utc(rows[0][6]).format('YYYY-MM-DD HH:mm')
            : 'No recent scans were performed on this resource.';
      }
  ...
  private constructTimeline(data: DataTableResponseObject) {
    ...
    // Create a Timeline
    this.changesTimeline = new Timeline(
      container,
  ...
  private initializeChangesView(data: DataTableResponseObject) {
    ...
    let changeSetIdColumnIndex = DataTableUtilities.getColumnIndexByName(
      data,
      'ChangeSetId'
    );
    let inputsColumnIndex = DataTableUtilities.getColumnIndexByName(
      data,
      'Inputs'
    );
    let initiatedByColumnIndex = DataTableUtilities.getColumnIndexByName(
      data,
      'InitiatedBy'
    );
    ...
    if (latestChangeSet != null) {
      ...
      this.initiatedBy = this.changeSetsLocalCopy.hasOwnProperty(
        data.rows[0][0]
      )
        ? this.getInitiatedByUsers(this.changeSetsLocalCopy[data.rows[0][0]])
        : [];
      this.changeDetectorRef.detectChanges();
      this.loadingChangesTable = false;
      this.changesTableError = '';
  ...
  private triggerChangeEvent(properties: any): void {
    ...
    if (
      ...
      typeof properties.items[0] != 'undefined'
    ) {
  ...
  refreshChangesTable(): void {
    ...
    let changeSetIdDom = <HTMLInputElement>(
      document.getElementById('changeSetId')
    );
  ...
  private loadChangesTable(changeSetId: string): void {
    ...
    if (this.changeSetsCache.hasOwnProperty(changeSetId)) {
      ...
      this.initiatedBy = this.changeSetsLocalCopy.hasOwnProperty(changeSetId)
        ? this.getInitiatedByUsers(this.changeSetsLocalCopy[changeSetId])
        : [];
      ...
      let queryParams = `&fId=102&btnId=9&inpId=5&val=${encodeURIComponent(
        changeSetId
      )}`;
      if (!this.changeAnalysisService.getAppService()) {
        ...
        let resourceGroups =
          ChangeAnalysisUtilities.getResourceGroup(dependentResourceId);
        let provider =
          ChangeAnalysisUtilities.getResourceType(dependentResourceId);
        let resourceName = ChangeAnalysisUtilities.getResourceName(
          dependentResourceId,
          provider
        );
        queryParams += `&inpId=1&val=${encodeURIComponent(
          sub
        )}&inpId=2&val=${encodeURIComponent(
          resourceGroups
        )}&inpId=3&val=${encodeURIComponent(
          provider
        )}&inpId=4&val=${encodeURIComponent(resourceName)}`;
      }
      ...
      this.diagnosticService
        .getDetector(
          ...
          true,
          this.detectorControlService.isInternalView,
          queryParams
        )
        .subscribe(
          ...
          (error: any) => {
            this.loadingChangesTable = false;
            this.changesTableError =
              'Unable to load changes for the selected change group. Please try refresh or try after sometime';
          }
        );
    }
  }
    ...
    this.diagnosticService
      ...
      .subscribe(
        (response: DetectorResponse) => {
          ...
          if (submissionState == 'NotEnabled') {
            ...
            this.scanStatusMessage =
              'Unable to submit scan request. Please check if you have enabled scan for code changes in settings and try again.';
          } else {
        ...
        (error: any) => {
          this.scanState = '';
          this.scanStatusMessage =
            'Unable to submit scan request. Please refresh or try again after sometime';
        }
      );
  }
        ...
        (response: DetectorResponse) => {
          ...
          if (submissionState == 'Completed') {
            ...
            if (diff >= 60) {
              ...
              this.scanStatusMessage =
                'Scanning is complete. Click the below button to view the latest changes now.';
  ...
  private setScanState(submissionState: string, completedTime?: string) {
    switch (submissionState) {
      ...
      case 'InScan':
        this.scanStatusMessage =
          'Scanning is in progress. This may take few minutes';
        this.allowScanAction = false;
        break;
      case 'Completed':
        this.scanStatusMessage =
      ...
      case 'Failed':
        this.scanStatusMessage =
          'The last scan request failed. Click the below button to submit new scan request';
        this.allowScanAction = true;
      ...
      .subscribe(
        (response: DetectorResponse) => {
          ...
          if (totalItems > 0) {
            ...
            this.scanDate =
              'Changes were last scanned on ' +
              moment.utc(newChangeRows.rows[0][6]).format('YYYY-MM-DD HH:mm');
          } else {
  ...
  setDefaultScanStatus(): void {
    this.scanStatusMessage =
      'Click the below button to to scan your resource and get the latest changes';
    this.allowScanAction = true;
    this.showViewChanges = false;
    this.scanState = '';
  }
  ...
  logTimelineEventClick(): void {
    ...
    this.logEvent(
      TelemetryEventNames.ChangeAnalysisTimelineClicked,
      eventProps
    );
  }
  ...
  getColumnIndexByName(searchColumnName: string): number {
    return this.changeSetsColumn.findIndex(
      (column) =>
        column.columnName.toLowerCase() === searchColumnName.toLowerCase()
    );
  }
  ...
  getInitiatedByUsers(changeSet: any[]): any[] {
    ...
    if (initiatedByListIndex >= 0) {
      return changeSet[initiatedByListIndex].length > 0
        ? changeSet[initiatedByListIndex]
        : ['Unable to Determine'];
    }
    if (
      changeSet[initiatedByIndex] === null ||
      changeSet[initiatedByIndex] === ''
    ) {
      return ['N/A'];
    }
