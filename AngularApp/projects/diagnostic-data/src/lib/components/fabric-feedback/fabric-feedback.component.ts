  ...
  implements AfterViewInit, OnInit, OnDestroy
{
  ...
  feedbackIcons: { id: string; text: string; ariaLabel: string }[] = [
    {
      ...
      text: 'dissatisfied',
      ariaLabel:
        'Are you satisfied with your experience? Dissatisfied Radio button 1 of 3'
    },
    {
      id: 'EmojiNeutral',
      text: 'ok',
      ariaLabel:
        'Are you satisfied with your experience? Ok Radio button 2 of 3'
    },
    {
      id: 'Emoji2',
      text: 'satisfied',
      ariaLabel:
        'Are you satisfied with your experience? Satisfied Radio button 3 of 3'
    }
  ];
  ...
  submitFeedback() {
    ...
    const detectorName =
      this.feedbackPanelConfig.detectorName || this.globals.getDetectorName();
    if (this.isPublic) {
      const isHomepage =
        !this.activatedRoute.root.firstChild.firstChild.firstChild.firstChild
          .snapshot.params['category'];
      this.ratingEventProperties['Place'] = isHomepage
        ? 'LandingPage'
        : 'CategoryPage';
    } else {
      const user = this.globals.getUserAlias();
      this.ratingEventProperties['User'] = user;
      this.ratingEventProperties['Place'] = detectorName
        ? 'OverviewPage'
        : 'DetectorPage';
    }
  ...
  onOpenFeedbackPanel() {
    ...
    if (
      this.isPublic &&
      globals.messagesData.feedbackPanelConfig != null &&
      globals.messagesData.feedbackPanelConfig.url.split('?')[0] ==
    ...
    if (
      document.getElementsByName('feedback-icon-rating') != undefined &&
      document.getElementsByName('feedback-icon-rating').length > 0 &&
      document.getElementsByName('feedback-icon-rating')[0] != undefined
    ) {
      (<HTMLElement>(
        document.getElementsByName('feedback-icon-rating')[0]
      )).focus();
    }
  }
