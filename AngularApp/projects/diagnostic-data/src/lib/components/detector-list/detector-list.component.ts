...
export class DetectorListComponent extends DataRenderBaseComponent {
  ...
  private getResponseFromResource() {
    ...
    if (isFromDependentResource) {
      this.parseResourceService
        .checkIsResourceSupport(this.overrideResourceUri, false)
        .subscribe((error) => {
  ...
  private getDetectorResponses(): void {
    this._diagnosticService.getDetectors(this.overrideResourceUri).subscribe(
      (detectors) => {
  ...
  private getDetectorViewModel(
    ...
    const viewModel: DetectorViewModel = {
      ...
      request: this._diagnosticService.getDetector(
        ...
        this._detectorControl.shouldRefresh,
        this._detectorControl.isInternalView,
        queryString,
        overwriteResourceUrl
      )
    };
  ...
  toggleDetectorHeaderStatus(viewModel: any) {
    viewModel.expanded =
      viewModel.loadingStatus === LoadingStatus.Success && !viewModel.expanded;
  ...
  checkIsFromDependentResource(): boolean {
    if (
      !this.renderingProperties.resourceUri ||
      this.renderingProperties.resourceUri === ''
    )
      return false;
  ...
  public openBladeDiagnoseDetectorId(
    ...
    if (viewModel != null && viewModel.model.metadata.id) {
      ...
      if (viewModel.model.metadata.category) {
        ...
        category = this._router.url.split('/')[11]
          ? this._router.url.split('/')[11]
  ...
  startDetectorRendering(detectorList: DetectorMetaData[]) {
    ...
    this.detectorMetaData = detectorList.filter(
      (detector) =>
        this.renderingProperties.detectorIds.indexOf(detector.id) >= 0
    );
    let detectorMetaData1 = this.renderingProperties.detectorIds.filter(
      (id) =>
        this.detectorMetaData.findIndex((metaData) => metaData.id == id) >= 0
    );
    detectorList.filter(
      (detector) =>
        this.renderingProperties.detectorIds.indexOf(detector.id) >= 0
    ...
    this.detectorViewModels = this.detectorMetaData.map((detector) =>
      this.getDetectorViewModel(
        detector,
        this.renderingProperties.additionalParams,
        this.overrideResourceUri
      )
    );
    ...
    this.detectorViewModels.forEach((viewModel, index) => {
      requests.push(
        (<Observable<DetectorResponse>>viewModel.request).pipe(
          map((response: DetectorResponse) => {
            this.detectorViewModels[index] =
              this.updateDetectorViewModelSuccess(viewModel, response);

            this.loading =
              this.detectorViewModels.findIndex(
                (vm) => vm.loadingStatus === LoadingStatus.Loading
              ) > -1
            ...
            if (
              ...
              if (
                ...
                let insightInfo = this.getDetectorInsightInfo(
                  this.detectorViewModels[index]
                );
                let issueDetectedViewModel: DetectorViewModeWithInsightInfo = {
                ...
                this.issueDetectedViewModels =
                  this.issueDetectedViewModels.sort((n1, n2) => {
                ...
                let successViewModel: DetectorViewModeWithInsightInfo = {
                  model: this.detectorViewModels[index],
                  ...insightInfo
                };
  ...
  public selectDetector(viewModel: DetectorViewModeWithInsightInfo) {
    if (viewModel != null && viewModel.model.metadata.id) {
      ...
      if (targetDetector !== '') {
        ...
        this.queryParams = UriUtilities.removeChildDetectorStartAndEndTime(
        ...
        if (targetDetector === 'appchanges' && this.isPublic) {
          ...
          if (this.isPublic && !(this.overrideResourceUri == '')) {
            ...
            const routeUrl = url.endsWith('/overview')
              ? `../detectors/${targetDetector}`
              : `../../detectors/${targetDetector}`;
            ...
            this._router.navigate(
              [`${resourceId}/detectors/${targetDetector}`],
              { queryParams: combinedParams }
            );
          }
        }
    ...
    if (viewModel != null && viewModel.model.metadata.id) {
      ...
      if (targetDetector !== '') {
        ...
        Object.keys(combinedParams).forEach((x) => {
          paramString =
            paramString === ''
              ? `${paramString}${x}=${combinedParams[x]}`
              : `${paramString}&${x}=${combinedParams[x]}`;
        });
        const linkAddress =
          this.overrideResourceUri == ''
            ? `${resourceId}/detectors/${targetDetector}?${paramString}`
            : `${this.overrideResourceUri}/detectors/${targetDetector}?${paramString}`;
