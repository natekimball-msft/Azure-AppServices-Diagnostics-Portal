...
export class FormStepView extends StepView {
  ...
  constructor(view: FormStepView) {
    ...
    for (let i = 0; i < view.inputs.length; ++i) {
      if (
        view.inputs[i].itype == InputType.DropDown &&
        view.inputs[i].options == null
      ) {
        throw new Error('dropdown option should not be null');
      }
    }
  ...
  try {
    ...
    var result = markdown
      ...
      .map((s) =>
        s.replace(
          regex,
          `<a target="_blank" href="$2" title="$4" onclick="networkCheckLinkClickEventLogger('${id}','$2', '$1')">$1</a>`
        )
      )
      .join('![');
  } catch (e) {
...
export class StepFlowManager {
        ...
        for (var i = 0; i < views.length; ++i) {
          ...
          if (this._currentFlow != null) {
            ...
            this._telemetryService.logEvent('NetworkCheck.ViewLog', {
              ...
              views: PIIUtilities.removePII(
                JSON.stringify(this._viewLogs, (key, value) =>
                  ({ container: true }[key] ? undefined : value)
                )
              ),
  ...
  public addView(
    viewPromise: StepView | Promise<StepView>,
    loadingText?: string
  ) {
    return this.addViews(
      Promise.resolve(viewPromise).then((v) => [v]),
      loadingText
    );
  }
  ...
  public addViews(
    ...
    this._stepViewQueue[idx].loadingText =
      loadingText || this._defaultLoadingText;
    return idx;
  }
  ...
  private generateLogEventFunc(flow: StepFlow) {
    ...
    return (eventName: string, payload: any) =>
      telemetryService.logEvent(`NetworkCheck.Flow.${eventName}`, {
        flowId: flow.id,
        payload,
        resourceUri: this._resourceUri,
        sessionId: this._sessionId
