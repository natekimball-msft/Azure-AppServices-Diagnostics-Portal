...
export class HighchartsGraphComponent implements OnInit {
  ...
  public static chartProperties: {
    [chartContainerId: string]: KeyValue<string, any>[];
  } = {};
  public static getChartProperty(
    ...
    if (chartContainerId != '') {
      let retVal = null;
      if (
        !!this.chartProperties[chartContainerId] &&
        this.chartProperties[chartContainerId].length > 0
      ) {
  ...
  public static addOrUpdateChartProperty(
    propertyName: string,
    propertyValue: any,
    chartContainerId: string
  ): boolean {
    if (chartContainerId == '') {
      ...
      let existingValue = HighchartsGraphComponent.getChartProperty(
        propertyName,
        chartContainerId
      );
      if (existingValue) {
        HighchartsGraphComponent.chartProperties[chartContainerId].some(
          (prop) => {
            if (prop.key == propertyName) {
        ...
        if (
          HighchartsGraphComponent.chartProperties[chartContainerId] == null
        ) {
  ...
  private _updateChartCursor(
    currChart: Highcharts.Chart,
    cursorValue: string
  ): void {
    if (currChart) {
      if (this.getCurrentChartContainerId()) {
        let currChartContainer = <HTMLElement>(
          document.getElementById(this.getCurrentChartContainerId())
        );
        if (
          currChartContainer.getElementsByClassName('highcharts-plot-border') &&
          currChartContainer.getElementsByClassName('highcharts-plot-border')
            .length > 0
        ) {
          let plotBorderElement = <HTMLElement>(
            currChartContainer.getElementsByClassName(
              'highcharts-plot-border'
            )[0]
          );
          if (
            !!plotBorderElement &&
            (!this._updateChartCursorTimer ||
              (!!this._updateChartCursorTimer &&
                this._updateChartCursorTimer.closed))
          ) {
            this._updateChartCursorTimer = interval(50).subscribe(() => {
              if (
                plotBorderElement.getAttribute('cursor') != cursorValue &&
                plotBorderElement.getAttribute('fill') != 'white'
              ) {
  ...
  private _handleZoomBehaviorUpdate(value: zoomBehaviors) {
    if (value & zoomBehaviors.GeryOutGraph) {
      ...
      if (currChart) {
        let currPlotBands = [];
        if (
          currChart.options.xAxis instanceof Array &&
          currChart.options.xAxis.length > 0
        ) {
          currPlotBands = currChart.options.xAxis[0].plotBands;
        }
        let alreadyGreyedOut =
          currPlotBands.length > 1 &&
          currPlotBands.some((plotBand) => plotBand.color === '#e5e5e5');
        if (!alreadyGreyedOut) {
          currPlotBands.push({
            color: '#e5e5e5',
  ...
  @Input() public set zoomBehavior(value: zoomBehaviors) {
    ...
    if (
      !this._zoomBehaviorUpdateTimer ||
      (!!this._zoomBehaviorUpdateTimer && this._zoomBehaviorUpdateTimer.closed)
    ) {
      this._zoomBehaviorUpdateTimer = interval(100).subscribe(() => {
        if (
          HighchartsGraphComponent.addOrUpdateChartProperty(
            'zoomBehavior',
            this._zoomBehavior,
            this.getCurrentChartContainerId()
          )
        ) {
  ...
  private getCurrentChartContainerId(): string {
    if (
      this.el.nativeElement.getElementsByClassName('highcharts-container') &&
      this.el.nativeElement.getElementsByClassName('highcharts-container')
        .length > 0
    ) {
  ...
  highchartCallback: Highcharts.ChartLoadCallbackFunction = function () {
    ...
    chart.customNamespace['toggleSelectionButton'] = chart.renderer
      .button(
        'None',
        null,
        -10,
        function () {
          ...
          var statusToSet =
            this.text && this.text.textStr && this.text.textStr === 'All'
              ? true
              : false;
          ...
          var ariaLabel = statusToSet
            ? 'Select all the series'
            : 'Deselect all the series';
          this.attr({
            role: 'button',
            text: textStr,
          ...
          keyCodeMap: [
            ...
            [
              ...
              function () {
                ...
                var buttonElement =
                  namespace['toggleSelectionButton'] &&
  ...
  private customChartSelectionCallbackFunction: Highcharts.ChartSelectionCallbackFunction =
    (event: Highcharts.ChartSelectionContextObject) => {
      if (this._zoomBehavior & zoomBehaviors.FireXAxisSelectionEvent) {
        if (event.xAxis) {
          let fromSelection = moment.utc(
            Highcharts.dateFormat('%Y-%m-%d %H:%M:00', event.xAxis[0].min)
          );
          let toSelection = moment.utc(
            Highcharts.dateFormat('%Y-%m-%d %H:%M:00', event.xAxis[0].max)
          );
          ...
          if (
            !!currChart &&
            !!currChart.series &&
            currChart.series.length > 0
          ) {
            let firstNonEmptySeries: number = -1;
            for (let index = 0; index < currChart.series.length; index++) {
              if (
                currChart.series[index].visible &&
                currChart.series[index].xAxis.hasData()
              ) {
              ...
              currChart.series[firstNonEmptySeries].points.forEach((pt) => {
                if (
                  moment
                    .duration(
                      moment
                        .utc(Highcharts.dateFormat('%Y-%m-%d %H:%M:00', pt.x))
                        .diff(fromSelection)
                    )
                    .asMinutes() < 4
                ) {
                  fromPoint = moment.utc(
                    Highcharts.dateFormat('%Y-%m-%d %H:%M:00', pt.x)
                  );
                }
                if (
                  toPoint == null &&
  ...
  private customSetExtremesCallbackFunction: Highcharts.AxisSetExtremesEventCallbackFunction =
      ...
      if (evt.trigger !== 'sync') {
        ...
        for (let i = 0; i < Highcharts.charts.length; i++) {
          ...
          if (chart && currChart !== chart) {
            let targetZoomBehavior: zoomBehaviors =
              HighchartsGraphComponent.getChartProperty(
                'zoomBehavior',
                chart.container.id
              ) as zoomBehaviors;
            if (
              targetZoomBehavior == null ||
              !(targetZoomBehavior & zoomBehaviors.CancelZoom)
            ) {
              if (chart.xAxis[0].setExtremes) {
                // It is null while updating
                chart.xAxis[0].setExtremes(evt.min, evt.max, true, true, {
                  trigger: 'sync'
                });
              }
            }
  ...
  onClick(ev: MouseEvent) {
    if (this.zoomBehavior & zoomBehaviors.ShowXAxisSelectionDisabledMessage) {
      ...
      if (currChart) {
        ...
        let normalizedEvent: PointerEventObject =
          currChart.pointer.normalize(ev);
        let msg = currChart.renderer
          .label(
            waitMSG,
  ...
  onKeydownHandler(ev: KeyboardEvent) {
    ...
    if (key === 'ArrowLeft' || key === 'ArrowRight') {
      let currentId = this.el.nativeElement.getElementsByClassName(
        'highcharts-container'
      )[0].id;
  ...
  private updateHighChartTheme(theme: string) {
    if (
      ...
      !!this.themeService &&
      this.currentTheme &&
      theme.toLocaleLowerCase() !== this.currentTheme
    ) {
  ...
  constructor(
    ...
    private highChartsHoverService: HighChartsHoverService,
    private themeService: GenericThemeService
  ) {
  ...
  ngOnInit() {
    this.backgroundColor =
      this.themeService.getPropertyValue('--bodyBackground');
  ...
  private syncCharts(ev: MouseEvent) {
    ...
    let currentCharts = this.el.nativeElement.getElementsByClassName(
      'highcharts-container'
    );
    ...
    for (let i = 0; i < Highcharts.charts.length; i++) {
      ...
      if (chart) {
        if (currentId === chart.container.id) {
          ...
          let bbLeft =
            this.el.nativeElement.offsetLeft + chart.plotLeft + sideNavWidth;
    ...
    if (!!this.xAxisPlotBands && this.xAxisPlotBands.length > 0) {
      ...
      this.xAxisPlotBands.forEach((plotBand) => {
        var currPlotBand = {
          ...
          zIndex: plotBand.style
            ? plotBand.style
            : xAxisPlotBandStyles.BehindPlotLines,
          borderWidth:
            !!plotBand.borderWidth && plotBand.borderWidth > 0
    ...
    this.options = {
      ...
      xAxis: {
        ...
        tickFormat: function (d: any) {
          return moment(d).utc().format('MM/DD HH:mm');
        },
