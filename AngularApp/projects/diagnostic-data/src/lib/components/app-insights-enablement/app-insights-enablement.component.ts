...
export class AppInsightsEnablementComponent implements OnInit {
  constructor(
    private _appInsightsService: AppInsightsQueryService,
  ...
  ngOnInit() {
    if (this.isEnabledInProd) {
      ...
      this._appInsightsService.loadAppInsightsResourceObservable.subscribe(
        (loadStatus) => {
          if (loadStatus === true) {
            let appInsightsSettings =
              this._appInsightsService.appInsightsSettings;
            ...
            if (this.isAppInsightsEnabled) {
              this._appInsightsService.logAppInsightsEvent(
                this.resourceId,
                TelemetryEventNames.AppInsightsEnabled
              );
              this._appInsightsService
                .getAppInsightsConnected(this.resourceId)
                .subscribe((connected) => {
                  if (connected) {
                    this._appInsightsService.logAppInsightsEvent(
                      this.resourceId,
                      TelemetryEventNames.AppInsightsAlreadyConnected
                    );
                    this.isAppInsightsConnected = true;
                    this._appInsightsService
                      .getAppInsightsStoredConfiguration(this.resourceId)
                      .subscribe(
                        (storedResponse) => {
                          if (
                            storedResponse &&
                            storedResponse.AppId &&
                            storedResponse.ApiKey
                          ) {
                            const additionalHeaders = new HttpHeaders({
                              'appinsights-app-id': storedResponse.AppId,
                              'appinsights-encryptedkey': storedResponse.ApiKey
                            });
                            this._backendCtrlService
                              .get<any>(
                                `api/appinsights/validate`,
                                additionalHeaders,
                                true
                              )
                              .subscribe(
                                (resp) => {
                                  if (resp.isValid === true) {
                                    ...
                                    if (
                                      resp.updatedEncryptionBlob != null &&
                                      resp.updatedEncryptionBlob.length > 1
                                    ) {
                                      this._appInsightsService
                                        .updateAppInsightsEncryptedAppSettings(
                                          resp.updatedEncryptionBlob,
                                          storedResponse.AppId
                                        )
                                        .subscribe(
                                          (appSettingUpdated) => {
                                            if (appSettingUpdated) {
                                              this._appInsightsService.logAppInsightsEvent(
                                                this.resourceId,
                                                TelemetryEventNames.AppInsightsAppSettingsUpdatedWithLatestSecret
                                              );
                                            }
                                ...
                                (error) => {
                                  ...
                                  if (error.status === 403) {
                                    this._appInsightsService.logAppInsightsEvent(
                                      this.resourceId,
                                      TelemetryEventNames.AppInsightsConfigurationInvalid
                                    );
                                  } else {
                                    this._appInsightsService.logAppInsightsEvent(
                                      this.resourceId,
                                      TelemetryEventNames.AppInsightsFailedDuringKeyValidation
                                    );
                                    if (error.error) {
                                      this.appInsightsValiationError +=
                                        ' - ' + error.error;
                    ...
                    this._appInsightsService
                      .checkAppInsightsAccess(this.appInsightsResourceUri)
                      .subscribe(
                        (hasWriteAccess) => {
                          if (!hasWriteAccess) {
                            ...
                            this._appInsightsService.logAppInsightsEvent(
                              this.resourceId,
                              TelemetryEventNames.AppInsightsResourceMissingWriteAccess
                            );
                          } else {
                            this._appInsightsService
                              .getAppInsightsApiKeysCount()
                              .subscribe((keyCount) => {
                                ...
                                if (keyCount < maxApiKeysPerAiResource) {
                                  ...
                                  this._appInsightsService.logAppInsightsEvent(
                                    this.resourceId,
                                    TelemetryEventNames.AppInsightsMaxApiKeysExceeded
                                  );
                                }
                              });
                          }
                        ...
                        (errorCheckingAccess) => {
                          this._appInsightsService.logAppInsightsError(
                            this.resourceId,
                            TelemetryEventNames.AppInsightsAccessCheckError,
                            errorCheckingAccess
                          );
                          this.loadingSettings = false;
                          this.hasWriteAccess = false;
            ...
            if (
              this._appInsightsService.appInsightsSettings
                .appSettingsHaveInstrumentationKey &&
              ...
              this._appInsightsService.logAppInsightsEvent(
                this.resourceId,
                TelemetryEventNames.AppInsightsFromDifferentSubscription
              );
            } else {
              this._appInsightsService.logAppInsightsEvent(
                this.resourceId,
                TelemetryEventNames.AppInsightsNotEnabled
              );
            }
            this.loadingSettings = false;
          }
  ...
  enable() {
    this._appInsightsService.logAppInsightsEvent(
      this.resourceId,
      TelemetryEventNames.AppInsightsEnableClicked
    );
      ...
      .subscribe(
        (resp) => {
          ...
          if (resp) {
            ...
            this._appInsightsService.logAppInsightsEvent(
              this.resourceId,
              TelemetryEventNames.AppInsightsConnected
            );
          }
        },
        (error) => {
          ...
          this._appInsightsService.logAppInsightsError(
            this.resourceId,
            TelemetryEventNames.AppInsightsConnectionError,
            this.error
          );
        }
      );
