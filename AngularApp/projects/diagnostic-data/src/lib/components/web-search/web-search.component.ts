  ...
  implements OnInit
{
  ...
  @Input() webSearchConfig: WebSearchConfiguration;
  @Input() searchResults: any[] = [];
  @Input() numArticlesExpanded: number = 5;
  @Output() searchResultsChange: EventEmitter<any[]> = new EventEmitter<
    any[]
  >();
  pesId: string = '';
  sapProductId: string = '';
  ...
  preLoadingErrorMessage: string =
    'Some error occurred while fetching web results.';
  ...
  triggerSearch() {
    ...
    var shouldGetResultsFromDeepSearch =
      this.isPublic && this.deepSearchEnabled;
  ...
  private getInputsForDeepSearch() {
    ...
    if (this.webSearchConfig) {
      ...
      if (
        ...
        this.webSearchConfig.ExcludedSites.length > 0
      ) {
  ...
  private getDeepSearchTask(query: Query) {
    return this._contentService.fetchResultsFromDeepSearch(query).pipe(
      map((res) => res),
      retryWhen((errors) => {
        let numRetries = 0;
        return errors.pipe(
  ...
  viewOrHideAnchorTagText(
    ...
    if (totalDocuments && numDocumentsExpanded) {
      ...
      remainingDocuments = viewRemainingArticles
        ? `last ${remainingDocuments} `
        : remainingDocuments;
    }

    return !viewRemainingArticles
  ...
  checkIfDeepSearchIsEnabled() {
    var deepSearchObservable = this.isPublic
      ? this._contentService.IsDeepSearchEnabled(
          this.getEffectiveProductId(),
          this.getEffectiveSupportTopicId()
        )
      : this._documentsSearchService.IsEnabled(this.getEffectiveProductId());

    let checkStatusTask = deepSearchObservable.pipe(
      map((res) => res),
      retryWhen((errors) => {
        let numRetries = 0;
        return errors.pipe(
