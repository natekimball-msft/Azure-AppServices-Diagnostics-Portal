...
import {
  ...
  DiagnosticData,
  GanttChartInnerMarkdownPosition,
  RenderingType,
  TimeSeriesRendering,
  TimeSeriesType
  ...
  implements OnInit, DataRenderer
{
  ...
  processData(data: DiagnosticData) {
    ...
    if (data) {
      ...
      this.defaultValue =
        this.renderingProperties.defaultValue !== null
          ? this.renderingProperties.defaultValue
          : this.defaultValue;
      ...
      this.customizeXAxis =
        this.graphOptions &&
        this.graphOptions.customizeX &&
        this.graphOptions.customizeX === 'true';
      ...
      if (chartRowData != null) {
        ...
        if (
          ...
          chartRowData.push({
            ...
            EndTime: momentNs.utc(currentTime).add(timeGrain, 'm').toISOString()
          });
        } else {
  ...
  private _processDiagnosticDataForOtherCharts(data: DiagnosticData) {
    ...
    if (counterNameColumnIndex >= 0) {
      ...
      uniqueCounterNames = data.table.rows
    ...
    numberValueColumns.forEach((column) => {
      if (uniqueCounterNames.length > 0) {
        uniqueCounterNames.forEach((counterName) => {
          const seriesName = this._getSeriesName(
            column.columnName,
            counterName
          );
          timeSeriesDictionary[seriesName] = <TimeSeries>{
            name: seriesName,
            series: <GraphSeries>{ key: seriesName, values: [] }
          };
          ...
          highchartTimeSeriesDictionary[seriesName] = <HighChartTimeSeries>{
            name: seriesName,
      ...
      numberValueColumns.forEach((column) => {
        ...
        if (columnIndex > -1 && row[columnIndex] != null) {
          const point: TablePoint = <TablePoint>{
            ...
            counterName:
              counterNameColumnIndex >= 0 ? row[counterNameColumnIndex] : null
          };
    ...
    Object.keys(timeSeriesDictionary).forEach((key) => {
      const pointsForThisSeries = tablePoints
        .filter(
          (point) =>
            this._getSeriesName(point.column, point.counterName) === key
        )
        .sort((b, a) =>
          !this.customizeXAxis
            ? a.timestamp.diff(b.timestamp)
      ...
      if (!this.customizeXAxis) {
        ...
        while (
          pointToAdd &&
          pointToAdd.timestamp &&
          pointToAdd.timestamp.isBefore(this.startTime)
        ) {
        ...
        for (
          ...
          timeSeriesDictionary[key].series.values.push(<GraphPoint>{
            x: d.clone(),
            y: value
          });
          highchartTimeSeriesDictionary[key].series.data.push([
            d.clone().valueOf(),
            value
          ]);
        }
        ...
        pointsForThisSeries.forEach((pointToAdd) => {
          ...
          timeSeriesDictionary[key].series.values.push(<GraphPoint>{
            x: momentNs.utc(pointToAdd.timestamp),
            y: pointToAdd.value
          });
          highchartTimeSeriesDictionary[key].series.data.push([
            momentNs.utc(pointToAdd.timestamp).valueOf(),
            pointToAdd.value
          ]);
        });
      }
  ...
  private _getGreatestCommonFactor(
    timestamp: momentNs.Moment
  ): momentNs.Duration {
    const minuteGcf = this._gcd(
      timestamp.minutes(),
      this.timeGrain.asMinutes()
    );
    if (minuteGcf !== 60) {
      return moment.duration(minuteGcf, 'minutes');
    }

    const hourGcf = this._gcd(timestamp.hours(), this.timeGrain.asHours());
    if (hourGcf !== 24) {
      return moment.duration(hourGcf, 'hours');
    }

    const daysInMonth = timestamp.daysInMonth();
    const daysGcf =
      hourGcf === 24 ? this._gcd(timestamp.days(), daysInMonth) : 0;
    if (daysGcf !== daysInMonth) {
      return moment.duration(daysGcf, 'days');
    }
  ...
  private _prepareStartAndEndTime() {
    ...
    TimeUtilities.roundDown(end, this.timeGrain);
      ...
      .minute(end.minute() - (end.minute() % this.timeGrain.minutes()))
      .second(0);
  ...
  private _getCounterNameColumnIndex(): number {
    const columnIndex = this.renderingProperties.counterColumnName
      ...
      : this.dataTable.columns.findIndex(
          (column) => column.dataType === DataTableDataType.String
        );
  ...
  private _getCounterValueColumns() {
    const columns = this.renderingProperties.seriesColumns
      ? this.dataTable.columns.filter(
          (column) =>
            this.renderingProperties.seriesColumns.indexOf(column.columnName) >=
            0
        )
      : this.dataTable.columns.filter(
          (column) =>
            DataTableDataType.NumberTypes.indexOf(column.dataType) >= 0
        );
