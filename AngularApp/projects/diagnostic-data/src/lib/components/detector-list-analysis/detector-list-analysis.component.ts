...
import {
  detectorSearchEnabledPesIds,
  detectorSearchEnabledPesIdsInternal
} from '../../models/search';
...
} from '../../services/generic-breadcrumb.service';
...
const DEFAULT_DESCRIPTION_FOR_DETECTORS_WITH_NO_INSIGHT: string =
...
export class DetectorListAnalysisComponent
  ...
  implements OnInit
{
  ...
  preLoadingErrorMessage: string =
    'Some error occurred while fetching diagnostics.';
  ...
  constructor(
    ...
    public _appInsightsService: AppInsightsQueryService,
    private _supportTopicService: GenericSupportTopicService,
    protected _globals: GenieGlobals,
    private _solutionService: SolutionService,
    @Inject(DIAGNOSTIC_DATA_CONFIG) config: DiagnosticDataConfig,
    private portalActionService: PortalActionGenericService,
    private _resourceService: GenericResourceService,
    private _genericBreadcrumbService: GenericBreadcrumbService,
    private _genericUserSettingsService: GenericUserSettingService,
    private _genericCategoryService: GenericFeatureService
  ) {
    ...
    if (this.isPublic) {
      this._appInsightsService
        .CheckIfAppInsightsEnabled()
        .subscribe((isAppinsightsEnabled) => {
  ...
  ngOnInit() {
    this.withinGenie =
      this.analysisId === 'searchResultsAnalysis' &&
    ...
    if (
      (this.analysisId === 'searchResultsAnalysis' ||
        this.analysisId === 'supportTopicAnalysis') &&
      this.searchTerm &&
      this.searchTerm.length > 0
    ) {
      this.refresh();
    ...
    this._genericUserSettingsService
      ...
      .subscribe((expandAnalysisCheckCard) => {
    ...
    this._diagnosticService.getDetectors().subscribe((detectors) => {
      const metaData = detectors.find(
        (d) => d.id === this.analysisId && d.type === DetectorType.Analysis
      );
  ...
  getApplicationInsightsData(response: DetectorResponse) {
    ...
    let appInsightDiagnosticData = response.dataset.filter(
      (data) =>
        (<Rendering>data.renderingProperties).type ===
        RenderingType.ApplicationInsightsView
    );
    ...
    if (
      this.isPublic &&
      this.appInsightQueryMetaDataList &&
      this.appInsightQueryMetaDataList.length > 0
    ) {
      this._appInsightsService.loadAppInsightsResourceObservable.subscribe(
        (loadStatus) => {
          if (loadStatus === true) {
            this.loadingAppInsightsResource = false;
            this.appInsightQueryMetaDataList.forEach((appInsightData) => {
              this._appInsightsService
                .ExecuteQuerywithPostMethod(appInsightData.query)
                .subscribe((data) => {
                  if (data && data['Tables']) {
  ...
  getQueryParamsForAnalysisDetector(): string {
    ...
    let queryParamsToSkipForAnalysisDetectors = [
      'startTimeChildDetector',
      'endTimeChildDetector'
    ];
  ...
  analysisContainsDowntime(): Observable<boolean> {
    ...
    return this._diagnosticService
      .getDetector(
        ...
        this._detectorControl.endTimeString,
        false,
        this._detectorControl.isInternalView,
        this.getQueryParamsForAnalysisDetector()
      )
      .pipe(
        map((response: DetectorResponse) => {
          let downTimeRenderingType = response.dataset.find(
            (set) =>
      ...
      this._activatedRoute.paramMap.subscribe((params) => {
        this.analysisId =
          ...
          !!params.get('analysisId')
            ...
            : this.analysisId;
        this.detectorId =
          params.get(this.detectorParmName) === null
            ? ''
            : params.get(this.detectorParmName);
        if (
          this.detectorId == '' &&
          !!this._activatedRoute.firstChild &&
          this._activatedRoute.firstChild.snapshot &&
          this._activatedRoute.firstChild.snapshot.paramMap.has(
            this.detectorParmName
          ) &&
          this._activatedRoute.firstChild.snapshot.paramMap.get(
            this.detectorParmName
          ).length > 1
        ) {
          this.detectorId =
        ...
        if (this.analysisId != 'searchResultsAnalysis' && this.detectorId == '')
                ...
                if (this.searchTerm && this.searchTerm.length > 1) {
                  ...
                  if (!!this.detectorId && this.detectorId !== '') {
                    ...
                    this._diagnosticService
                      .getDetectors()
                      .subscribe((detectorList) => {
                        if (detectorList) {
                          if (this.detectorId !== '') {
                            let currentDetector = detectorList.find(
                              (detector) => detector.id == this.detectorId
                            );
              ...
              if (this.analysisId == 'supportTopicAnalysis') {
                ...
                this._diagnosticService
                  ...
                  .subscribe((detectorList) => {
                    if (detectorList) {
                      this._supportTopicService
                        .getMatchingDetectors()
                        .subscribe((matchingDetectors) => {
                          var analysisToProcess: string[] = [];
                          matchingDetectors.forEach((matchingDetector) => {
                            if (
                              matchingDetector.type === DetectorType.Analysis
                            ) {
                            ...
                            this.insertInDetectorArray({
                              name: matchingDetector.name,
                              id: matchingDetector.id
                            });
                          });

                          if (
                            analysisToProcess &&
                            analysisToProcess.length > 0
                          ) {
                            analysisToProcess.forEach((analysisId) => {
                              let childrenOfCurrentAnalysis =
                                this.getChildrenOfAnalysis(
                                  analysisId,
                                  detectorList
                                );
                              if (
                                childrenOfCurrentAnalysis &&
                                childrenOfCurrentAnalysis.length > 0
                              ) {
                                childrenOfCurrentAnalysis.forEach(
                                  (child: DetectorMetaData) => {
                                    this.insertInDetectorArray({
                                      name: child.name,
                                      id: child.id
                                    });
                          ...
                          if (this.detectorId) {
                            ...
                            this.detectorName = currentDetector.name;
                            let currViewModel = {
                              model: this.getDetectorViewModel(
                                currentDetector,
                                currDowntime,
                                containsDownTime
                              ),
                            ...
                            if (
                              currViewModel.model.startTime != null &&
                              currViewModel.model.endTime != null
                            ) {
                              this._router.navigate(
                                [`./detectors/${currentDetector.id}`],
                                {
                                  relativeTo: this._activatedRoute,
                                  queryParams: {
                                    startTime: currViewModel.model.startTime,
                                    endTime: currViewModel.model.endTime,
                    ...
                    if (detectorList) {
                      if (this.detectorId !== '') {
                        ...
                        detectorList.forEach((element) => {
                          if (
                            element.analysisTypes != null &&
                            element.analysisTypes.length > 0
                          ) {
                        ...
                        if (
                          ...
                          this.analysisContainsDowntime().subscribe(
                            (containsDowntime) => {
                              if (containsDowntime) {
                                this._router.navigate(
                                  [`./detectors/${currentDetector.id}`],
                                  {
                                    ...
                                    queryParams: {
                                      startTimeChildDetector:
                                        currViewModel.model.startTime,
                                      endTimeChildDetector:
                                        currViewModel.model.endTime
                                    },
                                    queryParamsHandling: 'merge',
                                    replaceUrl: true
                                  }
  ...
  renderInsightsFromSearch(downTime: DownTime) {
    this._resourceService.getPesId().subscribe((pesId) => {
      if (
        !(
          (this.isPublic &&
            detectorSearchEnabledPesIds.findIndex((x) => x == pesId) < 0) ||
          (!this.isPublic &&
            detectorSearchEnabledPesIdsInternal.findIndex((x) => x == pesId) <
              0)
        )
      ) {
        this.searchId = uuid();
        let searchTask = this._diagnosticService
          .getDetectorsSearch(this.searchTerm)
          .pipe(
            map((res) => res),
            catchError((e) => of([]))
          );
        let detectorsTask = this._diagnosticService.getDetectors().pipe(
          map((res) => res),
          catchError((e) => of([]))
        );
        this.showPreLoader = true;
        observableForkJoin([searchTask, detectorsTask]).subscribe(
          (results) => {
            ...
            if (searchResults.length == detectorList.length) {
              searchResults = [];
              logDetail =
                'Search results is same as detector list. This means that the search term is not being sent to Runtimehost API';
            }
            ...
            if (detectorList && searchResults && searchResults.length > 0) {
              searchResults.forEach((result) => {
                if (result.type === DetectorType.Detector) {
                  ...
                  var childList = this.getChildrenOfAnalysis(
                    result.id,
                    detectorList
                  );
  ...
  evaluateAndEmitDowntimeInteractionState(
    analysisContainsDownTime: boolean,
    totalChildDetectorsToLoad: number,
    zoomBehavior: zoomBehaviors,
    incrementTotalDetectorsLoadedCount: boolean = true
  ) {
    if (analysisContainsDownTime) {
      if (
        zoomBehavior & zoomBehaviors.ShowXAxisSelectionDisabledMessage ||
        zoomBehavior & zoomBehaviors.GeryOutGraph
      ) {
        ...
        if (this.downtimeResetTimer) {
          clearTimeout(this.downtimeResetTimer);
        }
        this.downtimeResetTimer = setTimeout(() => {
          //Adding this to reset zoom behavior once the timeout expires.
          this.updateDowntimeZoomBehavior.emit(
            zoomBehaviors.CancelZoom |
              zoomBehaviors.FireXAxisSelectionEvent |
              zoomBehaviors.UnGreyGraph
          );
        ...
        if (incrementTotalDetectorsLoadedCount) {
          this.totalChildDetectorsLoaded < totalChildDetectorsToLoad
            ? this.totalChildDetectorsLoaded++
            : (this.totalChildDetectorsLoaded = totalChildDetectorsToLoad);
        }

        if (
          this.totalChildDetectorsLoaded / totalChildDetectorsToLoad >=
            PERCENT_CHILD_DETECTORS_COMPLETED_TO_ALLOW_DOWNTIME_INTERACTION &&
          this.allowUpdateDowntimeZoomBehaviorEvent === true
        ) {
          this.updateDowntimeZoomBehavior.emit(zoomBehavior);
          this.allowUpdateDowntimeZoomBehaviorEvent = false;
  ...
  containsAnyDisplayableRenderingType(response: DetectorResponse): boolean {
    ...
    let index = response.dataset.findIndex(
      (set) =>
        DetctorNonDisplayableRenderingTypes.find(
          (renderingType) =>
            renderingType === (<Rendering>set.renderingProperties).type
        ) == undefined
    );
    return index > -1;
  }
    ...
    if (this.containsChildDetectors(response)) {
      ...
      response.dataset.forEach((diagnosticData) => {
        if (
          ...
          let currChildDetectorRendering: DetectorListRendering = <
            DetectorListRendering
          >diagnosticData.renderingProperties;
          if (
            !!currChildDetectorRendering.detectorIds &&
            currChildDetectorRendering.detectorIds.length > 0
          ) {
            currChildDetectorRendering.detectorIds.forEach(
              (childDetectorId) => {
                let matchingDetector = this.allDetectors.find(
                  (d) => d.id == childDetectorId
                );
                if (
                  !!matchingDetector &&
                  !!matchingDetector.name &&
                  !!matchingDetector.id &&
                  this.insertInDetectorArray({
                    name: matchingDetector.name,
                    id: matchingDetector.id
  ...
  insertInDetectorViewModel(detectorViewModel: any): boolean {
    if (
      ...
      (!!this.detectorViewModels && this.detectorViewModels.length < 1)
    ) {
      this.detectorViewModels.push(detectorViewModel);
      return true;
    } else {
      if (
        this.detectorViewModels.findIndex(
          (currViewModel) =>
            (<DetectorMetaData>currViewModel.metadata).id ==
              (<DetectorMetaData>detectorViewModel.metadata).id &&
  ...
  startDetectorRendering(
    ...
    this.showWebSearchTimeout = setTimeout(() => {
    ...
    this.detectorMetaData = detectorList.filter(
      (detector) => this.detectors.findIndex((d) => d.id === detector.id) >= 0
    );
    ...
    let viewModelsToProcess = this.detectorMetaData.map((detector) =>
      this.getDetectorViewModel(detector, downTime, containsDownTime)
    );
    if (!isReEntry) {
    ...
    if (this.detectorViewModels.length > 0) {
      ...
      this.evaluateAndEmitDowntimeInteractionState(
        containsDownTime,
        this.detectorViewModels.length,
        zoomBehaviors.CancelZoom |
          zoomBehaviors.ShowXAxisSelectionDisabledMessage |
          zoomBehaviors.GeryOutGraph,
        false
      );
    }
    viewModelsToProcess.forEach((metaData, index) => {
      requests.push(
        (<Observable<DetectorResponse>>metaData.request).pipe(
          map((response: DetectorResponse) => {
            this.evaluateAndEmitDowntimeInteractionState(
              containsDownTime,
              viewModelsToProcess.length,
              zoomBehaviors.CancelZoom |
                zoomBehaviors.FireXAxisSelectionEvent |
                zoomBehaviors.UnGreyGraph
            );
            viewModelsToProcess[index] = this.updateDetectorViewModelSuccess(
              metaData,
              response
            );

            this.processParentChildHierarchy(
              viewModelsToProcess[index].response,
              downTime,
              containsDownTime
            );
            ...
            if (
              ...
              if (
                viewModelsToProcess[index].status === HealthStatus.Critical ||
                viewModelsToProcess[index].status === HealthStatus.Warning
              ) {
                let insight = this.getDetectorInsight(
                  viewModelsToProcess[index]
                );
                let issueDetectedViewModel = {
                  model: viewModelsToProcess[index],
                ...
                if (this.issueDetectedViewModels.length > 0) {
                  this.issueDetectedViewModels =
                    this.issueDetectedViewModels.filter(
                      (iVM) =>
                        !!iVM.model &&
                        !!iVM.model.metadata &&
                        !!iVM.model.metadata.id &&
                        iVM.model.metadata.id !=
                          issueDetectedViewModel.model.metadata.id
                    );
                }

                this.issueDetectedViewModels.push(issueDetectedViewModel);
                this.issueDetectedViewModels =
                  this.issueDetectedViewModels.sort(
                    (n1, n2) => n1.model.status - n2.model.status
                  );
              } else {
                if (this.containsAnyDisplayableRenderingType(response)) {
                  ...
                  if (!insight) {
                    ...
                    insight = {
                      title: DEFAULT_DESCRIPTION_FOR_DETECTORS_WITH_NO_INSIGHT,
                      description: ''
                    };
                    if (
                      !viewModelsToProcess[index].status ||
                      viewModelsToProcess[index].status == HealthStatus.None
                    ) {
                      viewModelsToProcess[index].status = HealthStatus.Info;
                    }
                  }

                  let successViewModel = {
                    model: viewModelsToProcess[index],
                    insightTitle: insight.title,
                    insightDescription: insight.description
                  };
                  ...
                  if (this.successfulViewModels.length > 0) {
                    this.successfulViewModels =
                      this.successfulViewModels.filter(
                        (sVM) =>
                          !!sVM.model &&
                          !!sVM.model.metadata &&
                          !!sVM.model.metadata.id &&
                          sVM.model.metadata.id !=
                            successViewModel.model.metadata.id
                      );
                  }
            ...
            return {
              ...
              ChildDetectorLoadingStatus:
                viewModelsToProcess[index].loadingStatus
            };
          }),
          catchError((err) => {
            this.evaluateAndEmitDowntimeInteractionState(
    ...
    observableForkJoin(requests).subscribe((childDetectorData) => {
      ...
      this.childDetectorsEventProperties['ChildDetectorsList'] =
        JSON.stringify(childDetectorData);
      if (this.searchId && this.searchId.length > 0) {
        this.childDetectorsEventProperties['SearchId'] = this.searchId;
      }
      this.logEvent(
        TelemetryEventNames.ChildDetectorsSummary,
        this.childDetectorsEventProperties
      );
    });
    ...
    return detectorList
      .filter(
        (element) =>
          element.analysisTypes != null &&
          element.analysisTypes.length > 0 &&
          element.analysisTypes.findIndex((x) => x == analysisId) >= 0
      )
      .map((element) => {
  ...
  insertInDetectorArray(detectorItem): boolean {
    if (this.withinGenie) {
      if (
        this.detectors.findIndex((x) => x.id === detectorItem.id) < 0 &&
        detectorItem.score >= this.targetedScore
      ) {
  ...
  getPendingDetectorCount(): number {
    ...
    if (this.detectorViewModels) {
      this.detectorViewModels.forEach((metaData, index) => {
        if (
          this.detectorViewModels[index].loadingStatus == LoadingStatus.Loading
        ) {
          ++pendingCount;
        }
      });
  ...
  getDetectorInsight(viewModel: any): any {
    let allInsights: Insight[] = InsightUtils.parseAllInsightsFromResponse(
      viewModel.response,
      true
    );
    let insight: any;
    if (allInsights.length > 0) {
      let detectorInsight = allInsights.find(
        (i) => i.status === viewModel.status
      );
  ...
  private updateDetectorViewModelSuccess(
    viewModel: any,
  ...
  private getDetectorViewModel(
    ...
    return {
      ...
      expanded: false,
      response: null,
      request: this._diagnosticService.getDetector(
        detector.id,
        startTimeString,
        endTimeString,
        false,
  ...
  public navigateToDetector(): void {
    if (!this.isPublic) {
      if (!!this.drillDownDetectorId && this.drillDownDetectorId.length > 0) {
        this._router.navigate([`./popout/${this.drillDownDetectorId}`], {
          relativeTo: this._activatedRoute,
          queryParamsHandling: 'merge'
        });
      }
  ...
  public goBackToAnalysis(): void {
    ...
    if (this.analysisId === 'searchResultsAnalysis' && this.searchTerm) {
      this._router.navigate([`../../../../${this.analysisId}/search`], {
        relativeTo: this._activatedRoute,
        queryParamsHandling: 'merge',
      ...
      if (this.analysisId === 'supportTopicAnalysis' && this.searchTerm) {
        if (this.detectorId == '') {
          this._router.navigate([`../../${this.analysisId}/dynamic`], {
            relativeTo: this._activatedRoute,
            queryParamsHandling: 'merge',
            queryParams: { searchTerm: this.searchTerm },
            replaceUrl: true
        ...
        if (!!this.analysisId && this.analysisId.length > 0) {
          this._router.navigate([`../${this.analysisId}`], {
            relativeTo: this._activatedRoute,
            queryParamsHandling: 'merge'
          });
        }
  ...
  private updateDrillDownMode(inDrillDownMode: boolean, viewModel: any): void {
    ...
    if (!this.inDrillDownMode) {
      ...
      if (
        !!viewModel &&
        !!viewModel.model &&
        !!viewModel.model.metadata &&
        !!viewModel.model.metadata.name
      ) {
  ...
  public selectDetector(viewModel: any) {
    if (viewModel != null && viewModel.model.metadata.id) {
      ...
      const currentCategoryId =
        this._activatedRoute?.firstChild?.snapshot.params['category'];
      categoryId =
        this._genericCategoryService.getCategoryIdByNameAndCurrentCategory(
          viewModel?.model?.metadata?.category,
          currentCategoryId
        );
      ...
      if (detectorId !== '') {
        ...
        this.logEvent(
          TelemetryEventNames.ChildDetectorClicked,
          clickDetectorEventProperties
        );
        ...
        if (
          ...
          if (this.withinGenie) {
            const isHomepage = !(
              !!this._activatedRoute.root.firstChild &&
              !!this._activatedRoute.root.firstChild.firstChild &&
              !!this._activatedRoute.root.firstChild.firstChild.firstChild &&
              !!this._activatedRoute.root.firstChild.firstChild.firstChild
                .firstChild &&
              !!this._activatedRoute.root.firstChild.firstChild.firstChild
                .firstChild.firstChild &&
              !!this._activatedRoute.root.firstChild.firstChild.firstChild
                .firstChild.firstChild.snapshot &&
              !!this._activatedRoute.root.firstChild.firstChild.firstChild
                .firstChild.firstChild.snapshot.params['category']
            );
            if (
              detectorId == 'appchanges' &&
              !this._detectorControl.internalClient
            ) {
            ...
            if (isHomepage) {
              this.openBladeDiagnoseDetectorId(
                categoryId,
                detectorId,
                DetectorType.Detector
              );
            } else {
              this.logEvent(TelemetryEventNames.SearchResultClicked, {
                searchMode: this.searchMode,
                searchId: this.searchId,
                detectorId: detectorId,
                rank: 0,
                title: clickDetectorEventProperties.ChildDetectorName,
                status: clickDetectorEventProperties.Status,
                ts: Math.floor(new Date().getTime() / 1000).toString()
              });
          ...
          if (this.analysisId === 'supportTopicAnalysis') {
            ...
            if (
              ...
              if (
                viewModel.model.startTime != null &&
                viewModel.model.endTime != null
              ) {
                this._detectorControl.setCustomStartEnd(
                  viewModel.model.startTime,
                  viewModel.model.endTime
                );
    ...
    if (viewModel != null && viewModel.model.metadata.id) {
      ...
      if (detectorId !== '') {
        ...
        Object.keys(queryParams).forEach((x) => {
          paramString =
            paramString === ''
              ? `${paramString}${x}=${queryParams[x]}`
              : `${paramString}&${x}=${queryParams[x]}`;
        });
        const linkAddress = `${
          this._router.url.split('/analysis/')[0]
        }/detectors/${detectorId}?${paramString}`;
  ...
  openSolutionPanel(viewModel: any) {
    if (viewModel != null && viewModel.model != null && viewModel.model.title) {
      ...
      let detectorId: string =
        viewModel.model.metadata != null && viewModel.model.metadata.id != null
          ? viewModel.model.metadata.id
          : '';
      let status: string =
        viewModel.model.status != null
          ? JSON.stringify(viewModel.model.status)
          : '';
