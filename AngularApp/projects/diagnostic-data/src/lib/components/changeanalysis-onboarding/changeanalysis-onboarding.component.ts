...
@Component({
  ...
  styleUrls: [
    './changeanalysis-onboarding.component.scss',
...
export class ChangeAnalysisOnboardingComponent extends DataRenderBaseComponent {
  ...
  private parseData(data: DataTableResponseObject) {
    ...
    if (!this.onboardingText) {
      this.onboardingText = this.isPublic
        ? 'Enable Change Analysis to investigate the changes made to your web application.'
        : 'Change Analysis is not enabled. Contact your customer to enable Change Analysis.';
    }
  }
  ...
  logEnablementClick(): void {
    ...
    this.telemetryService.logEvent(
      TelemetryEventNames.ChangeAnalysisEnableClicked,
      eventProps
    );
  }
}
