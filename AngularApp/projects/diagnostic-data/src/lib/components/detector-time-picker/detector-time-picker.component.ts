...
export class DetectorTimePickerComponent implements OnInit {
  ...
  parseDateFromString: IDatePickerProps['parseDateFromString'] = (s) => {
    ...
    const year =
      datePart[0].length > 0
        ? Number.parseInt(datePart[0], 10)
        : this.endDate.getFullYear();
    const month =
      datePart[1].length > 0
        ? Math.max(1, Math.min(12, parseInt(datePart[1], 10))) - 1
        : this.endDate.getMonth();
    const date =
      datePart[2].length > 1
        ? Math.max(1, Math.min(31, parseInt(datePart[2], 10)))
  ...
  choiceGroupOptions: IChoiceGroupOption[] = [
    {
      key: TimePickerOptions.Last1Hour,
      text: TimePickerOptions.Last1Hour,
      onClick: () => {
        this.setTime(1);
      }
    },
    {
      key: TimePickerOptions.Last6Hours,
      text: TimePickerOptions.Last6Hours,
      onClick: () => {
        this.setTime(6);
      }
    },
    {
      key: TimePickerOptions.Last12Hour,
      text: TimePickerOptions.Last12Hour,
      onClick: () => {
        this.setTime(12);
      }
    },
    {
      key: TimePickerOptions.Last24Hours,
      text: TimePickerOptions.Last24Hours,
      onClick: () => {
        this.setTime(24);
      }
    },
  ...
  ngOnInit() {
    if (this.isPublic === false) {
      this.choiceGroupOptions.splice(4, 0, {
        key: TimePickerOptions.Last3Days,
        text: TimePickerOptions.Last3Days,
        onClick: () => {
          this.setTime(72);
        }
      });
    ...
    this.detectorControlService.timePickerInfoSub.subscribe(
      (timerPickerInfo) => {
        const option = this.choiceGroupOptions.find(
          (option) => timerPickerInfo.selectedKey === option.key
        );
    ...
    if (this.detectorControlService.timeRangeDefaulted) {
      ...
      this.updateTimerErrorMessage.emit(
        this.detectorControlService.timeRangeErrorString
      );
    }
    ...
    this.detectorControlService.update.subscribe((validUpdate) => {
      ...
      const isSameStartAndEndTime =
        this.checkParamIsSameAsMoment(
          queryParams['startTime'],
          this.detectorControlService.startTime
        ) &&
        this.checkParamIsSameAsMoment(
          queryParams['endTime'],
          this.detectorControlService.endTime
        );
      if (
        this.detectorControlService.startTime &&
        this.detectorControlService.endTime
        ...
        queryParams['endTime'] =
          this.detectorControlService.endTime.format('YYYY-MM-DDTHH:mm');
      }
      if (this.detectorControlService.changeFromTimePicker) {
        queryParams =
          UriUtilities.removeChildDetectorStartAndEndTime(queryParams);
      }
  ...
  applyTimeRange() {
    ...
    if (this.showTimePicker) {
      startDateWithTime = this.convertDateTimeToString(
        this.startDate,
        this.startClock
      );
      endDateWithTime = this.convertDateTimeToString(
        this.endDate,
        this.endClock
      );
      //for timer picker, date and hour,minute
      let infoStartDate = new Date(this.startDate);
      infoStartDate.setHours(
        Number.parseInt(this.startClock.split(':')[0]),
        Number.parseInt(this.startClock.split(':')[1])
      );
      let infoEndDate = new Date(this.endDate);
      infoEndDate.setHours(
        Number.parseInt(this.endClock.split(':')[0]),
        Number.parseInt(this.endClock.split(':')[1])
      );
      timePickerInfo = {
      ...
      const localStartTime = new Date(
        currentDate.getTime() - this.hourDiff * 60 * 60 * 1000
      );
      ...
      const infoSelectOption =
        this.hourDiff.toString() === '72'
          ? this.choiceGroupOptions.find((option) => option.key.includes('3'))
          : this.choiceGroupOptions.find((option) =>
              option.key.includes(this.hourDiff.toString())
            );
      timePickerInfo = {
    ...
    if (timePickerInfo.startDate) {
      const startTimeString = moment(timePickerInfo.startDate).format(
        this.detectorControlService.stringFormat
      );
      eventProperties['StartTime'] = startTimeString;
    }
    if (timePickerInfo.endDate) {
      const endTimeString = moment(timePickerInfo.startDate).format(
        this.detectorControlService.stringFormat
      );
      eventProperties['EndTime'] = endTimeString;
    }
    this.telemetryService.logEvent(
      TelemetryEventNames.TimePickerApplied,
      eventProperties
    );
  }
