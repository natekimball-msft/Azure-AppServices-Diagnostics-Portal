...
export class CommAlertComponent implements OnInit {
  private activeAlertTitle: string =
    'An Azure service outage may be impacting this subscription. (Issue : {title})';
  ...
  constructor(
    ...
    private genieGlobals: GenieGlobals
  ) {
    this.commAlertTitle = '';
  ...
  ngOnInit() {
    this._themeService.currentThemeSub.subscribe(() => {
      this.messageBarStyles = {
        root: {
          height: '49px',
          backgroundColor: this._themeService.getPropertyValue(
            '--resolvedIssueMessageBarBGColor'
          )
        }
      };
    });
    ...
    this.commsService
      .getServiceHealthCommunications()
      .subscribe((commsList: Communication[]) => {
        this.azureServiceCommList = commsList;
        const commAlert = commsList.find(
          (comm: Communication) => comm.isAlert === true
        );

        if (commAlert) {
          this.commAlertToShow = commAlert;
          this.isAlertExpanded =
            this.autoExpand && this.commAlertToShow.isExpanded;
          this.commPublishedTime = moment
            .utc(this.commAlertToShow.publishedTime)
          ...
          if (commAlert.status === CommunicationStatus.Active) {
            ...
            this._themeService.currentThemeSub.subscribe(() => {
              this.messageBarStyles.root.backgroundColor =
                this._themeService.getPropertyValue(
                  '--activeIssueMessageBarBGColor'
                );
            });
          } else {
          ...
          if (commAlert.commType === 2) {
            // Emerging Issue
            this.commAlertTitle = commAlert.title;
          } else {
            this.commAlertTitle = this.commAlertTitle.replace(
              '{title}',
              commAlert.title
            );
  ...
  private _getImpactedServices() {
    ...
    const allCommsForImpactingIncident = this.azureServiceCommList.filter(
      (x) => x.incidentId === this.commAlertToShow.incidentId
    );
    allCommsForImpactingIncident.forEach((item) => {
      if (item && item.impactedServices) {
        impactedServices = impactedServices.concat(
          item.impactedServices.map((y) => y.name)
        );
    ...
    const impactedServicesArray = impactedServices.filter(
      (value, index, arr) => arr.indexOf(value) === index
    );

    this.impactedServices =
      impactedServicesArray && impactedServicesArray.toString();
    const uniqueRegions = impactedRegions.filter(
      (value, index, arr) => arr.indexOf(value) === index
    );

    if (uniqueRegions.length > 3) {
      const firstThreeRegions = uniqueRegions.slice(0, 3);
      this.impactedRegions =
        firstThreeRegions &&
        firstThreeRegions
          .toString()
          .concat(`(+${uniqueRegions.length - 3} more)`);
    } else {
