...
export class LoaderDetectorViewComponent implements OnInit {
    ...
    this.timer = setTimeout(() => {
      if (this.i < this.loadingStages.length) {
        this.delay =
          this.i === 0
            ? 0
            : this.random(this.loadingStages[this.i - 1].duration);
  ...
  ngOnDestroy() {
    ...
    this.duration =
      this.endLoadingTimeInMilliSeconds - this.startLoadingTimeInMilliSeconds;
    this.telemetryService.logEvent(
      TelemetryEventNames.LoadingDetectorViewEnded,
      {
        TrackingEventId: this.trackingEventId,
        EndLoadingTime: endLoadingTimeISOString,
