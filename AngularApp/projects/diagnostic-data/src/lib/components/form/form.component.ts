...
export class FormComponent extends DataRenderBaseComponent {
      ...
      for (let i = 0; i < totalForms; i++) {
        ...
        for (let ip = 0; ip < formInputs.length; ip++) {
          if (formInputs[ip]['inputType'] === InputType.Button) {
            this.detectorForms[i].formButtons.push(
              new FormButton(
            ...
            this.detectorForms[i].formInputs.push(
              new RadioButtonList(
                `${this.detectorForms[i].formId}.${formInputs[ip]['inputId']}`,
                formInputs[ip]['inputId'],
                formInputs[ip]['inputType'],
                formInputs[ip]['label'],
                formInputs[ip]['items'],
                formInputs[ip]['toolTip'] != undefined
                  ? formInputs[ip]['toolTip']
                  : '',
                formInputs[ip]['tooltipIcon'] != ''
                  ? formInputs[ip]['tooltipIcon']
                  : 'fa-info-circle',
                formInputs[ip]['isVisible'] != undefined
                  ? formInputs[ip]['isVisible']
                  : true
              )
            );
            ...
            this.detectorForms[i].formInputs.push(
              new Dropdown(
                ...
                formInputs[ip]['tooltipIcon'] != ''
                  ? formInputs[ip]['tooltipIcon']
                  : 'fa-info-circle',
                formInputs[ip]['children'] != undefined
                  ? formInputs[ip]['children']
                  : [],
                formInputs[ip]['isVisible'] != undefined
                  ? formInputs[ip]['isVisible']
                  : true
  ...
  OnSubmitFormAction(formId: any, buttonId: any) {
    let formToExecute = this.detectorForms.find(
    ...
    if (formToExecute != undefined) {
      ...
      this.activatedRoute.queryParams.subscribe((allRouteParams) => {
        queryParams = UriUtilities.addAdditionalQueryParams(
          allRouteParams,
          queryParams
        );
      });
              ...
              if (response.body != undefined) {
                ...
                if (
                  ...
                  this.compilationPackage.assemblyName =
                    response.body.compilationOutput.assemblyName;
                }
  ...
  setInputValues() {
    let detectorQueryParams = JSON.parse(
      this.detectorControlService.detectorQueryParamsString
    );
    if (
      detectorQueryParams != undefined &&
      detectorQueryParams.detectorId == this.detector
    ) {
      let formToSetValues = this.detectorForms.find(
        (form) => form.formId == detectorQueryParams.fId
      );
      detectorQueryParams.inputs.forEach((ip) => {
        let inputElement = formToSetValues.formInputs.find(
          (input) => input.inputId == ip.inpId
        );
  ...
  validateInputs(formInputs: FormInput[]): boolean {
    for (let input of formInputs) {
      if (
        input.isRequired &&
        (input.inputValue == undefined || input.inputValue == '')
      ) {
  ...
  changeVisibility(
    ...
    let inputsToHide = currentDropdown['children'].filter(
      (item) => selectedChildren.indexOf(item) < 0
    );
