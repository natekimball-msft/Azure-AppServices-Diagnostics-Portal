...
export class SolutionComponent extends DataRenderBaseComponent {
  ...
  constructor(
    ...
    private _siteService: SolutionService
  ) {
  ...
  inferActionType(solution: Solution): ActionType {
    ...
    let overrideKeys = Object.keys(solution.OverrideOptions).map((key) =>
      key.toLowerCase()
    );
  ...
  public showCompletionConfirmation(): boolean {
    return (
      this.solution.Action != ActionType.GoToBlade &&
      this.solution.Action != ActionType.OpenTab
    );
  }
  ...
  buildInternalText() {
    ...
    let detectorLink = UriUtilities.BuildDetectorLink(
      this.solution.ResourceUri,
      this.solution.DetectorId
    );
    let detectorLinkMarkdown = `Go to [App Service Diagnostics](${detectorLink})`;

    if (
      markdownBuilder &&
      markdownBuilder.toLowerCase().includes('{detectorlink}')
    ) {
      markdownBuilder = markdownBuilder.replace(
        /{detectorlink}/gi,
        detectorLink
      );
  ...
  chooseAction(): Observable<any> {
    switch (this.solution.Action) {
      case ActionType.ArmApi: {
        return this._siteService.ArmApi(
          this.solution.ResourceUri,
          this.overrideOptions
        );
      }
      case ActionType.GoToBlade: {
        return this._siteService.GoToBlade(
          this.solution.ResourceUri,
          this.overrideOptions
        );
      }
      case ActionType.OpenTab: {
        return this._siteService.OpenTab(
          this.solution.ResourceUri,
          this.overrideOptions
        );
      }
      case ActionType.ToggleStdoutSetting: {
        return this._siteService.ToggleStdoutSetting(
          this.solution.ResourceUri,
          this.overrideOptions
        );
      }
      default: {
        throw new Error(
          `ActionType ${this.solution.Action} does not have a corresponding action`
        );
      }
    }
  }
