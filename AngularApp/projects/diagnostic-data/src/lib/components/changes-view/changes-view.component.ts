...
export class ChangesViewComponent
  ...
  implements OnInit
{
  ...
  ngOnInit() {
    this.tableItems = [];
    let changesDataItem = this.changesDataSet.find(
      (set) =>
        (<Rendering>set.renderingProperties).type === RenderingType.ChangesView
    );
    ...
    if (changesTable.rows.length > 0) {
      changesTable.rows.forEach((row) => {
        let level: ChangeLevel =
          ChangeLevel[this.getChangeProperty(row, 'level', changesTable)];
        let description = this.getChangeProperty(
          row,
          'description',
          changesTable
        );
        ...
        let displayName = this.getChangeProperty(
          row,
          'displayName',
          changesTable
        );
        ...
        this.tableItems.push({
          ...
          displayName:
            ChangeAnalysisUtilities.prepareDisplayValueForTable(displayName),
          description:
            description == null || description == '' ? 'N/A' : description,
          oldValue: oldValue,
          newValue: newValue,
          initiatedBy: ChangeAnalysisUtilities.getInitiatedByField(
            this.initiatedByList
          ),
          jsonPath: jsonPath,
          originalModel:
            ChangeAnalysisUtilities.prepareValuesForDiffView(oldValue),
          modifiedModel:
            ChangeAnalysisUtilities.prepareValuesForDiffView(newValue),
          initiatedByList: row['initiatedByList']
            ? this.getInitiatedByUsers(row)
            : this.initiatedByList
        });
      });
  ...
  private getChangeProperty(
    row: any[],
    propertyName: string,
    changesTable: DataTableResponseObject
  ): string {
    if (row.hasOwnProperty(propertyName)) {
      return row[propertyName];
    } else {
      let propertyIndex = DataTableUtilities.getColumnIndexByName(
        changesTable,
        propertyName,
        true
      );
  ...
  selectChange(changeItem: Change) {
    this.expandedElement =
      this.expandedElement === changeItem ? null : changeItem;

    let eventProps = this.getTelemetryPropertiesFromChange(changeItem);
    (eventProps['isExpand'] = this.expandedElement === changeItem),
      this.logEvent(
        TelemetryEventNames.ChangeAnalysisChangeClicked,
        eventProps
      );
  }

  getFeedbackButtonClass(
    changeItem: Change,
    isHelpfulButton: boolean
  ): string[] {
  ...
  sendFeedback(changeItem: Change, isHelpful: boolean): void {
    ...
    this.logEvent(
      TelemetryEventNames.ChangeAnalysisChangeFeedbackClicked,
      eventProps
    );
  }
  ...
  private getTelemetryPropertiesFromChange(changeItem: Change): any {
    let eventProps = {
      ...
      dataSource: ChangeAnalysisUtilities.getDataSourceFromChangesetId(
        this.changesetId
      ),
  ...
  getInitiatedByUsers(row: any[]): any[] {
    return row['initiatedByList'].length > 0
      ? row['initiatedByList']
      : ['Unable to Determine'];
  }
}
