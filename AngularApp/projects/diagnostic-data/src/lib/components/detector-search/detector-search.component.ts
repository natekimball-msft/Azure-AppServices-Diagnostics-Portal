...
export class DetectorSearchComponent
  ...
  implements OnInit
{
  ...
  ngOnInit() {
    ...
    this._resourceService.getPesId().subscribe((pesId) => {
      var searchConf = new SearchConfiguration(
        this.diagnosticData ? this.diagnosticData.table : null,
        pesId,
        null
      );
      ...
      if (
        ...
        (!this.isPublic &&
          detectorSearchEnabledPesIdsInternal.findIndex((x) => x == pesId) < 0)
      ) {
        ...
        this._activatedRoute.queryParamMap
          ...
          .subscribe((qParams) => {
            ...
            if (!this.searchTerm || this.searchTerm.length == 0) {
              if (
                this.searchConfiguration.CustomQueryString &&
                this.searchConfiguration.CustomQueryString.length > 1
              ) {
  ...
  scrollToResults() {
    ...
    var elementToScroll =
      document.querySelector('.mainContentSection') ||
      document.getElementById('app-content') ||
      document.body;
    var elementToScrollTo = document.querySelector(
      '.search-container-for-scroll'
    ) as HTMLElement;
  ...
  announceAlert() {
    if (this.autoFocus) {
      ...
      setTimeout(() => {
        this.charAlertRef.nativeElement.focus();
        this.charAlertRef.nativeElement.click();
      }, 500);
      setTimeout(() => {
  ...
  refresh() {
    if (!this.detectorId && this.searchTerm && this.searchTerm.length > 1) {
      if (this.searchConfiguration.DetectorSearchEnabled) this.triggerSearch();
    }
  }
  ...
  triggerSearch() {
    ...
    let childrenTask = this.getChildrenOfParentDetector(this.detector).pipe(
      map((res) => res),
      catchError((e) => {
        this.handleRequestFailure();
        return of([]);
    ...
    observableForkJoin([searchTask, detectorsTask, childrenTask]).subscribe(
      (results) => {
        if (this.autoFocus) {
          setTimeout(() => {
            this.searchResultsSection.nativeElement.click();
            this.searchResultsSection.nativeElement.focus();
          }, 2000);
        }
          ...
          results: JSON.stringify(
            searchResults.map(
              (det: DetectorMetaData) =>
                new Object({ id: det.id, score: det.score })
            )
          ),
          ts: Math.floor(new Date().getTime() / 1000).toString(),
          logDetail: logDetail
        ...
        if (detectorList && searchResults && searchResults.length > 0) {
          searchResults.forEach((result) => {
            if (
              result.id === this.detector ||
              childrenOfParent.findIndex(
                (x: string) => x.toLowerCase() == result.id.toLowerCase()
              ) >= 0
            ) {
              return;
              ...
              if (childList && childList.length > 0) {
                childList.forEach((child: DetectorMetaData) => {
                  if (
                    child.id !== this.detector &&
                    childrenOfParent.findIndex(
                      (x: string) => x.toLowerCase() == child.id.toLowerCase()
                    ) < 0 &&
                    child.type === DetectorType.Detector
                  ) {
                    this.insertInDetectorArray({
                      name: child.name,
                      id: child.id,
          ...
          this.detectors = this.detectors
            ...
            .slice(
              0,
              Math.min(
                this.searchConfiguration.DetectorSearchConfiguration.MaxResults,
  ...
  startDetectorRendering(detectorList) {
    ...
    this.detectorViewModels = this.detectorMetaData.map((detector) =>
      this.getDetectorViewModel(detector)
    );
  ...
  getChildrenOfParentDetector(parentDetectorId) {
    ...
    return (<Observable<DetectorResponse>>(
      this._diagnosticService.getDetector(
        parentDetectorId,
        this.detectorControlService.startTimeString,
        this.detectorControlService.endTimeString
      )
    )).pipe(
      flatMap((response: DetectorResponse) => {
        if (response.metadata.type == DetectorType.Analysis) {
              ...
              return detectorList
                .filter(
                  (element) =>
                    element.analysisTypes != null &&
                    element.analysisTypes.length > 0 &&
                    element.analysisTypes.findIndex(
                      (x) => x == parentDetectorId
                    ) >= 0
                )
          ...
          response.dataset.forEach((ds: DiagnosticData) => {
            if (ds.renderingProperties.type === RenderingType.DetectorList)
              detectorList = (<DetectorListRendering>ds.renderingProperties)
                .detectorIds;
          });
  ...
  insertInDetectorArray(detectorItem) {
    if (
      detectorItem.score >
        this.searchConfiguration.DetectorSearchConfiguration
          .MinScoreThreshold &&
