...
export class SolutionOrchestratorComponent
  ...
  implements OnInit
{
  ...
  panelHeaderText = '';
  panelSolutions: any[] = [];
  showPanel(solutions, headerText) {
    this.panelSolutions = solutions;
    this.panelHeaderText = headerText;
    this.isPanelOpen = true;
  }
  showSolutions(viewModel) {
    this.telemetryService.logEvent(
      TelemetryEventNames.SolutionOrchestratorViewSolutionsClicked,
      {
    ...
    this.showPanel(
      viewModel.solutions != null ? viewModel.solutions : [],
      `${viewModel.model.metadata.name} solutions`
    );
  }
  ...
  selectSolution(sol, i) {
    ...
    this.telemetryService.logEvent(
      TelemetryEventNames.SolutionOrchestratorOptionSelected,
      {
        searchId: this.searchId,
        solutionTitle: sol.Title,
        solutionScore: sol.Score,
        solutionDetector: sol.DetectorId,
        solutionIndex: i,
        ts: Math.floor(new Date().getTime() / 1000).toString()
      }
    );
  ...
  constructor(
    ...
    private _contentService: GenericContentService
  ) {
  ...
  selectResult(doc: any) {
    ...
    this.logEvent(TelemetryEventNames.WebQueryResultClicked, {
      searchId: this.searchId,
      article: JSON.stringify(doc),
      ts: Math.floor(new Date().getTime() / 1000).toString()
    });
  }
  ...
  ngOnInit() {
    ...
    this._activatedRoute.paramMap.subscribe((params) => {
      let detectorId =
        params.get('detectorName') === null ? null : params.get('detectorName');
  ...
  updateSearchTerm(searchValue: { newValue: any }) {
    if (
      ...
      !!searchValue.newValue.currentTarget &&
      !!searchValue.newValue.currentTarget.value
    ) {
  ...
  getAzureGuides() {
    if (!this.supportDocumentRendered) {
      ...
      this._supportTopicService.getSelfHelpContentDocument().subscribe(
        (res) => {
  ...
  getBingSearchTask(preferredSites: string[]) {
    return this._contentService
      .searchWeb(
        this.searchTerm,
        this.webSearchConfig.MaxResults.toString(),
        this.webSearchConfig.UseStack,
        preferredSites,
        this.webSearchConfig.ExcludedSites
      )
      .pipe(
        map((res) => res),
        retryWhen((errors) => {
  ...
  getDocuments() {
    if (!this.webSearchConfig) {
      this.webSearchConfig = new WebSearchConfiguration(
        this.pesId,
        this.sapProductId
      );
    }
    ...
    if (
      ...
      this.webSearchConfig.PreferredSites &&
      this.webSearchConfig.PreferredSites.length > 0
    ) {
      searchTaskPrefs = this.getBingSearchTask(
        this.webSearchConfig.PreferredSites
      );
    } else {
    ...
    let postFinish = () => {
      if (searchTaskComplete && searchTaskPrefsComplete) {
        let webresults = this.mergeBingResults([
          searchTaskResult,
          searchTaskPrefsResult
        ]);
  ...
  displayWebResults(results) {
    ...
    if (
      ...
      results.webPages.value.length > 0
    ) {
      ...
      this.logEvent(TelemetryEventNames.WebQueryResults, {
        searchId: this.searchId,
        query: this.searchTerm,
        results: JSON.stringify(
          this.webDocuments.map((result) => {
            return {
  ...
  rankResultsBySource(resultsList) {
    ...
    resultsList.forEach((item) => {
      ...
      if (seenSources.hasOwnProperty(itemSource)) {
        if (seenSources[itemSource] > 2) part2.push(item);
        else {
          part1.push(item);
  ...
  mergeBingResults(results) {
    ...
    if (
      !(
        finalResults &&
        finalResults.webPages &&
        finalResults.webPages.value &&
        finalResults.webPages.value.length > 0
      )
    ) {
      finalResults = {
        webPages: {
    ...
    if (results.length > 1) {
      if (
        results[1] &&
        results[1].webPages &&
        results[1].webPages.value &&
        results[1].webPages.value.length > 0
      ) {
        results[1].webPages.value.forEach((result) => {
          var idx = finalResults.webPages.value.findIndex(
            (x) => x.url == result.url
          );
          if (idx < 0) {
  ...
  onSearchEnter(searchValue: { newValue: string }) {
    ...
    if (searchTerm !== this.searchTerm) {
      this._router.navigate([`../solutionorchestrator`], {
  ...
  viewSolutionSupportingData(solution: Solution) {
    ...
    this.telemetryService.logEvent(
      TelemetryEventNames.SolutionOrchestratorViewSupportingDataClicked,
      {
        ...
        solutionIndex: this.mainSolutionIndex.toString(),
        ts: Math.floor(new Date().getTime() / 1000).toString()
      }
    );
    if (detectorId) {
      this._router.navigate(
        [`../solutionorchestrator/detectors/${detectorId}`],
        {
          relativeTo: this._activatedRoute,
          queryParamsHandling: 'merge',
          preserveFragment: true,
    ...
    if (viewModel != null && viewModel.model.metadata.id) {
      ...
      this.telemetryService.logEvent(
        TelemetryEventNames.SolutionOrchestratorViewSupportingDataClicked,
        {
          ...
          detectorId: detectorId,
          insightTitle: viewModel.insightTitle,
          insightStatus: viewModel.model.status,
          score: viewModel.score,
          ts: Math.floor(new Date().getTime() / 1000).toString()
        }
      );
      this.breadcrumbItems = [
        ...
        {
          text: viewModel.model.metadata.name,
          key: viewModel.model.metadata.id
        }
      ];
    }
    if (detectorId) {
  ...
  startDetectorRendering(downTime: DownTime, containsDownTime: boolean) {
    ...
    this.detectorMetaData = this.detectorList.filter(
      (detector) => this.detectors.findIndex((d) => d.id === detector.id) >= 0
    );
    this.detectorViewModels = this.detectorMetaData.map((detector) =>
      ...
      requests.push(
        (<Observable<DetectorResponse>>metaData.request).pipe(
          map((response: DetectorResponse) => {
            this.detectorViewModels[index] =
              this.updateDetectorViewModelSuccess(metaData, response);

            if (
              this.detectorViewModels[index].loadingStatus !==
              LoadingStatus.Failed
            ) {
              if (
                this.detectorViewModels[index].status ===
                  HealthStatus.Critical ||
                this.detectorViewModels[index].status === HealthStatus.Warning
              ) {
                ...
                let issueDetectedViewModel = {
                  ...
                  score: this.detectorViewModels[index].score
                };
                ...
                if (this.successfulViewModels.length > 0) {
                  this.successfulViewModels = this.successfulViewModels.filter(
                    (sVM) =>
                      !!sVM.model &&
                      !!sVM.model.metadata &&
    ...
    observableForkJoin(requests).subscribe((childDetectorData) => {
      ...
      this.topLevelSolutions = this.allSolutions
        .filter((x) => x.Score > this.detectorThresholdScore)
        .sort((a, b) => {
          if (a.Score > b.Score) return -1;
          else return 1;
        });
      this.logEvent(TelemetryEventNames.SolutionOrchestratorSummary, {
        searchId: this.searchId,
        searchTerm: this.searchTerm,
        proposedSolutions: this.topLevelSolutions.map((x) => {
          return {
            title: x.Title,
  ...
  getDetectorInsight(viewModel: any): any {
    ...
    if (allInsights.length > 0) {
      ...
      insight = {
        ...
        solutions: detectorInsight.solutions
      };
