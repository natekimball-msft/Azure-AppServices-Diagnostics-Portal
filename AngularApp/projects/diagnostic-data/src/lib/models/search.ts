...
export var detectorSearchEnabledPesIds: string[] = [
  '14748',
  '16072',
  '16170',
  '15791',
...
export var detectorSearchEnabledPesIdsInternal: string[] = [
  '14748',
  '16072',
  '16170',
  '16450',
...
var productPreferredSitesConfig: PreferredSitesConfig = {
  //WEB APP WINDOWS
  '14748': [
    'github.com/Azure-App-Service',
    'stackoverflow.com',
    'azure.github.io/AppService'
  ],
  //FUNCTION APPS
  ...
  //LOGIC APPS
  '15791': [
    'docs.microsoft.com/azure/logic-apps',
    'github.com/logicappsio',
    'github.com/Azure/logicapps',
    'techcommunity.microsoft.com/t5/integrations-on-azure',
    'stackoverflow.com'
  ],
  //API Management
  '15551': [
    'github.com/Azure/api-management-samples',
    'github.com/Azure/azure-api-management-devops-resource-kit',
...
export class SearchConfiguration {
  ...
  public constructor(table: any, pesId: string, sapPoductId: string) {
    ...
    if (table && table.columns && table.rows && table.rows.length > 0) {
      this.DetectorSearchEnabled =
        table.rows[0][
          table.columns.findIndex(
            (x) => x.columnName == 'DetectorSearchEnabled'
          )
        ];
      this.WebSearchEnabled =
        table.rows[0][
          table.columns.findIndex((x) => x.columnName == 'WebSearchEnabled')
        ];
      this.CustomQueryString =
        table.rows[0][
          table.columns.findIndex((x) => x.columnName == 'CustomQueryString')
        ];
      var detectorSearchConfig =
        table.rows[0][
          table.columns.findIndex(
            (x) => x.columnName == 'DetectorSearchConfiguration'
          )
        ];
      this.DetectorSearchConfiguration = detectorSearchConfig
        ? JSON.parse(detectorSearchConfig)
        : this.DetectorSearchConfiguration;
      var webSearchConfigData =
        table.rows[0][
          table.columns.findIndex(
            (x) => x.columnName == 'WebSearchConfiguration'
          )
        ];
      var webSearchConfig: WebSearchConfiguration = webSearchConfigData
        ? JSON.parse(webSearchConfigData)
        : this.WebSearchConfiguration;
      this.WebSearchConfiguration.PreferredSites =
        webSearchConfig.PreferredSites &&
        webSearchConfig.PreferredSites.length > 0
          ? webSearchConfig.PreferredSites
          : this.WebSearchConfiguration.PreferredSites;
      this.WebSearchConfiguration.ExcludedSites =
        webSearchConfig.ExcludedSites &&
        webSearchConfig.ExcludedSites.length > 0
          ? Array.from(
              new Set(
                webSearchConfig.ExcludedSites.concat(
                  this.WebSearchConfiguration.ExcludedSites
                )
              )
            )
...
export class WebSearchConfiguration {
  ...
  public constructor(pesId: string, sapProductId: string) {
    ...
    if (sapProductId) {
      productPreferredSites = sapProductPreferredSitesConfig[sapProductId]
        ? sapProductPreferredSitesConfig[sapProductId]
        : [];
      productExcludedSites = sapProductExcludedSitesConfig[sapProductId]
        ? sapProductExcludedSitesConfig[sapProductId]
        : [];
    } else {
      productPreferredSites = productPreferredSitesConfig[pesId]
        ? productPreferredSitesConfig[pesId]
        : [];
      productExcludedSites = productExcludedSitesConfig[pesId]
        ? productExcludedSitesConfig[pesId]
        : [];
    }
    this.PreferredSites = productPreferredSites;
    this.ExcludedSites = Array.from(
      new Set(globalExcludedSites.concat(productExcludedSites))
    );
  }
}
