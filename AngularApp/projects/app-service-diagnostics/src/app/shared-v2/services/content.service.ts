...
export class ContentService {
  ...
  constructor(
    ...
    this._backendApi
      .get<string>(`api/appsettings/ContentSearch:Ocp-Apim-Subscription-Key`)
  ...
  getContent(searchString?: string): Observable<any[]> {
    const searchResults = searchString
      ? this.content.filter((article) => {
          return (
            article.title.indexOf(searchString) != -1 ||
  ...
  searchWeb(
    ...
    resultsCount: string = '3',
    useStack: boolean = true,
    preferredSites: string[] = [],
    excludedSites: string[] = globalExcludedSites
  ): Observable<any> {
    const query = this.constructQueryParameters(
      questionString,
      useStack,
      preferredSites,
      excludedSites
    ...
    return this.ocpApimKeySubject.pipe(
      mergeMap((key: string) => {
  ...
  public constructQueryParameters(
    questionString: string,
    useStack: boolean,
    preferredSites: string[],
    ...
    var stackTypeSuffix = this._resourceService['appStack']
      ? ` ${this._resourceService['appStack']}`
      : '';
    stackTypeSuffix = stackTypeSuffix.toLowerCase();
    if (
      stackTypeSuffix &&
      stackTypeSuffix.length > 0 &&
      stackTypeSuffix == 'static only'
    ) {
    ...
    var preferredSitesSuffix = preferredSites
      .map((site) => `site:${site}`)
      .join(' OR ');
    ...
    var excludedSitesSuffix = excludedSites
      .map((site) => `NOT (site:${site})`)
      .join(' AND ');
    ...
    const query = encodeURIComponent(
      `${questionString}${
        useStack ? stackTypeSuffix : ''
      } AND ${searchSuffix}${preferredSitesSuffix}${excludedSitesSuffix}`
    );
    return query;
  }
  ...
  public IsDeepSearchEnabled(
    ...
    if (isPesIdValid && isSupportTopicIdValid) {
      ...
      var listOfEnabledSupportTopics =
        this._config.documentSearchEnabledSupportTopicIds[pesId];

      var isDeepSearchEnabledForThisSupportTopic =
        listOfEnabledSupportTopics &&
        (listOfEnabledSupportTopics.length == 0 ||
          listOfEnabledSupportTopics.findIndex((x) => x == supportTopicId) >
            -1);
      this.featureEnabledForSupportTopic =
        isDeepSearchEnabledForThisSupportTopic;
    }
    ...
    return (
      this._backendApi
        ...
        .map(
          (status) =>
            status.toLowerCase() == 'true' && this.featureEnabledForSupportTopic
        )
    );
  }
  ...
  public fetchResultsFromDeepSearch(query: Query): Observable<any> {
    if (query.bingSearchEnabled) {
      query.customFilterConditionsForBing = this.constructQueryParameters(
        query.searchTerm,
        query.useStack,
    ...
    return this._http.get<any>(
      this.deepSearchEndpoint + '?' + queryString,
      this.httpOptions
    );
  }
