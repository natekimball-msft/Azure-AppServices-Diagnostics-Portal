...
export class SupportTopicService {
  ...
  private selfHelpContentUrl =
    'https://mpac.support.ext.azure.com/api/v1/selfHelpArticles?articleTypes=Generic&articleTypes=Resource';
  ...
  private vnetSupportTopicIds: {
  ...
  }[] = [
    {
      ...
      supportTopicId: '32542212',
      sapSupportTopicId: 'e46a61a3-caeb-38aa-89ea-f851e9d63652'
    }, // Web App (Linux)\Networking\Configuring VNET integration with App Service
    {
      name: 'VNet Integration',
      supportTopicId: '32542212',
      sapSupportTopicId: 'f9793673-0fe9-0109-52fa-8b166b645087'
    }, // Web App (Windows)\Networking\Configuring VNET integration with App Service
    {
      name: 'VNet Integration',
      supportTopicId: '32542212',
      sapSupportTopicId: '2b508ce1-e160-1a35-6355-71172d0af5b0'
    }, // Web App for Containers\Networking\Configuring VNET integration with App Service
    {
      name: 'VNet Integration',
      supportTopicId: '32630473',
      sapSupportTopicId: '2f550394-351c-4add-5807-eba6428dac71'
    }, // Function App\Networking\Configuring VNET integration with Functions
    {
      name: 'VNet Integration',
      supportTopicId: '32799806',
      sapSupportTopicId: '73f8d0d6-2b1c-0fb9-f08e-ce5675ae4e9a'
    }, // Logic App\Networking\Configuring VNET integration
    {
      name: 'Outbound Connectivity',
      supportTopicId: '32820919',
      sapSupportTopicId: '6baa322b-ef6b-f235-c0c0-82ff512602c4'
    }, // Web App (Linux)\Networking\Outbound Connectivity
    {
      name: 'Outbound Connectivity',
      supportTopicId: '32820919',
      sapSupportTopicId: '1653e0e2-8249-fb67-6d0e-767fa14d79b2'
    }, // Web App (Windows)\Networking\Outbound Connectivity
    {
      name: 'Outbound Connectivity',
      supportTopicId: '32820919',
      sapSupportTopicId: '1653e0e2-8249-fb67-6d0e-767fa14d79b2'
    }, // Web App for Containers\Networking\Outbound Connectivity
    {
      name: 'Outbound Connectivity',
      supportTopicId: '32820562',
      sapSupportTopicId: '8856038e-a071-236e-9823-41313ee4cb85'
    } // Function App\Networking\Outbound Connectivity
  ];
  ...
  constructor(
    ...
    protected _diagnosticService: DiagnosticService,
  ...
  private getSelfHelpContentApollo(): Observable<any> {
    if (
      this.pesId &&
      this.pesId.length > 0 &&
      this.sapSupportTopicId &&
      this.sapSupportTopicId.length > 0
    ) {
      // To generate a unique apollo search Id, we use resource name, current timestamp and a guid.
      let apolloResourceId = `${
        this._resourceService.resource.name
      }-${Math.floor(Date.now() / 1000)}-${uuid()}`;
      let requestBody = {
        properties: {
          triggerCriteria: [
            {
      ...
      return this._armService
        .putResource(
          resourceUri,
          requestBody,
          this.apolloApiConfig.apiVersion,
          true,
          apolloHeaders
        )
        .pipe(
          map((response: ResponseMessageEnvelope<any>) => {
  ...
  private getSelfHelpContentLegacy(): Observable<any> {
    if (
      ...
      this.pesId.length > 0 &&
      this.supportTopicId &&
      this.supportTopicId.length > 0
    ) {
      return this._authService.getStartupInfo().pipe(
        flatMap((res) => {
          var selfHelpContentForSupportTopicUrl =
            this.selfHelpContentUrl +
            '&productId=' +
            encodeURIComponent(this.pesId) +
            '&topicId=' +
            encodeURIComponent(this.supportTopicId);
          const headers = new HttpHeaders({
            Authorization: `Bearer ${res.token}`
          });

          return this._http
            .get(selfHelpContentForSupportTopicUrl, {
              headers: headers
            })
            .pipe(
              map((response) => {
                return this.cleanSelfHelpContentLegacy(response);
              })
            );
        })
      );
  ...
  public getSelfHelpContentDocument(): Observable<any> {
    ...
    return this.getSelfHelpContentApollo().pipe(
      map((res) => res),
      catchError((err) => {
        ...
        map((detectors) => {
          return detectors.filter(
            (detector) =>
              ...
              detector.supportTopicList.findIndex(
                (supportTopicId) =>
                  supportTopicId.sapSupportTopicId === this.sapSupportTopicId ||
                  supportTopicId.id === this.supportTopicId
              ) >= 0
          );
        })
      );
  ...
  addStartAndEndTimeIfNotPresent(queryParams: any): any {
    ...
    !queryParams.startTime
      ? (queryParams['startTime'] = defaultStartTimeString)
      : noop;
    !queryParams.endTime
      ? (queryParams['endTime'] = defaultEndTimeString)
      : noop;

    return queryParams;
  }

  getPathForSupportTopic(
    supportTopicId: string,
    pesId: string,
    searchTerm: string,
    sapSupportTopicId: string = '',
    ...
    return this._resourceService.getPesId().pipe(
      flatMap((pesId) => {
        ...
        for (var i = 0; i < this.vnetSupportTopicIds.length; ++i) {
          if (
            supportTopicId == this.vnetSupportTopicIds[i].supportTopicId ||
            sapSupportTopicId == this.vnetSupportTopicIds[i].sapSupportTopicId
          ) {
        ...
        if (networkSupportTopic != null) {
          if (
            networkSupportTopic == 'Outbound Connectivity' &&
            ...
            return observableOf({
              ...
              queryParams: {
                redirectFrom,
                supportTopic: networkSupportTopic,
                supportTopicId,
                sapSupportTopicId
              }
            });
          ...
          flatMap((detectors) => {
            ...
            if (detectors) {
              ...
              if (sapSupportTopicId != '') {
                matchingDetectors = detectors.filter(
                  (detector) =>
                    detector.supportTopicList &&
                    detector.supportTopicList.findIndex(
                      (supportTopicId) =>
                        supportTopicId.sapSupportTopicId === sapSupportTopicId
                    ) >= 0
                );
              ...
              if (matchingDetectors.length < 1) {
                matchingDetectors = detectors.filter(
                  (detector) =>
                    detector.supportTopicList &&
                    detector.supportTopicList.findIndex(
                      (supportTopic) => supportTopic.id === supportTopicId
                    ) >= 0
                );
              ...
              if (matchingDetectors && matchingDetectors.length > 0) {
                if (
                  matchingDetectors.length === 1 &&
                  matchingDetectors[0] &&
                  matchingDetectors[0].id
                ) {
                  ...
                  if (
                    this._resourceService.subscriptionId ==
                      'c258f9c0-3d64-4761-8697-cab631f28422' &&
                    this.solutionOrchestratorConfig &&
                    this.solutionOrchestratorConfig[this.pesId] &&
                    this.solutionOrchestratorConfig[this.pesId].length > 0 &&
                    this.solutionOrchestratorConfig[this.pesId].findIndex(
                      (s) => s == this.supportTopicId
                    ) >= 0
                  ) {
                    detectorPath = `solutionorchestrator`;
                    return observableOf({
                      path: detectorPath,
                      queryParams: queryParamsDic
                    });
                  } else {
            ...
            let keywordsList = [];
            return this._resourceService.getKeystoneDetectorId().pipe(
              flatMap((keystoneDetectorId) => {
                detectorPath = `/integratedSolutions` + detectorPath;
                if (keystoneDetectorId) {
                  let startTime, endTime: momentNs.Moment;
                  endTime = moment.utc().subtract(16, 'minutes');
                  startTime = endTime.clone().subtract(1, 'days');
                  let fakeStartTimeString =
                    ...
                    .pipe(
                      map((keystoneRes) => {
                        if (keystoneRes != undefined) {
                          let keystoneData = keystoneRes.dataset.find(
                            (data) =>
                              data.renderingProperties.type ===
                              RenderingType.KeystoneComponent
                          );
                          let keystoneInsight: KeystoneInsight = JSON.parse(
                            keystoneData.table.rows[0][0]
                          );
                          ...
                          if (
                            keywordsList &&
                            keywordsList.findIndex(
                              (keyword) =>
                                searchTerm.toLowerCase().indexOf(keyword) !== -1
                            ) !== -1
                          ) {
                            queryParamsDic['keystoneDetectorId'] =
                              keystoneDetectorId;
                            keystoneSolutionApplied = true;
                          }

                          this._telemetryService.logEvent(
                            'KeywordsListForKeyStone',
                            {
                              Keywords: keystoneInsight['Title'],
                              KeystoneSolutionApplied: String(
                                keystoneSolutionApplied
                              )
                            }
                          );
                        }
                        ...
                        if (
                          detectorPath.indexOf(
                            '/analysis/supportTopicAnalysis/dynamic'
                          ) > -1
                        ) {
                          queryParamsDic =
                            this.addStartAndEndTimeIfNotPresent(queryParamsDic);
                        }
