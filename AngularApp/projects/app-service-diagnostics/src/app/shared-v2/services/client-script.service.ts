...
import { DataRenderBaseComponent, DiagnosticData, TelemetryService } from 'diagnostic-data';
...
export class ClientScriptService {
    ...
    constructor(private _injector: Injector, private _telemetryService: TelemetryService) {
    ...
    public process(viewContainerRef: ViewContainerRef, data: DiagnosticData) {
        ...
        try {
            ...
            const componentRef = viewContainerRef.createComponent(component.uiComponent);

            if (component.func != null) {
                component.func(this._injector)
                    .then(result => {
                        this.componentManager.copyComponentContent(component.uiComponent, componentRef.instance, result);
                    })
                    .catch(error => {
                        this._telemetryService.logException(error, `ClientScriptComponent.${componentName}`);
                    });
            }
