...
export var cleanApolloSolutions = (docContent) => {
  ...
  var targetParts = sections.filter(
    (x: string) => x.length > 1 && !isReplacementString(x)
  );

  //Also remove the FIRST SECTION if it has h2 tag or if it doesn't contain any links
  targetParts =
    targetParts[0].includes('<h2>') || !targetParts[0].includes('a href')
      ? targetParts.slice(1, targetParts.length)
      : targetParts;
