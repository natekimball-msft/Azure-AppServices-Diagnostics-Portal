...
export class ThemeService {
  ...
  getPropertyValue(key: string): string {
    return this.active &&
      Object.keys(this.active.properties).findIndex(
        (property) => property === key
      ) >= 0
      ? this.active.properties[key]
      : '';
  }
  ...
  constructor(private _authService: AuthService) {
    this.setActiveTheme(
      ...
      this.currentHighContrastKeyValue
    );
    this._authService.getStartupInfo().subscribe((startupInfo) => {
      if (startupInfo) {
        const theme =
          startupInfo.theme != undefined ? startupInfo.theme.toLowerCase() : '';
        const highContrastKey =
          startupInfo.highContrastKey != undefined
            ? startupInfo.highContrastKey.toString()
            : '';

        if (
          theme !== this.currentThemeValue ||
          highContrastKey !== this.currentHighContrastKeyValue
        ) {
          this.currentThemeValue =
            theme !== this.currentThemeValue ? theme : this.currentThemeValue;
          this.currentHighContrastKeyValue =
            highContrastKey !== this.currentHighContrastKeyValue
              ? highContrastKey
              : this.currentHighContrastKeyValue;
