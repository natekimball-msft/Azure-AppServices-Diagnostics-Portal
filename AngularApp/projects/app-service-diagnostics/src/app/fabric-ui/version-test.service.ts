...
export const allowV3PResourceTypeList: {
  type: string;
...
export class VersionTestService {
  public isLegacySub: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    true
  );
  public isVnextSub: boolean = false;
  public isVnextOnlyResource: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(true);
  public initializedPortalVersion: BehaviorSubject<string> =
    new BehaviorSubject<string>('v2');
      ...
      this._siteService.currentSite.subscribe((site) => {
        this.overrideUseLegacy.subscribe((overrideValue) => {
          if (resourceType === ResourceType.Site) {
            const isVnextOnlyResourceType = this.isVnextOnlyResourceType(
              site,
              resourceType
            );
            const isVnextResourceType = this.isVnextResourceType(
              site,
              resourceType
            );
            // Initialize with the new version if the subscription falls in vnext sub groups and the resource is web app
            this.initializedPortalVersion.next(
              this.isVnextSub && isVnextResourceType ? 'v3' : 'v2'
            );
  ...
  private isVnextOnlyResourceType(
    site: Site,
    resourceType: ResourceType
  ): boolean {
    let isLinuxPlatform =
      site && site.kind && site.kind.toLowerCase().indexOf('linux') >= 0;
    return (
      resourceType === ResourceType.Site &&
      site &&
      (site.appType === AppType.WebApp ||
        site.appType === AppType.FunctionApp) &&
      !isLinuxPlatform
    );
  }
  ...
  private isVnextResourceType(site: Site, resourceType: ResourceType): boolean {
    ...
    return (
      resourceType === ResourceType.Site &&
      site &&
      (site.appType === AppType.WebApp || site.appType == AppType.FunctionApp)
    );
  }

  private isVnextSubscription(
    subscriptionId: string,
    percentageToRelease: number = 0.1
  ): boolean {
    percentageToRelease =
      percentageToRelease < 0 || percentageToRelease > 1
        ? 0.1
        : percentageToRelease;
