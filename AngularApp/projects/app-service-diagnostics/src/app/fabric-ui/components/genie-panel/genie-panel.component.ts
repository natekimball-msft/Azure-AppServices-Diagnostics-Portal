...
export class GeniePanelComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private _genieChatFlow: GenieChatFlow,
    private _messageProcessor: GenieMessageProcessor,
  ...
  updateView(event?: any): void {
    ...
    // "view-loaded" event will be emitted by feedback component after genie result
    if (
      event &&
      event.hasOwnProperty('data') &&
      event['data'] === 'view-loaded'
    ) {
      (<HTMLTextAreaElement>document.getElementById('genieChatBox')).disabled =
        false;
  ...
  updateStatus(event?: any): void {
    ...
    if (
      event &&
      event.hasOwnProperty('hasResult') &&
  ...
  scrollToBottom(event?: any): void {
    try {
      this.myScrollContainer.nativeElement.childNodes[0].scrollTop =
        this.myScrollContainer.nativeElement.childNodes[0].scrollHeight;
    } catch (err) {}
  }
  ...
  onSearchEnter(event: any): void {
    ...
    let inputValue = (<HTMLTextAreaElement>(
      document.getElementById('genieChatBox')
    )).value;
    ...
    this._genieChatFlow
      .createMessageFlowForAnaysis(
        inputValue,
        analysisMessageGroupId,
        this.resourceId
      )
      .subscribe((analysisMessages: Message[]) => {
