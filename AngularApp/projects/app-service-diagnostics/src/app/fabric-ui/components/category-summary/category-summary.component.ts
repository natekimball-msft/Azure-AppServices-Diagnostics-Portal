...
export class CategorySummaryComponent implements OnInit {
  ...
  public _checkIsWindowsOrLinuxApp(): boolean {
    ...
    return (
      ...
      (webSiteService.platform === OperatingSystem.windows ||
        webSiteService.platform === OperatingSystem.linux)
    );
  }
  ...
  ngOnInit() {
    if (this._resourceService.armResourceConfig) {
      this.categoryService.initCategoriesForArmResource(
    ...
    this.categoryService.categories.subscribe((categories) => {
      ...
      this._activatedRoute.params.subscribe((parmas) => {
        ...
        this.category = categories.find(
          (category) =>
            category.id.toLowerCase() ===
              this._activatedRoute.snapshot.params.category.toLowerCase() ||
        ...
        this.resourceName =
          this._activatedRoute.snapshot.parent.params.resourcename;
        this._portalActionService.updateDiagnoseCategoryBladeTitle(
          `${this.resourceName} | ` + this.categoryName
        );
      });
    });
  }
  ...
  ngAfterViewInit() {
    this._telemetryService.logPageView(TelemetryEventNames.CategoryPageLoaded, {
