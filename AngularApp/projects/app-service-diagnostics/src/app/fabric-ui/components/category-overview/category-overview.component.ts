...
export class CategoryOverviewComponent implements OnInit {
  ...
  ngOnInit() {
    let categoryParam =
      this._activatedRoute.parent.snapshot.params.category.toLowerCase();
    ...
    this._categoryService.categories.subscribe((categories) => {
      this.category = categories.find(
        (category) =>
          categoryParam === category.id.toLowerCase() ||
          category.name.replace(/\s/g, '').toLowerCase() === categoryParam
      );
      if (!!this.category) {
  ...
  ngAfterViewInit() {
    this._telemetryService.logPageView(
      TelemetryEventNames.CategoryOverviewPageLoaded,
      { Category: this.categoryId, CategoryName: this.category.name }
    );
  }
}
