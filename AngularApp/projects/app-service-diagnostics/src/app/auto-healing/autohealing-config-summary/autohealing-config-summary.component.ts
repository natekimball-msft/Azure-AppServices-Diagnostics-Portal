...
export class AutohealingConfigSummaryComponent implements OnInit, OnChanges {
  ...
  updateComponent() {
    ...
    if (
      this.actualhealSettings.autoHealRules != null &&
      ...
      if (this.actualhealSettings.autoHealRules.triggers.privateBytesInKB > 0) {
        conditions.push(
          'Process consumes more than ' +
            FormatHelper.formatBytes(
              this.actualhealSettings.autoHealRules.triggers.privateBytesInKB *
                1024,
              2
            ) +
            ' private bytes of memory'
        );
      }
      ...
      if (this.actualhealSettings.autoHealRules.triggers.requests != null) {
        conditions.push(
          'App has served  ' +
            this.actualhealSettings.autoHealRules.triggers.requests.count +
            ' requests in a duration of  ' +
            FormatHelper.timespanToSeconds(
              this.actualhealSettings.autoHealRules.triggers.requests
                .timeInterval
            ) +
            ' seconds'
        );
      ...
      if (this.actualhealSettings.autoHealRules.triggers.slowRequests != null) {
        conditions.push(
          this.actualhealSettings.autoHealRules.triggers.slowRequests.count +
            ' requests take more than  ' +
            FormatHelper.timespanToSeconds(
              this.actualhealSettings.autoHealRules.triggers.slowRequests
                .timeTaken
            ) +
            ' seconds in a duration of  ' +
            FormatHelper.timespanToSeconds(
              this.actualhealSettings.autoHealRules.triggers.slowRequests
                .timeInterval
            ) +
      ...
      if (
        this.actualhealSettings.autoHealRules.triggers.slowRequestsWithPath !=
        null
      ) {
        for (
          let index = 0;
          ...
          conditions.push(
            slowRequestRule.count +
              ' requests ' +
              requestPath +
              'take more than  ' +
              FormatHelper.timespanToSeconds(slowRequestRule.timeTaken) +
              ' seconds in a duration of  ' +
              FormatHelper.timespanToSeconds(slowRequestRule.timeInterval) +
              ' seconds'
          );
        }
      }
      ...
      if (this.actualhealSettings.autoHealRules.triggers.statusCodes != null) {
        for (
          let index = 0;
          index <
          this.actualhealSettings.autoHealRules.triggers.statusCodes.length;
          index++
        ) {
          const statusCodeRule =
            this.actualhealSettings.autoHealRules.triggers.statusCodes[index];
          ...
          if (statusCodeRule.win32Status > 0) {
            statusCodesString +=
              ' and win-32 status ' + statusCodeRule.win32Status.toString();
          }
          ...
          conditions.push(
            statusCodeRule.count +
              ' requests ' +
              requestPath +
              'end up with HTTP Status ' +
              statusCodesString +
              ' in a duration of  ' +
              FormatHelper.timespanToSeconds(statusCodeRule.timeInterval) +
              ' seconds'
          );
        }
      }
      ...
      if (
        this.actualhealSettings.autoHealRules.triggers.statusCodesRange != null
      ) {
        for (
          let index = 0;
          index <
          this.actualhealSettings.autoHealRules.triggers.statusCodesRange
            .length;
          index++
        ) {
          const statusCodeRule =
          ...
          conditions.push(
            statusCodeRule.count +
              ' requests ' +
              requestPath +
              'end up with HTTP Status in range (' +
              statusCodeRule.statusCodes +
              ') in a duration of  ' +
              FormatHelper.timespanToSeconds(statusCodeRule.timeInterval) +
              ' seconds'
          );
      ...
      if (
        ...
        if (
          this.actualhealSettings.autoHealRules.actions.actionType ===
          ...
          if (
            this.actualhealSettings.autoHealRules.actions.customAction !=
              null &&
            this.actualhealSettings.autoHealRules.actions.customAction.exe !=
              null
          ) {
            actionExe =
              this.actualhealSettings.autoHealRules.actions.customAction.exe;
        ...
        if (
          this.actualhealSettings.autoHealRules.actions != null &&
          this.actualhealSettings.autoHealRules.actions
            .minProcessExecutionTime != null
        ) {
          const seconds = FormatHelper.timespanToSeconds(
            this.actualhealSettings.autoHealRules.actions
      ...
      this.settingsSummary = {
        ...
        ProcessStartupLabel: this.processStartupLabel
      };
    }
  }
  ...
  openEventViewer() {
    this._router.navigateByUrl(
      `/resource${this.resourceId}/categories/DiagnosticTools/tools/eventviewer`
    );
  }
}
