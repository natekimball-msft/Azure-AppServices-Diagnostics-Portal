...
export class AutohealingComponent implements OnInit {
  ...
  constructor(
    ...
    private _autohealingService: AutohealingService,
        ...
        if (siteInfo) {
          ...
          if (!this.isWindowsApp) {
            this._daasService
              ...
              .subscribe((isDiagServerEnabled) => {
                this.isLinuxDotNetApp =
                  this._webSiteService.platform === OperatingSystem.linux &&
                  this._webSiteService.linuxFxVersion.startsWith(
                    'DOTNETCORE'
                  ) &&
                  isDiagServerEnabled;
              });
          ...
          this._autohealingService
            .getAutohealSettings(this.siteToBeDiagnosed)
            .subscribe(
              (autoHealSettings) => {
              ...
              (err) => {
                this.retrievingAutohealSettings = false;
                this.errorMessage = `Failed with an error ${JSON.stringify(
                  err
                )} while retrieving autoheal settings`;
              }
            );
        }
      });
  ...
  updateConditionsAndActions() {
    if (
      this.autohealingSettings != null &&
      this.autohealingSettings.autoHealRules != null &&
      this.autohealingSettings.autoHealRules.actions != null
    ) {
      if (
        this.autohealingSettings.autoHealRules.actions
          .minProcessExecutionTime != null
      ) {
        this.minProcessExecutionTime = FormatHelper.timespanToSeconds(
          this.autohealingSettings.autoHealRules.actions.minProcessExecutionTime
        );
      } else {
      ...
      this.actionSelected =
        this.autohealingSettings.autoHealRules.actions.actionType;
      if (
        this.autohealingSettings.autoHealRules.actions.actionType ===
        AutoHealActionType.CustomAction
      ) {
        this.customAction =
          this.autohealingSettings.autoHealRules.actions.customAction;
      }
    } else {
  ...
  saveMinProcessTimeChanged(val: number) {
    ...
    if (this.autohealingSettings.autoHealRules.actions != null) {
      if (
        ...
        this.autohealingSettings.autoHealRules.actions.minProcessExecutionTime =
          FormatHelper.secondsToTimespan(this.minProcessExecutionTime);
      } else {
        this.autohealingSettings.autoHealRules.actions.minProcessExecutionTime =
          FormatHelper.secondsToTimespan(0);
      }
  ...
  checkForChanges() {
    ...
    if (this.autohealingSettings.autoHealEnabled) {
      if (this.checkRulesValid()) {
        ...
        if (
          this.originalAutoHealSettings != null &&
          this.originalAutoHealSettings.autoHealRules != null
        ) {
          originalAutoHealSettingsString = this.orderedJsonStringify(
            this.originalAutoHealSettings.autoHealRules
          );
        }
        let currentSettingsString = this.orderedJsonStringify(
          this.autohealingSettings.autoHealRules
        );
      ...
      if (
        this.originalAutoHealSettings.autoHealEnabled !==
        this.autohealingSettings.autoHealEnabled
      ) {
        this.saveEnabled = true;
      }
    }
  ...
  checkTriggersValid(triggers: AutoHealTriggers): boolean {
    let isValid = false;
    if (
      (triggers.privateBytesInKB != null && triggers.privateBytesInKB > 0) ||
      ...
      (triggers.statusCodesRange != null &&
        triggers.statusCodesRange.length > 0) ||
      (triggers.slowRequestsWithPath != null &&
        triggers.slowRequestsWithPath.length > 0)
    ) {
      isValid = true;
    }
    return isValid;
  ...
  checkActionsValid(actions: AutoHealActions): boolean {
    let isValid = false;
    if (
      actions.actionType === AutoHealActionType.LogEvent ||
      actions.actionType === AutoHealActionType.Recycle
    ) {
      isValid = true;
    } else {
      if (
        actions.customAction != null &&
  ...
  checkRulesValid(): boolean {
    ...
    if (
      ...
      if (
        this.checkTriggersValid(
          this.autohealingSettings.autoHealRules.triggers
        ) &&
        this.checkActionsValid(this.autohealingSettings.autoHealRules.actions)
      ) {
        rulesValid = true;
      }
    }
  ...
  saveChanges() {
    ...
    this._autohealingService
      .updateAutohealSettings(this.siteToBeDiagnosed, this.autohealingSettings)
      .subscribe(
        (savedAutoHealSettings) => {
          this.saveEnabled = false;
        ...
        (err) => {
          this._logger.LogError(
            `Failed with an error ${JSON.stringify(
              err
            )} while saving autoheal settings`,
            'AutoHealing'
          );
          this.savingAutohealSettings = false;
          this.errorMessageSaving = `Failed with an error ${JSON.stringify(
            err
          )} while saving autoheal settings`;
        }
      );
  ...
  updateTriggerStatus(triggerRule: number) {
    ...
    this.triggerSelected != triggerRule
      ? (this.triggerSelected = triggerRule)
  ...
  triggerRuleUpdated(ruleEvent, triggerRule: number) {
    switch (triggerRule) {
      case 0: {
        this.autohealingSettings.autoHealRules.triggers.slowRequests =
          ruleEvent.slowRequests;
        this.autohealingSettings.autoHealRules.triggers.slowRequestsWithPath =
          ruleEvent.slowRequestsWithPath;
        this.slowRequestRules = this.getSlowRequestRules();
        break;
      }
      case 1: {
        this.autohealingSettings.autoHealRules.triggers.privateBytesInKB =
          ruleEvent;
        break;
      }
      case 2: {
      ...
      case 3: {
        this.autohealingSettings.autoHealRules.triggers.statusCodes =
          ruleEvent.statusCodes;
        this.autohealingSettings.autoHealRules.triggers.statusCodesRange =
          ruleEvent.statusCodesRange;
        this.statusCodeRules = this.getStatusCodeRules();
        break;
      }
    }

    this.triggers[triggerRule].IsConfigured =
      this.triggers[triggerRule].checkRuleConfigured();
    this.checkForChanges();
  }
  ...
  initTriggersAndActions() {
    ...
    this.triggers.push({
      ...
      checkRuleConfigured: () =>
        self.autohealingSettings != null &&
        self.autohealingSettings.autoHealRules != null &&
        self.autohealingSettings.autoHealRules.triggers != null &&
        (self.autohealingSettings.autoHealRules.triggers.slowRequests != null ||
          (self.autohealingSettings.autoHealRules.triggers
            .slowRequestsWithPath != null &&
            self.autohealingSettings.autoHealRules.triggers.slowRequestsWithPath
              .length > 0)),
      IsConfigured: false
    });
    this.triggers.push({
      Name: 'Memory Limit',
      IsEnabled: this.isWindowsApp,
      Icon: 'fa fa-microchip',
      checkRuleConfigured: () =>
        self.autohealingSettings != null &&
        self.autohealingSettings.autoHealRules != null &&
        self.autohealingSettings.autoHealRules.triggers != null &&
        self.autohealingSettings.autoHealRules.triggers.privateBytesInKB > 0,
      IsConfigured: false
    });
    this.triggers.push({
      Name: 'Request Count',
      ...
      checkRuleConfigured: () =>
        ...
        self.autohealingSettings.autoHealRules != null &&
        self.autohealingSettings.autoHealRules.triggers != null &&
        self.autohealingSettings.autoHealRules.triggers.requests != null,
      IsConfigured: false
    });
    this.triggers.push({
      Name: 'Status Codes',
      ...
      checkRuleConfigured: () =>
        ...
        self.autohealingSettings.autoHealRules != null &&
        self.autohealingSettings.autoHealRules.triggers != null &&
        ((self.autohealingSettings.autoHealRules.triggers.statusCodes &&
          this.autohealingSettings.autoHealRules.triggers.statusCodes.length >
            0) ||
          (self.autohealingSettings.autoHealRules.triggers.statusCodesRange &&
            this.autohealingSettings.autoHealRules.triggers.statusCodesRange
              .length > 0)),
      IsConfigured: false
    });
    ...
    this.actions.push({
      Name: 'Log an Event',
      Icon: 'fa fa-book',
      Enabled: this.isWindowsApp
    });
    this.actions.push({
      Name: 'Custom Action',
      Icon: 'fa fa-bolt',
      Enabled: this.isWindowsApp || this.isLinuxDotNetApp
    });
  }
  ...
  reset() {
    ...
    this.autohealingSettings = JSON.parse(
      JSON.stringify(this.originalAutoHealSettings)
    );
    this.updateConditionsAndActions();
    this.checkForChanges();
    this.collapseAllTiles();
  ...
  generateSasRecommendation() {
    return (
      this.originalAutoHealSettings &&
      this.originalAutoHealSettings.autoHealEnabled &&
      this.originalAutoHealSettings.autoHealRules &&
      this.originalAutoHealSettings.autoHealRules.actions &&
      this.originalAutoHealSettings.autoHealRules.actions &&
      this.originalAutoHealSettings.autoHealRules.actions.customAction &&
      this.originalAutoHealSettings.autoHealRules.actions.customAction.exe &&
      this.originalAutoHealSettings.autoHealRules.actions.customAction.exe
        .toLowerCase()
        .indexOf('daasconsole.exe') > -1 &&
      this.originalAutoHealSettings.autoHealRules.actions.customAction
        .parameters &&
      this.originalAutoHealSettings.autoHealRules.actions.customAction.parameters
        .toLowerCase()
        .indexOf('blobsasuri') > -1
    );
  }
  ...
  validateAutoHealRules() {
    ...
    const minProcessExecutionTimeNotSet: string =
      "To avoid mitigation actions to kick in immediately after app starts, it is advisable to set the startup time to at least 600 seconds (10 minutes). This will ensure that mitigation actions don't kick in during app's cold start.";
    const actionSetToRecycle: string =
      "Please ensure that the rules configured don't end up recycling your app too many times to avoid any performance hits or app downtimes during the cold start of the application.";
    const diagnosticToolChosenCustom: string =
      'You have chosen a custom action to execute whenever mitigation kicks in.';
    const diagnosticToolChosen: string =
      ' If this is a production app, please ensure that you try this out on a deployment slot first to protect against any downtimes to your application.';
    const diagnosticJavaToolChosen: string =
      'The Java diagnostic tools use either jMap or jStack process to collect dumps. Both of these tools freeze the process while collecting data. As a result, the app cannot serve any requests during this time and performance will be impacted. It may take longer to collect these dumps if the process is consuming high memory or has a high number of active threads.';
    const diagnosticMemoryDumpChosen: string =
      'You have chosen to collect a memory dump of the process. Please ensure that the storage account has enough space to copy memory dumps.';
    const diagnosticProfilerWithThreadStacksChosen: string =
      'When the profiler is chosen along with thread stacks option, the process is frozen for a few seconds to dump all raw thread stacks. This option is advisable if you are experiencing long delays (in minutes) to serve the requests or if the application is experiencing deadlocks. During this time, the process cannot serve any requests and application performance will be impacted.';
    const diagnosticProfilerChosen: string =
      'The profiling tool is light weight but incurs some CPU overhead during the data collection process.';
    ...
    if (
      ...
      if (
        ...
        if (
          this.autohealingSettings.autoHealRules.actions.customAction.exe.toLowerCase() ===
          'd:\\home\\data\\daas\\bin\\daasconsole.exe'
        ) {
          const customActionParam =
            this.autohealingSettings.autoHealRules.actions.customAction
              .parameters;

          if (customActionParam.indexOf('"Memory Dump"') > 0) {
            this.validationWarning.push(
              diagnosticMemoryDumpChosen + diagnosticToolChosen
            );
          } else if (
            customActionParam.indexOf('"JAVA Memory Dump"') > 0 ||
            customActionParam.indexOf('"JAVA Thread Dump"') > 0
          ) {
            this.validationWarning.push(
              diagnosticJavaToolChosen + diagnosticToolChosen
            );
          } else if (
            customActionParam.indexOf('"CLR Profiler With ThreadStacks"') > 0
          ) {
            this.validationWarning.push(
              diagnosticProfilerWithThreadStacksChosen + diagnosticToolChosen
            );
          } else if (customActionParam.indexOf('"CLR Profiler"') > 0) {
            this.validationWarning.push(
              diagnosticProfilerChosen + diagnosticToolChosen
            );
          }
        } else {
          this.validationWarning.push(
            diagnosticToolChosenCustom + diagnosticToolChosen
          );
        }
      }
    }
