...
export class AutohealingSlowrequestsRuleComponent extends AutohealingRuleComponent {
  ...
  addNewRule() {
    ...
    this.editingSingleRule =
      this.rule == null || this.rule.slowRequests == null;
    if (this.rule == null) {
      let slowRequestsArray: SlowRequestsBasedTrigger[] = [];
      this.rule = new SlowRequestsRules(
        new SlowRequestsBasedTrigger(),
        slowRequestsArray
      );
    }
  }
  ...
  isValid(): boolean {
    if (
      this.currentRule &&
      this.currentRule.timeInterval &&
      this.currentRule.timeInterval !== '' &&
      this.currentRule.timeTaken &&
      this.currentRule.timeTaken != ''
    ) {
      let isValid: boolean =
        this.currentRule.count > 0 &&
        FormatHelper.timespanToSeconds(this.currentRule.timeInterval) > 0 &&
        FormatHelper.timespanToSeconds(this.currentRule.timeTaken) > 0 &&
        this.isValidUrlPattern(this.currentRule.path);
      return isValid;
    } else {
      return false;
    }
  ...
  saveRule() {
    this.showIntervalRecommendation =
      this.currentRule.timeInterval < this.currentRule.timeTaken;
    ...
    if (this.editingSingleRule && !this.currentRule.path) {
      ...
      if (this.slowRequestIndex < 0) {
        ...
        this.rule.slowRequestsWithPath[this.slowRequestIndex] =
          this.currentRule;
      }
    }
