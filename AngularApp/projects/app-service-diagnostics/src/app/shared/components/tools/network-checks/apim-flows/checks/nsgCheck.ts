...
import {
  ApiManagementServiceResourceContract,
  PlatformVersion,
  VirtualNetworkConfigurationContract,
  VirtualNetworkType
} from '../contracts/APIMService';
import {
  NetworkSecurityGroupContract,
  ProvisioningState,
  SecurityRuleAccess,
...
import {
  PortRequirements,
  stv1portRequirements,
  stv2portRequirements
} from '../data/portRequirements';
...
class PortRange {
  ...
  has(port: number) {
    if (this.portLowerBound >= 0 && this.portUpperBound >= 0) {
      ...
      throw Error(
        `invalid state for PortRange obj: ${this.portLowerBound}, ${this.portUpperBound}, ${this.ports}`
      );
...
function sameProtocol(
  protocol1: SecurityRuleProtocol,
  protocol2: SecurityRuleProtocol
) {
  return (
    protocol1 == SecurityRuleProtocol.AST ||
    protocol2 == SecurityRuleProtocol.AST ||
    protocol1 == protocol2
  );
}
...
function samePorts(ports: number[], portRange: string | string[]) {
  ...
  let fullRange =
    ports.includes(-1) &&
    (portRange.includes('*') || portRange.includes('0-65536'));
...
function sameIP(ip1: string, ip2: string) {
  return (
    ...
    ip2 == 'Any' ||
    ip1 == ip2 ||
    (ip1 != 'Internet' && ip2 == 'VirtualNetwork') ||
    (ip1 == 'VirtualNetwork' && ip2 != 'Internet')
  );
}
...
function ruleAffects(
  requirement: PortRequirements,
  rule: SecurityRuleContract
): boolean {
  ...
  if (!requirement.dir.includes(rule.properties.direction)) return false;
  ...
  if (
    !sameIP(
      requirement.serviceDestination,
...
function generateRequirementViolationTable(
  requirements: RequirementResultsByNsg
): string {
...
async function getViolatedRequirements(
  ...
  const securityRules = [
    ...networkSecurityGroup.properties.defaultSecurityRules,
    ...networkSecurityGroup.properties.securityRules
  ];

  let portRequirements =
    platformVersion == PlatformVersion.STV1
      ? stv1portRequirements
      : stv2portRequirements;
  portRequirements = portRequirements.filter((req) =>
    req.vnetType.includes(networkType)
  );
  return {
...
async function getVnetInfoView(
  ...
  let violatedRequirementsByLocation: {
    [key: string]: RequirementResultsByNsg;
  } = {};

  violatedRequirementsByLocation[serviceResource.location] =
    await getViolatedRequirements(
  ...
  if (serviceResource.properties.additionalLocations) {
    for (let loc of serviceResource.properties.additionalLocations) {
      violatedRequirementsByLocation[loc.location] =
        await getViolatedRequirements(
          loc.virtualNetworkConfiguration,
          diagProvider,
          networkType,
          platformVersion
        );
  ...
  let worstStatus = getWorstStatus(
    Object.values(violatedRequirementsByLocation).map((violatedRequirements) =>
      getWorstStatus(violatedRequirements.reqs.map((req) => req.status))
    )
  );
  let view = new CheckStepView({
    ...
    subChecks: Object.entries(violatedRequirementsByLocation).map(
      ([loc, violatedRequirements]) => {
        if (violatedRequirements) {
          return {
            title: loc,
            level: getWorstStatus(
              violatedRequirements.reqs.map((req) => req.status)
            ),
            bodyMarkdown:
              generateRequirementViolationTable(violatedRequirements)
          };
        } else {
          return {
            title: loc,
...
export function nsgRuleCheck(
  ...
  if (networkType != VirtualNetworkType.NONE) {
    flowMgr.addView(
      getVnetInfoView(diagProvider, serviceResource, networkType),
      'Gathering VNET Configuration'
    );
  } else {
