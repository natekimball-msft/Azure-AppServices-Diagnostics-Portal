...
code line2` + "\r\n```\r\n";
        ...
        flowMgr.addView(new ButtonStepView({
            callback: ()=>{
                alert("call back function of button, button will be hidden after clicked");
            },
            text: "Test"
        }));

        // InputStepView sample
        flowMgr.addView(new InputStepView({
            title: "title",
        ...
        flowMgr.addView(new DropdownStepView({
            dropdowns: [
                {
...
async function GetDelayedView(){
    await delay(10);
    var markdown = 'You can pass a promise of the stepView object to `flowMgr.addView(promise[, "loading message"])` to render a loading icon before the promise is completed.';
