...
export class ResourcePermissionCheckManager {
  constructor() {
    ...
    this.checkView = new CheckStepView({
      title:
        'Access was restricted to some resources, check results will be incomplete',
      level: 3,
      hidden: true,
      subChecks: []
    });
  ...
  addResource(uri) {
    ...
    this.checkView.subChecks.push({
      title: `Have no access to ${uri}`,
      level: 3
    });
  }
}
...
export async function checkKuduAvailabilityAsync(diagProvider, flowMgr) {
  ...
  var kuduAvailabilityCheckPromise = (async () => {
    ...
    if (isKuduAccessible === false) {
      ...
      views.push(
        wordings.kuduNotAccessible.get(`https://${diagProvider.scmHostName}`)
      );
    }
    return views;
  })();
  flowMgr.addViews(
    kuduAvailabilityCheckPromise,
    'Checking Kudu availability...'
  );
  var completed = false;
  delay(10).then((t) => {
    if (!completed) {
      flowMgr.setLoadText(
        `Checking Kudu availability, this process can take at most ${timeout}s...`
      );
    }
  });
...
export async function getWebAppVnetInfo(siteArmId, armService) {
  ...
  var siteVnetInfo = await armService.getArmResourceAsync(
    swiftUrl,
    '2018-02-01'
  );
...
export async function runKuduAccessibleCheck(diagProvider) {
  ...
  if (isKuduAccessible === false) {
    ...
    views.push(
      commonRec.kuduNotAccessible.get(`https://${diagProvider.scmHostname}`)
    );
  }
  return views;
}

export async function checkVnetIntegrationV2Async(
  siteInfo,
  diagProvider,
  flowMgr,
  ...
  try {
    ...
    if (vnetIntegrationType == 'swift') {
      ...
      var subnetDataPromise = diagProvider.getArmResourceAsync(
        subnetResourceId,
        vnetChecker.apiVersion
      );
      var aspSitesDataPromise = diagProvider.getArmResourceAsync(
        vnetChecker.serverFarmId + '/sites',
        vnetChecker.apiVersion
      );
      var instanceDataPromise = diagProvider.getArmResourceAsync(
        vnetChecker.siteArmId + '/instances',
        vnetChecker.apiVersion
      );
      ...
      if (isKuduAccessible) {
        if (instanceData.status == 200) {
          swiftIntegrationOk = await vnetChecker.checkWorkerPingMeshAsync(
            instanceData
          );
        } else {
          diagProvider.logException(
            new Error(`Unexpected instanceData status: ${instanceData.status}`)
          );
        }
      ...
      if (swiftIntegrationOk == null || !swiftIntegrationOk) {
        if (subnetResourceId != null) {
          ...
          if (isContinue) {
            ...
            if (subnetData.status == 200) {
              ...
              if (vnetChecker.subnetSalExists(subnetData)) {
                if (
                  vnetChecker.isSubnetSalOwnerCorrect(
                    subnetData,
                    vnetChecker.serverFarmId
                  )
                ) {
              ...
              if (vnetChecker.isSubnetDelegated(subnetData)) {
                ...
                subnetSubChecks.push(
                  wordings.subnetDelegationResult.get(false)
                );
                subnetStatus = 2;
                isContinue = false;
              }

              var subnetMask = vnetChecker.getSubnetMask(
                subnetData,
                subnetMaskThresh
              );
              if (subnetMask != null) {
                ...
                if (subnetMask > subnetMaskThresh) {
                  subnetStatus = 1;
                  views.push(
                    wordings.subnetSizeIsNotGood.get(
                      subnetResourceId,
                      subnetMask,
                      siteInfo['sku'],
                      subnetMaskThresh,
                  ...
                  subnetSubChecks.push(
                    wordings.subnetSizeIsGood.get(subnetMask)
                  );
                }
              } else {
                diagProvider.logException(
                  new Error('Unexpected null subnetMask')
                );
              }
            } else {
              if (subnetData.status == 401 || subnetData.status == 403) {
                views.push(
                  wordings.noAccessToResource.get(
                    subnetResourceId,
                    'subnet',
                    diagProvider.portalDomain
                  )
                );
              } else if (subnetData.status == 404) {
                views.push(
                  wordings.subnetNotFound.get(subnetSubChecks, subnetResourceId)
                );
                subnetStatus = 2;
              } else {
                throw new Error(
                  `Unexpected subnet data status:${subnetData.status}`
                );
              }
              isContinue = false;
            }
          ...
          if (subnetStatus != null) {
            subChecks.push(
              wordings.subnetCheckResult.get(subnetStatus, subnetSubChecks)
            );
          }
          ...
          if (isContinue) {
            ...
            if (aspSitesData.status == 200) {
              var subnets =
                vnetChecker.getAspConnectedSubnetsAsync(aspSitesData);
              if (subnets.length == 1) {
                subChecks.push(
                  wordings.swiftAspUnderLimitation.get(subnets, 1)
                );
              } else if (subnets.length > 1) {
                views.push(
                  wordings.swiftAspExceedsLimitation.get(
                    subnets,
                    1,
                    vnetChecker.serverFarmId,
                    subChecks
              ...
              if (aspSitesData.status == 401) {
                ...
                diagProvider.logException(
                  new Error(
                    `Unexpected aspSitesData status: ${aspSitesData.status}`
                  )
                );
              }
            }
          ...
          if (isContinue) {
            if (isKuduAccessible) {
              if (instanceData.status == 200) {
                var ips = await vnetChecker.getInstancesPrivateIpAsync(
                  instanceData
                );
                if (ips != null) {
                  ...
                  if (notAllocated > 0) {
                    views.push(
                      wordings.swiftPrivateIpNotAssigned.get(
                        notAllocated,
                        subchecks
                      )
                    );
                ...
                diagProvider.logException(
                  new Error(
                    `Unexpected instanceData status: ${instanceData.status}`
                  )
                );
              }
            } else {
          ...
          var integrationCheck = wordings.vnetIntegrationResult.get(
          ...
          diagProvider.logException(
            new Error(`invalid subnetResourceId ${subnetResourceId}`)
          );
        }
      ...
      if (
        siteGWVnetInfo[0]['properties'] != null &&
        siteGWVnetInfo[0]['properties']['vnetResourceId'] != null
      ) {
        var vnetResourceId = siteGWVnetInfo[0]['properties']['vnetResourceId'];
        var vnetData = await diagProvider.getArmResourceAsync(
          vnetResourceId,
          vnetChecker.apiVersion
        );
        if (vnetData.status == 200) {
          ...
          views.push(
            wordings.gatewayVnetNotFound.get(subChecks, vnetResourceId)
          );
          integrationCheckStatus = 2;
          isContinue = false;
        } else {
          diagProvider.logException(
            new Error(`Unknown vnetData status ${vnetData.status}`)
          );
        }
      } else {
        diagProvider.logException(
          new Error(`invalid siteGWVnetInfo ${JSON.stringify(siteGWVnetInfo)}`)
        );
      }
      var integrationCheck = wordings.vnetIntegrationResult.get(
        integrationCheckStatus,
      ...
      throw new Error(
        'Unexpected null result returned from vnetChecker.getVnetIntegrationTypeAsync()'
      );
    }
...
export async function checkWorkerPingMeshAsync(
  siteInfo,
  diagProvider,
  flowMgr
) {
  ...
  var wordings = new VnetIntegrationWordings();
  var vnetChecker = new VnetIntegrationConfigChecker(siteInfo, diagProvider);
  var instanceData = await diagProvider.getArmResourceAsync(
    vnetChecker.siteArmId + '/instances',
    vnetChecker.apiVersion
  );
  if (instanceData.status == 200) {
    ...
    if (ips != null) {
      ...
      if (notAllocated > 0) {
        views.push(
          wordings.swiftPrivateIpNotAssigned.get(notAllocated, subchecks)
        );
      } else {
        subChecks.push(wordings.swiftPrivateIpAssigned.get(total));
      }
...
export async function checkDnsSettingV2Async(
  siteInfo,
  diagProvider,
  flowMgr,
  isKuduAccessiblePromise,
      ...
      if (await isKuduAccessiblePromise) {
        ...
        if (appSettingDns[0] != null) {
          ...
          for (let idx = 0; idx < appSettingDns.length; ++idx) {
            ...
            if (dns != null) {
              ...
              appSettingDnsSubChecks.push(
                wordings.dnsReachability.get(
                  dns,
                  idx == 0 ? 'WEBSITE_DNS_SERVER' : 'WEBSITE_DNS_ALT_SERVER',
                  result
                )
              );
              if (result == false) {
          ...
          if (unreachableDns.length > 0) {
            views.push(
              wordings.dnsIsUnhealthy.get(
                dnsSettings,
                unreachableDns,
                'App Settings',
                subChecks,
                appSettingDnsSubChecks
              )
            );
            ...
            subChecks.push(
              wordings.dnsIsHealthy.get(appSettingDnsSubChecks, 'App Settings')
            );
          }
        ...
        if (isContinue) {
          ...
          if (vnetData.status == 200) {
            ...
            if (vnetDnsSettings != null && vnetDnsSettings.length > 0) {
              if (dnsSettings != null && dnsSettings.length > 0) {
                ...
                if (vnetDnsSettings.length > 2) {
                  // if windows
                  subChecks.push(
                    wordings.onlyTwoVnetDnsWillBeApplied.get(vnetDnsSettings)
                  );
                }
                ...
                for (let idx = 0; idx < dnsSettings.length; ++idx) {
                  ...
                  let result = await dnsChecker.isDnsServerReachableAsync(dns);
                  vnetDnsSubChecks.push(
                    wordings.dnsReachability.get(
                      dns,
                      `VNet DNS list position ${idx}`,
                      result
                    )
                  );
                  if (result == false) {
                ...
                if (unreachableDns.length > 0) {
                  ...
                  subChecks.push(
                    wordings.dnsIsHealthy.get(vnetDnsSubChecks, 'VNet')
                  );
                }
              }
            }
            var fallbackToAzureDns =
              await dnsChecker.getAppSettingAlwaysFallbackToPublicDnsAsync();
            if (dnsSettings != null && dnsSettings.length > 0) {
            ...
            if (
              dnsSettings != null &&
              (dnsSettings.length == 0 || fallbackToAzureDns)
            ) {
              dnsSettings.push('');
            }
          } else {
...
export function extractHostPortFromConnectionString(connectionString) {
  ...
  if (connectionString != undefined && connectionString != null) {
    ...
    if (connectionStringKVMap['AccountName'] != undefined) {
      ...
      hostName =
        connectionStringKVMap['EndpointSuffix'] != undefined
          ? storageAccountName +
            '.blob.' +
            connectionStringKVMap['EndpointSuffix']
          : storageAccountName + '.blob.core.windows.net';
      port =
        connectionStringKVMap['DefaultEndpointsProtocol'] == 'http' ? 80 : 443;
      ...
      hostName = connectionStringKVMap['Endpoint']
        .split('sb://')[1]
        .split('/')[0];
      port = 443;
      ...
      hostName = connectionStringKVMap['AccountEndpoint']
        .split('https://')[1]
        .split(':')[0];
      port = connectionStringKVMap['AccountEndpoint']
        .split('https://')[1]
        .split(':')[1]
        .split('/')[0];
      ...
      hostName = connectionStringKVMap['IngestionEndpoint']
        .split('https://')[1]
        .split('/')[0];
      port = 443;
    }
...
export function extractHostPortFromKeyVaultReference(keyVaultReference) {
  ...
  if (keyVaultReference.indexOf(secretUriPrefix) != -1) {
    ...
    hostName =
      keyVaultReference.split(vaultNamePrefix)[1].split(';')[0] +
      '.' +
      keyVaultDomainName;
  }
...
export function addSubnetSelectionDropDownView(
  siteInfo,
  diagProvider,
  flowMgr,
  title,
  ...
  var dropdownView = new DropdownStepView({
    ...
    async callback(dropdownIdx, selectedIdx) {
      if (dropdownIdx === 0) {
        ...
        vnets = await diagProvider.getArmResourceAsync(
          `/subscriptions/${subscription.subscriptionId}/providers/Microsoft.Network/virtualNetworks`,
          '2018-07-01'
        );
        dropdownView.dropdowns.length = 1;
        vnets = vnets.value
        ...
        subnets =
          vnet.properties == null
            ? []
            : vnet.properties.subnets.filter((s) => s && s.name != null);
        subnets = subnets.sort((s1, s2) =>
          s1.name.toLowerCase() > s2.name.toLowerCase() ? 1 : -1
        );
  ...
  diagProvider.getArmResourceAsync('subscriptions').then((s) => {
    subscriptions = s.value
      .filter((s) => s && s.displayName != null)
      .sort((s1, s2) =>
        s1.displayName.toLowerCase() > s2.displayName.toLowerCase() ? 1 : -1
      );
...
export async function checkNetworkTroubleshooterApiAsync(diagProvider) {
  ...
  var tcpPingPromise = diagProvider
    .tcpPingNetworkTroubleshooterAsync(healthyTarget, healthyPort)
    .catch((e) => {
      logDebugMessage(e);
      return {};
    });
  ...
  if (response == null) {
    throw new Error(
      'Unexpected Null response in CheckNetworkTroubleShooterApiAsync for Tcppingpromise'
    );
  } else {
