...
import {
  checkKuduAvailabilityAsync,
  checkVnetIntegrationV2Async,
  checkDnsSettingV2Async,
  checkAppSettingsAsync,
  extractHostPortFromConnectionString,
  extractHostPortFromKeyVaultReference,
  checkNetworkTroubleshooterApiAsync
} from './flowMisc.js';
...
export class ConnectionStringType {
  static get BlobStorageAccount() {
    return 'BlobStorage';
  }
  static get QueueStorageAccount() {
    return 'QueueStorage';
  }
  static get FileShareStorageAccount() {
    return 'FileShare';
  }
  static get ServiceBus() {
    return 'ServiceBus';
  }
  static get EventHubs() {
    return 'EventHubs';
  }
}
...
export var functionsFlow = {
  ...
  async func(siteInfo, diagProvider, flowMgr) {
    ...
    if (
      siteInfo.kind.includes('linux') ||
      siteInfo.kind.includes('container')
    ) {
      isKuduAccessiblePromise = false;
    } else {
      isKuduAccessiblePromise = checkKuduAvailabilityAsync(
        diagProvider,
        flowMgr
      );
    }
    var dnsServers = null;
    var vnetConfigChecker = new VnetIntegrationConfigChecker(
      siteInfo,
      diagProvider
    );
    var vnetIntegrationType =
      await vnetConfigChecker.getVnetIntegrationTypeAsync();
    var isVnetIntegrated =
      vnetIntegrationType != null && vnetIntegrationType != 'none';
    if (isVnetIntegrated) {
      var isVnetIntegrationHealthy = await checkVnetIntegrationV2Async(
        siteInfo,
        diagProvider,
        flowMgr,
        isKuduAccessiblePromise
      );
    ...
    if (!(await isKuduAccessiblePromise)) {
      if (
        ...
        flowMgr.addView(
          new CommonWordings().connectivityCheckUnsupported.get()
        );
      } else {
        flowMgr.addView(
          new VnetDnsWordings().cannotCheckWithoutKudu.get('Functions settings')
        );
      }
      return;
    }
    var checkBackendAccess = await checkNetworkTroubleshooterApiAsync(
      diagProvider
    );
    ...
    var checkFunctionAppCommonDepsPromise = (async () => {
      ...
      if (appSettings.AzureWebJobsStorage == undefined) {
        if (appSettings.AzureWebJobsStorage__blobServiceUri != undefined) {
          ...
          var subChecksL2 = await networkCheckConnectionString(
            propertyName,
            connectionString,
            ConnectionStringType.BlobStorageAccount,
            dnsServers,
            diagProvider,
            isVnetIntegrated,
          ...
          var title =
            maxCheckLevel == 0
              ? `Network connectivity test to Azure storage endpoint configured in app setting "${propertyName}" was successful.`
              : `Network connectivity test to Azure storage endpoint configured in app setting "${propertyName}" failed.`;
          subChecksL1.push({
            title: title,
            subChecks: subChecksL2,
            level: maxCheckLevel
          });
        }
        if (appSettings.AzureWebJobsStorage__queueServiceUri != undefined) {
          ...
          var subChecksL2 = await networkCheckConnectionString(
            propertyName,
            connectionString,
            ConnectionStringType.QueueStorageAccount,
            dnsServers,
            diagProvider,
            isVnetIntegrated,
            failureDetailsMarkdown
        ...
        var connectionStringType = isBackendAccessible
          ? ConnectionStringType.BlobStorageAccount
          : undefined;
        var subChecksL2 = await networkCheckConnectionString(
          propertyName,
      ...
      connectionString = appSettings[propertyName];
      var connectionStringType = isBackendAccessible
        ? ConnectionStringType.FileShareStorageAccount
        : undefined;
      var subChecksL2 = await networkCheckConnectionString(
        propertyName,
        connectionString,
      ...
      var title =
        maxCheckLevel == 0
          ? `Network connectivity test to the Azure storage endpoint configured in app setting "${propertyName}" was successful.`
      ...
      if (
        ...
        var title =
          maxCheckLevel == 0
            ? `Network connectivity test to the endpoint configured in app setting "${propertyName}" was successful.`
      ...
      if (connectionString != undefined) {
        ...
        var title =
          maxCheckLevel == 0
            ? `Network connectivity test to the Application Insights endpoint was successful.`
      ...
      var title =
        maxCheckLevel == 0
          ? 'Evaluated network connectivity for common Function App dependencies.'
      ...
      return new CheckStepView({
        title: title,
        subChecks: subChecksL1final,
        level: maxCheckLevel
      });
    })(); // end of checkFunctionAppCommonDepsPromise

    flowMgr.addView(
      checkFunctionAppCommonDepsPromise,
      'Checking common Function App settings...'
    );
    ...
    var checkFunctionBindingsPromise = (async () => {
      ...
      var functionsList = await diagProvider.getArmResourceAsync(
        functionAppResourceId + '/functions'
      );
      if (functionsList == undefined || functionsList.value == undefined) {
        return new CheckStepView({
          title: 'Could not get the list of functions in this Function App.',
          detailsMarkdown:
            'If retrying does not work, the app is most likely in an unhealthy state.  Please check the values configured in the app settings **AzureWebJobsStorage** and **WEBSITE_CONTENTAZUREFILECONNECTIONSTRING** point to an existing storage account and that it is accessible by the Function app using the checks above.',
          level: 2
        });
      }
      if (functionsList.value.length == 0) {
        return new InfoStepView({
          infoType: 0,
          title: 'No functions were found for this Function App'
        });
      }
      ...
      functionsList.value.forEach((func) => {
        ...
        func.properties.config.bindings.forEach((binding) => {
          var bindingInfo = {
            name: binding.name,
            type: binding.type,
            connectionStringProperty: undefined,
            connectionString: undefined
          };
          ...
          } else if (binding.connectionStringSetting != undefined) {
            // CosmosDB
            bindingInfo.connectionStringProperty =
              binding.connectionStringSetting;
          } else {
            // not a trigger/binding that has connection info (e.g. Timer trigger, HTTP trigger)
            return;
          }
          // bindingInfo.connectionString
          var connectionString =
            appSettings[bindingInfo.connectionStringProperty];
          bindingInfo.connectionString = connectionString;
          // bindingInfo.entityName
          if (
            binding.type == 'serviceBusTrigger' &&
            binding.topicName != undefined
          ) {
            // Service Bus topic
            binding.entityName = binding.topicName;
          } else if (
            binding.type == 'serviceBusTrigger' &&
            binding.queueName != undefined
          ) {
            // Service Bus queue
            binding.entityName = binding.queueName;
          } else if (
            binding.type == 'eventHubTrigger' &&
            binding.eventHubName != undefined
          ) {
            // Event Hubs
      ...
      if (functionsInfo.length == 0) {
        return new InfoStepView({
          infoType: 0,
          title:
            'No functions with configured connection strings were found for this Function App'
        });
      }
      ...
      var promisesL1 = functionsInfo.map(async (functionInfo) => {
        ...
        var promisesL2 = functionInfo.bindings.map(async (binding) => {
          ...
          var connectionStringType = isBackendAccessible
            ? bindingTypeToConnectionStringType(binding.type)
            : undefined;
          (
            await networkCheckConnectionString(
              binding.connectionStringProperty,
              connectionString,
              connectionStringType,
              dnsServers,
              diagProvider,
        ...
        var functionName =
          functionInfo.name.split('/').length < 2
            ? functionInfo.name
            : functionInfo.name.split('/')[1];
        var maxCheckLevel = getMaxCheckLevel(subChecksL2);
        var title =
          maxCheckLevel == 0
            ? `Function "${functionName}" - all network connectivity tests were successful.`
      ...
      var title =
        maxCheckLevel == 0
          ? 'Evaluated network connectivity for all Function input/output bindings.'
    ...
    flowMgr.addView(
      checkFunctionBindingsPromise,
      'Checking all Function bindings...'
    );
    ...
    flowMgr.addView(
      new InfoStepView({
        ...
        markdown:
          'Other than for Storage, Service Bus and Event Hubs dependencies, positive tests above indicate a network layer connection was successfully established between this app and the configured remote service.' +
          '\r\n\r\n' +
...
function getMaxCheckLevel(subChecks) {
  var maxCheckLevel = 0;
  subChecks.forEach(
    (check) => (maxCheckLevel = Math.max(maxCheckLevel, check.level))
  );
  return maxCheckLevel;
}

async function networkCheckConnectionString(
  propertyName,
  connectionString,
  connectionStringType = undefined,
  dnsServers,
  diagProvider,
  isVnetIntegrated,
  failureDetailsMarkdown = undefined,
  ...
  if (
     ...
     */
    var connectivityCheckResult = await validateConnection(
      propertyName,
      connectionStringType,
      diagProvider,
      entityName
    ...
    var title =
      maxCheckLevel == 0
        ? `Successfully connected to the ${service} resource configured for connection "${propertyName}".`
    ...
    if (isKeyVaultReference(connectionString)) {
      var kvConnectivityCheckResult = await networkCheckKeyVaultReferenceAsync(
        propertyName,
        connectionString,
        dnsServers,
        diagProvider,
      ...
      if (hostPort.HostName != undefined && hostPort.Port != undefined) {
        var connectivityCheckResult = await runConnectivityCheckAsync(
          hostPort.HostName,
          hostPort.Port,
          dnsServers,
          diagProvider,
        ...
        var title =
          maxCheckLevel == 0
            ? `Successfully accessed the endpoint "${hostPort.HostName}:${hostPort.Port}" configured in App Setting "${propertyName}"`
        ...
        // Unsupported or invalid connection string format
...
async function networkCheckKeyVaultReferenceAsync(
  propertyName,
  connectionString,
  dnsServers,
  diagProvider,
  ...
  var failureDetailsMarkdown =
    'Please refer to <a href= "https://docs.microsoft.com/en-us/azure/app-service/app-service-key-vault-references#reference-syntax" target="_blank">this documentation</a> to configure the Key Vault reference correctly.';
  var subChecks = [];
...
async function runConnectivityCheckAsync(
  hostname,
  port,
  dnsServers,
  diagProvider,
  ...
  var nameResolvePromise = (async function checkNameResolve() {
    ...
    if (diagProvider.isIp(hostname)) {
      ...
      for (var i = 0; i < dnsServers.length; ++i) {
        var result = await diagProvider
          .nameResolveAsync(hostname, dnsServers[i])
          .catch((e) => {
            logDebugMessage(e);
            return {};
          });
        var dns =
          dnsServers[i] == ''
            ? 'Azure DNS server'
            : `DNS server ${dnsServers[i]}`;
        if (result.ip != null) {
          ip = result.ip;
          checkResultsMarkdown.push(
            `Successfully resolved hostname **${hostname}** with ${dns}`
          );
          break;
        } else {
          checkResultsMarkdown.push(
            `Failed to resolve hostname **${hostname}** with ${dns}`
          );
        }
      }
    }
  ...
  var tcpPingPromise = diagProvider
    .tcpPingNetworkTroubleshooterAsync(hostname, port)
    .catch((e) => {
      logDebugMessage(e);
      return {};
    });
  ...
  if (resolvedIp != hostname) {
    hostname =
      hostname.length > lengthLimit
        ? hostname.substr(0, lengthLimit) + '...'
        : hostname;
    if (resolvedIp == null) {
  ...
  if (status == 0) {
    subChecks.push({
      title: `TCP ping to ${hostname} was successful`,
      level: 0
    });
  } else if (status == 1) {
    var markdown =
      `Connectivity test failed at TCP level for hostname **${hostname}** via resolved IP address ${resolvedIp}.  ` +
    ...
    subChecks.push({
      ...
      level: 2,
      detailsMarkdown:
        'Encountered an unknown problem, please send us feedback via the ":) Feedback" button above.'
    });
  }
  return subChecks;
}
...
async function validateConnection(
  ...
  var checkConnectionStringResult = await diagProvider
    .checkConnectionViaAppSettingAsync(propertyName, type, entityName)
    .catch((e) => {
      logDebugMessage(e);
    });
