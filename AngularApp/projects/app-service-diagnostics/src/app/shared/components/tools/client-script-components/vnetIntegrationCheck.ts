...
import {
  DynamicInsight,
  Insight
} from '../../../../../../../diagnostic-data/src/lib/models/insight';
...
export class VnetIntegrationCheck implements ClientScriptComponent {
  ...
  async func(injector: Injector): Promise<DataRenderBaseComponent> {
    var { siteInfo, diagProvider, telemetryService } =
      Utils.getAppServiceSiteObjectsAndServices(injector);
    ...
    var vnetIntegrationType: string =
      await vnetChecker.getVnetIntegrationTypeAsync();

    var insightComponent = new DynamicInsightV4Component(
      markdownService,
      telemetryService,
      router
    );
    ...
    if (vnetIntegrationType == 'swift') {
      ...
      if (instanceData.status == 200) {
        var swiftIntegrationOk: boolean =
          await vnetChecker.checkWorkerPingMeshAsync(instanceData);
        if (swiftIntegrationOk != null) {
          ...
          if (swiftIntegrationOk) {
            ...
            insight.description =
              'Regional VNet integration is configured and healthy';
          } else {
            insight = new DynamicInsight('Critical', title, false);
            insight.description =
              "Regional VNet integration is configured but it's unhealthy";
          }
      ...
      insightComponent.insight.description =
        'Gateway-required VNet integration is configured';
    } else {
      insightComponent.insight = new DynamicInsight('Info', title, false);
      insightComponent.insight.description =
