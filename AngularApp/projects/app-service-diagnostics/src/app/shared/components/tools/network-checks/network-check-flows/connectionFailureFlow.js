...
export var connectionFailureFlow = {
  ...
  async func(siteInfo, diagProvider, flowMgr) {
    ...
    var isContinue = await checkVnetIntegrationV2Async(
      siteInfo,
      diagProvider,
      flowMgr,
    ...
    var isLinuxOrContainer =
      siteInfo.kind.includes('linux') || siteInfo.kind.includes('container');

    var dnsSettings = [];
    if (!isLinuxOrContainer) {
      isContinue = await checkDnsSettingV2Async(
        siteInfo,
        diagProvider,
        flowMgr,
        isKuduAccessiblePromise,
    ...
    checkNetworkConfigAndConnectivity(
      siteInfo,
      diagProvider,
      flowMgr,
      isKuduAccessiblePromise,
...
async function runConnectivityCheck(hostname, port, diagProvider, lengthLimit) {
  var subChecks = [];
  var tcpPingResult = await diagProvider
    .tcpPingAsync(hostname, port)
    .catch((e) => {
      logDebugMessage(e);
      return {};
    });
  ...
  if (status == 0) {
    markdown =
      'Connectivity test succeeded at TCP level. ' +
    ...
    subChecks.push({
      title: `TCP ping to ${hostname} was succeeded`,
      level: 0
    });
    views.push(
      new CheckStepView({
    ...
    markdown =
      'Connectivity test failed at TCP level. ' +
    ...
    subChecks.push({
      title: `TCP ping to ${hostname} failed, timeout because target is unreachable`,
      level: 2
    });
    views.push(
      new CheckStepView({
    ...
    subChecks.push({
      title: `TCP ping to ${hostname} failed, errorcode:${status}`,
      level: 2
    });
    views.push(
      new CheckStepView({
...
async function checkNameResolvingAsync(hostname, siteInfo, diagProvider) {
  ...
  if (diagProvider.isIp(hostname)) {
    ...
    var result = await diagProvider
      .nameResolveByDaasAsync(hostname)
      .catch((e) => {
        logDebugMessage(e);
        return {};
      });
  ...
  if (isContinue) {
    ...
    var ipResults = ips
      .filter((ip) => diagProvider.isIp(ip, true))
      .map((ip) => [ip, isRfc1918Ip(ip)])
      .map((p) => [p[0], p[1], p[1] || routeAll]);
...
function isRfc1918Ip(ip) {
  ...
  for (var i = 0; i < 3; ++i) {
    ...
    let maskedIp = splitted
      .map((num, idx) => num & maskSplitted[idx])
      .join('.');
...
async function checkNetworkConfigAndConnectivity(
  siteInfo,
  diagProvider,
  flowMgr,
  isKuduAccessiblePromise,
  ...
  var connectivityCheckViews = (function generateConnectivityCheckViews() {
    if (!isKuduAccessible) {
      return [
        new CheckStepView({
          title:
            'Connectivity check (tcpping and nameresolver) is not available due to Kudu is inaccessible.',
          level: 3
        })
      ];
    }
        ...
        async callback(userInput) {
          ...
          if (userInput.startsWith('http')) {
            ...
            if (hostname == null) {
              ...
              userInput = this.text;
              userInput =
                userInput.length > userInputLimitInUI
                  ? userInput.substr(0, userInputLimitInUI) + '...'
                  : userInput;
            }
          } else {
          ...
          if (vnetIntegrationType == 'swift') {
            var nameResolvingPromise = checkNameResolvingAsync(
              hostname,
              siteInfo,
              diagProvider
            );
            flowMgr.addViews(
              nameResolvingPromise.then((p) => p.views),
              'Analyzing target server ip...'
            );
