  ...
  constructor() {
    ...
    this.dnsReachability = {
      get(dns, config, reachable) {
        return new CheckStepView({
          title: `DNS ${dns} configured in ${config} is ${
            reachable ? '' : 'not'
          } reachable`,
          level: reachable ? 0 : 2
        });
    ...
    this.dnsIsUnhealthy = {
      get(dnsSettings, unreachableDns, configuredIn, subChecks, dnsSubChecks) {
        ...
        if (dnsSettings.length == unreachableDns.length) {
          markdown =
            `None of the DNS server ${dnsSettings
              .filter((i) => i != null)
              .join(
                ' and '
              )} you configured in ${configuredIn} is not reachable from this app` +
          ...
          markdown =
            `One of the DNS server ${dnsSettings
              .filter((i) => i != null)
              .join(
                ' and '
              )} you configured in ${configuredIn} is not reachable from this app.` +
    ...
    this.appSettingDnsOnlyHasAlternative = {
      get() {
        return new CheckStepView({
          title:
            "WEBSITE_DNS_ALT_SERVER in App Setting won't be applied if WEBSITE_DNS_SERVER is not configured",
          level: 1
        });
      }
    };
    ...
    this.appSettingDnsOverrideVnetDns = {
      get() {
        return new CheckStepView({
          title:
            "You have WEBSITE_DNS_SERVER configured in App Setting, the DNS setting in VNet won't be applied",
          level: 1
        });
      }
    };
    ...
    this.onlyTwoVnetDnsWillBeApplied = {
      get(dnsSettings) {
        return new CheckStepView({
          title: `Detected ${
            dnsSettings.length
          } VNet DNS settings, but only 2 of them ${dnsSettings
            .slice(0, 2)
            .join(' and ')} will be applied`,
          level: 1
        });
      }
    };
    ...
    this.fallbackToAzureDnsConfigured = {
      get() {
        return new CheckStepView({
          title:
            'WEBSITE_ALWAYS_FALLBACK_TO_PUBLIC_DNS is set to 1, Azure default DNS 168.63.129.16 will be applied as fallback',
          level: 3
        });
      }
    };
    ...
    this.configuredDns = {
      get(dnsSettings, source, fallbackToAzureDns) {
        var defaultDns = {
          title: '168.63.129.16(Azure Default DNS)',
          level: 3
        };
        if (dnsSettings != null && dnsSettings.length > 0) {
          ...
          return new CheckStepView({
            title:
              'Custom DNS setting detected, following DNS will be used for name resolving',
            level: 3,
            subChecks: subChecks
          });
        } else {
          return new CheckStepView({
            title:
              'No custom DNS is configured, default Azure DNS will be applied',
            level: 3,
