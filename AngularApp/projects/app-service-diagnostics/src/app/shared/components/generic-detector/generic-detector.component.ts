...
@Component({
  ...
  providers: [FeatureNavigationService]
})
  ...
  constructor(
    ...
    this._activatedRoute.paramMap.subscribe((params) => {
      ...
      this._authServiceInstance.getStartupInfo().subscribe((startUpInfo) => {
        if (startUpInfo) {
          this.isCaseSubmissionSolutionIFrame =
            startUpInfo.isIFrameForCaseSubmissionSolution != undefined
              ? startUpInfo.isIFrameForCaseSubmissionSolution
              : false;
        }
      });
