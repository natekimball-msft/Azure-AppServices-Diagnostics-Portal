...
export class ConfigureStorageAccountComponent implements OnInit {
      ...
      (newStorageAccount) => {
        ...
        if (newStorageAccount.sasUri || newStorageAccount.connectionString) {
          this.validationResult.BlobSasUri = newStorageAccount.sasUri
            ? newStorageAccount.sasUri
            : '';
          this.validationResult.ConnectionString =
            newStorageAccount.connectionString
              ? newStorageAccount.connectionString
              : '';
  ...
  @Output() StorageAccountValidated: EventEmitter<DaasValidationResult> =
      ...
      .subscribe(
        (daasStorageConfiguration) => {
          if (
            ...
            daasStorageConfiguration.IsAppSetting
          ) {
            this._daasService
              .validateSasUri(this.siteToBeDiagnosed)
              .subscribe((resp) => {
                this.checkingBlobSasUriConfigured = false;
                if (resp.IsValid) {
                  this.validateStorageConfigurationResponse = null;
                  this.chosenStorageAccount =
                    this.getStorageAccountNameFromSasUri(
                      daasStorageConfiguration.SasUri
                    );
                  this.validationResult.BlobSasUri =
                    daasStorageConfiguration.SasUri;
                  this.validationResult.ConfiguredAsAppSetting =
                    daasStorageConfiguration.IsAppSetting;
                  this.validationResult.Validated = true;
                  this.StorageAccountValidated.emit(this.validationResult);
                } else {
            ...
            this._daasService
              .validateStorageAccount(this.siteToBeDiagnosed)
              .subscribe((resp) => {
                this.checkingBlobSasUriConfigured = false;
                if (resp.IsValid) {
                  this.validateStorageConfigurationResponse = null;
