...
export class ResourceRedirectComponent implements OnInit {
  ...
  ngOnInit() {
    this._telemetryService.updateCommonProperties({
      Location: TelemetrySource.DiagAndSolveBlade
    });
    this._versionTestService.isLegacySub.subscribe(
      (useLegacyVersion) => (this._useLegacyVersion = useLegacyVersion)
    );
  ...
  navigateToExperience() {
    this._authService.getStartupInfo().subscribe((info) => {
      if (info) {
        ...
        const sapSupportTopicId = info.sapSupportTopicId
          ? info.sapSupportTopicId
          : '';
        const sessionId = info.sessionId ? info.sessionId : '';
        const effectiveLocale = !!info.effectiveLocale
          ? info.effectiveLocale.toLowerCase()
          : '';
        const theme = !!info.theme ? info.theme.toLowerCase() : '';
        const highContrastKey = !!info.highContrastKey
          ? info.highContrastKey.toString()
          : '';
      ...
      if (!!info && !!info.resourceId && !!info.token) {
        if (
          !!info.optionalParameters &&
          Array.isArray(info.optionalParameters) &&
          info.optionalParameters.find((param) => param.key === 'categoryId')
        ) {
        ...
        this._newVersionEnabled =
          DemoSubscriptions.betaSubscriptions.indexOf(subscriptionId) >= 0;
        ...
        if (Array.isArray(info.optionalParameters)) {
          let startTime = info.optionalParameters.find(
            (param) => param.key === 'startTime'
          );
          let endTime = info.optionalParameters.find(
            (param) => param.key === 'endTime'
          );
          ...
          var caseSubjectParam = info.optionalParameters.find(
          ...
          var referrerParam = info.optionalParameters.find(
            (param) => param.key.toLowerCase() === 'referrer'
          );
          if (referrerParam) {
            this._telemetryService.updateCommonProperties({
              Location: TelemetrySource.PortalReferral
            });
        ...
        if (!this._useLegacyVersion) {
          ...
          if (Array.isArray(info.optionalParameters)) {
            let categoryIdParam = info.optionalParameters.find(
              (param) => param.key === 'categoryId'
            );
            if (categoryIdParam) {
              ...
              let detectorTypeParam = info.optionalParameters.find(
                (param) => param.key === 'detectorType'
              );
              let detectorIdParam = info.optionalParameters.find(
                (param) => param.key === 'detectorId'
              );
              let toolIdParam = info.optionalParameters.find(
                (param) => param.key === 'toolId'
              );
              let startTime = info.optionalParameters.find(
                (param) => param.key === 'startTime'
              );
  ...
  updateRouteBasedOnAdditionalParameters(
    route: string,
