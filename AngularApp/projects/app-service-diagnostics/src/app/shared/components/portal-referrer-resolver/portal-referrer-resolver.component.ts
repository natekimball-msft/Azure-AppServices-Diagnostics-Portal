...
import {
  PortalReferrerMap,
  PortalReferrerInfo
} from '../../models/portal-referrer-map';
...
export class PortalReferrerResolverComponent implements OnInit {
  ...
  constructor(
    ...
    private _logService: TelemetryService
  ) {
    this.isEvaluating = true;
    this.loadingMessage = 'Collecting data to analyze your issue...';

    this._resourceService
      .getIbizaBladeToDetectorMapings()
      .subscribe((mappingArray) => {
    ...
    this._authService.getStartupInfo().subscribe((startUpInfo) => {
      var referrerParam = startUpInfo.optionalParameters.find(
  ...
  matchReferrerAndRoute(referrer: PortalReferrerInfo): void {
    ...
    if (
      !!referrer.ExtensionName &&
      !!referrer.BladeName &&
      referrer.ExtensionName === 'Microsoft_Azure_CloudNativeTesting' &&
      referrer.BladeName === 'ReportsBlade'
    ) {
      if (
        this._resourceService &&
      ...
      if (
        referrer.DetectorType &&
        (referrer.DetectorType.toLowerCase() ===
          DetectorType.Analysis.toLowerCase() ||
          referrer.DetectorType.toLowerCase() ===
            DetectorType.Detector.toLowerCase()) &&
        referrer.DetectorId &&
        referrer.DetectorId.length > 1
      ) {
        this._logService.logEvent('IntegratedDiagnostics', {
        ...
        if (
          ...
          referrer.DetectorType.toLowerCase() ===
          DetectorType.DiagnosticTool.toLowerCase()
        ) {
          path = `${path}/tools/${referrer.DetectorId}`;
        }
      } else {
        var referrerMatch = this.portalReferrerToDetectorMap.find(
          (referrerMap) =>
            referrerMap.ReferrerExtensionName.toLowerCase() ===
              referrer.ExtensionName.toLowerCase() &&
            referrerMap.ReferrerBladeName.toLowerCase() ===
              referrer.BladeName.toLowerCase() &&
            referrerMap.ReferrerTabName.toLowerCase() ===
              referrer.TabName.toLowerCase()
        );
        ...
        if (referrerMatch) {
          if (referrerMatch.DetectorType === DetectorType.Detector) {
            ...
            referrerMatch.DetectorType === DetectorType.DiagnosticTool
          ) {
          ...
          this._logService.logEvent('IntegratedDiagnostics', {
            details:
              'Redirecting to home as detector map did not contain a match and Ibiza did not pass redirect parameters.',
            path: path,
            referrerInformation: JSON.stringify(referrer),
            targetType: '',
            target: ''
