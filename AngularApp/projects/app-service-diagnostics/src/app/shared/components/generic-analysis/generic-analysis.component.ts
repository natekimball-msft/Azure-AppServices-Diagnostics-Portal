...
@Component({
  ...
  styleUrls: [
    './generic-analysis.component.scss',
    '../generic-detector/generic-detector.component.scss'
  ]
})
export class GenericAnalysisComponent
  extends GenericDetectorComponent
  implements OnInit
{
  ...
  downTime: DownTime;
  @ViewChild('detectorListAnalysis', { static: true })
  detectorListAnalysis: DetectorListAnalysisComponent;
  ...
  constructor(
    private _activatedRouteLocal: ActivatedRoute,
    private _diagnosticServiceLocal: DiagnosticService,
    _resourceService: ResourceService,
    _authServiceInstance: AuthService,
    public _telemetryService: TelemetryService,
    _navigator: FeatureNavigationService,
    ...
    super(
      _activatedRouteLocal,
      _diagnosticServiceLocal,
      _resourceService,
  ...
  ngOnInit() {
    ...
    this._activatedRouteLocal.paramMap.subscribe((params) => {
      ...
      this.detectorId =
        params.get('detectorName') === null ? '' : params.get('detectorName');
      this._activatedRouteLocal.queryParamMap.subscribe((qParams) => {
        ...
        if (
          ...
          this.showSearchBar =
            this.searchMode === SearchAnalysisMode.CaseSubmission
              ? true
              : this.showSearchBar;
  ...
  triggerSearch() {
    if (this.searchTerm && this.searchTerm.length > 1) {
      ...
      searchBar.blur();
      this._routerLocal.navigate([`../../${this.analysisId}/search`], {
        relativeTo: this._activatedRouteLocal,
  ...
  showChatButton(): boolean {
    return (
      this.isPublic && this.cxpChatTrackingId != '' && this.cxpChatUrl != ''
    );
  }
