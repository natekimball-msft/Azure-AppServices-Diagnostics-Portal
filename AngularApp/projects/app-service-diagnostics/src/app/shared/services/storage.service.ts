...
export class StorageService {
  ...
  getStorageAccountKey(storageAccountId: string): Observable<StorageKeys> {
    let url =
      this._uriElementsService.getStorageAccountKeyUrl(storageAccountId);
    return this._armClient
      .postResourceWithoutEnvelope<StorageKeys, any>(url, null, this.apiVersion)
      .pipe(
  ...
  createStorageAccount(
    ...
    let url = this._uriElementsService.createStorageAccountsUrl(
      subscriptionId,
      resourceGroupName,
    ...
    return this._armClient
      .putResourceFullResponse(url, requestBody, true, this.apiVersion)
      .pipe(
  ...
  createContainerIfNotExists(
    ...
    let url = this._uriElementsService.getStorageContainerUrl(
      storageAccountId,
      containerName.toLowerCase()
    );

    return this._armClient
      .getResourceFullResponse<ArmResource>(url, true, this.apiVersion)
      .pipe(
        map((containerResp) => {
          if (
            containerResp.ok &&
            containerResp.body.properties &&
            !containerResp.body.properties.deleted
          ) {
            return true;
          }
        }),
  ...
  generateSasKey(
    storageAccountResourceUri: string,
    ...
    return this._armClient
      .postResourceFullResponse<SasUriPostResponse>(
        url,
        postBody,
        true,
        this.apiVersion
