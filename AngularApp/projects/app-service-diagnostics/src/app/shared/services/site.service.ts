...
export class SiteService {
  ...
  public currentSiteMetaData: BehaviorSubject<SiteInfoMetaData> =
    new BehaviorSubject<SiteInfoMetaData>(null);
  ...
  constructor(
    ...
    this._authService.getStartupInfo().subscribe((startUpInfo: StartupInfo) => {
      ...
      if (startUpInfo.resourceType === ResourceType.Site) {
        this._armClient
          .getResource<Site>(startUpInfo.resourceId)
          .subscribe((site: ResponseMessageEnvelope<Site>) => {
            ...
            this.currentSiteStatic.appType =
              site.kind && site.kind.toLowerCase().indexOf('functionapp') >= 0
                ? AppType.FunctionApp
                : AppType.WebApp;
  ...
  private findTargetedSite(siteName: string): Observable<Site> {
    ...
    return this.currentSite.pipe(
      mergeMap((site) => {
        ...
        return this._serverFarmService.sitesInServerFarm.pipe(
          map((sitesInServerFarm) => {
            return sitesInServerFarm.find(
              (site) => site.name.toLowerCase() === siteName
            );
          })
        );
      })
  ...
  restartSite(
    ...
    return this.findTargetedSite(siteName).pipe(
      mergeMap((targetedSite: Site) => {
        let slotName = '';
        ...
        const resourceUri: string = this._uriElementsService.getSiteRestartUrl(
          subscriptionId,
          targetedSite.resourceGroup,
          mainSiteName,
          slotName
        );
  ...
  restartSiteFromUri(resourceUri: string): Observable<HttpResponse<any>> {
    ...
    let result = this._armClient.postResourceFullResponse(
      restartUri,
      null,
      true
    );
    ...
    return this.findTargetedSite(siteName).pipe(
      mergeMap((targetedSite: Site) => {
        if (targetedSite.enabledHostNames.length > 0) {
          const scmHostNameFromSiteObject = targetedSite.enabledHostNames.find(
            (hostname) => hostname.indexOf('.scm.') > 0
          );
          if (
            scmHostNameFromSiteObject !== null &&
            scmHostNameFromSiteObject.length > 0
          ) {
            scmHostName = scmHostNameFromSiteObject;
          }
        }

        const url = this._uriElementsService.getKillSiteProcessUrl(
          subscriptionId,
          targetedSite.resourceGroup,
          targetedSite.name
        );
  ...
  getSiteAppSettings(
    ...
    const url = this._uriElementsService.getListAppSettingsUrl(
      subscriptionId,
      resourceGroup,
      siteName,
      slot
  ...
  getSiteConfigSettings(siteInfo: SiteInfoMetaData): Observable<any> {
    ...
    return this._armClient.getResource<ResponseMessageEnvelope<any>>(url).pipe(
  ...
  getAlwaysOnSetting(siteInfo: SiteInfoMetaData): Observable<boolean> {
    return this.getSiteConfigSettings(siteInfo).pipe(
      map((resp) => {
  ...
  getVirtualNetworkConnectionsInformation(
    subscriptionId: string,
    resourceGroup: string,
    siteName: string,
    slot: string = ''
  ): Observable<any> {
    const url = this._uriElementsService.getVirtualNetworkConnections(
      subscriptionId,
      resourceGroup,
      siteName,
      slot
  ...
  getSiteDaasInfoFromSiteMetadata(): Observable<SiteDaasInfo> {
    return this.currentSiteMetaData.pipe(
      map((siteInfo) => {
        if (siteInfo) {
  ...
  getCrashMonitoringSettings(
    site: SiteDaasInfo
  ): Observable<CrashMonitoringSettings> {
    return this.getSiteAppSettings(
      site.subscriptionId,
      ...
      map((settingsResponse) => {
        if (settingsResponse && settingsResponse.properties) {
          if (
            settingsResponse.properties['WEBSITE_CRASHMONITORING_SETTINGS'] &&
            ...
            crashMonitoringSettings = JSON.parse(
              settingsResponse.properties['WEBSITE_CRASHMONITORING_SETTINGS']
            );
  ...
  getCrashMonitoringDates(crashMonitoringSettings: CrashMonitoringSettings): {
    start: Date;
    end: Date;
  } {
        ...
        if (settingsResponse && settingsResponse.properties) {
          ...
          if (crashMonitoringSettings == null) {
            if (
              ...
              delete settingsResponse.properties[
            ...
            if (!crashMonitoringSettings.ExceptionFilter) {
              crashMonitoringSettings.ExceptionFilter =
                '-f E053534F -f C00000FD.STACK_OVERFLOW';
            }
            settingsResponse.properties['WEBSITE_CRASHMONITORING_ENABLED'] =
              true;
            settingsResponse.properties['WEBSITE_CRASHMONITORING_SETTINGS'] =
              JSON.stringify(crashMonitoringSettings);
            if (blobSasUri) {
              settingsResponse.properties['WEBSITE_DAAS_STORAGE_SASURI'] =
                blobSasUri;
            }
            settingsResponse.properties[
              'WEBSITE_CRASHMONITORING_USE_DEBUGDIAG'
  ...
  updateSiteAppSettings(
    ...
    const url = this._uriElementsService.getUpdateAppSettingsUrl(
      subscriptionId,
      resourceGroup,
      siteName,
      slot
  ...
  updateSettingsFromUri(resourceUri: string, body: any): Observable<any> {
    const restartUri =
      this._uriElementsService.getUpdateSettingsUri(resourceUri);
