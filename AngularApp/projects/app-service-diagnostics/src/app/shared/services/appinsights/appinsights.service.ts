...
export class AppInsightsService {
  ...
  private appInsightsTagName: string =
    'hidden-related:diagnostics/applicationInsightsSettings';
  private appInsightsApiEndpoint: string =
  ...
  private appInsightsEncryptedAppSettingName: string =
    'WEBSITE_APPINSIGHTS_ENCRYPTEDAPIKEY';
  ...
  constructor(
    ...
    private logger: AvailabilityLoggingService,
    private _telmetryService: TelemetryService,
    private _backendService: BackendCtrlService,
    private _rbacService: RBACService
  ) {
    this.loadAppInsightsResourceObservable = new BehaviorSubject<boolean>(null);
    this.loadAppDiagnosticPropertiesObservable = new BehaviorSubject<boolean>(
      null
    );
    ...
    this.authService.getStartupInfo().subscribe((startupInfo: StartupInfo) => {
      if (startupInfo.resourceType === ResourceType.Site) {
        ...
        const resourceUriParts = siteService.parseResourceUri(
          startupInfo.resourceId
        );
        ...
        this._backendService
          .get<string>(`api/appsettings/AppInsights:UseCertificates`)
          .subscribe(
            (useCertificatesSetting) => {
              if (
                useCertificatesSetting &&
              ...
              this.loadAppInsightsSettings(
                resourceUriParts.subscriptionId,
                resourceUriParts.resourceGroup,
                resourceUriParts.siteName,
                resourceUriParts.slotName
              );
            },
            (error) => {
              this.loadAppInsightsSettings(
  ...
  private loadAppInsightsSettings(
    subscriptionId: string,
    resourceGroup: string,
    siteName: string,
    slotName: string = ''
    ...
    this.appAnalysisService
      ...
      .subscribe((data) => {
        if (
          data &&
          data.appStack &&
          data.appStack.toLowerCase().indexOf('asp.net') > -1
        ) {
          ...
          if (this.appInsightsSettings.validForStack === undefined) {
            ...
            this.applicationInsightsValidForApp.next(
              this.appInsightsSettings.validForStack
            );
          }
        }
    ...
    this.getAppInsightsResourceForWebApp().subscribe(
      (aiResource: string) => {
        if (aiResource && aiResource !== '') {
          ...
          this.armService
            .getResourceWithoutEnvelope(aiResource, '2015-05-01')
            .subscribe((armResponse: any) => {
              if (armResponse && armResponse.properties) {
                if (this.isNotNullOrEmpty(armResponse.properties['AppId'])) {
                  this.appInsightsSettings.appId =
                    armResponse.properties['AppId'];
                }

                if (this.isNotNullOrEmpty(armResponse.properties['Name'])) {
                  this.appInsightsSettings.name =
                    armResponse.properties['Name'];
                }
              }
        ...
        this.logger.LogAppInsightsSettings(
          this.appInsightsSettings.enabledForWebApp
        );
      },
      (error) => {
        this.loadAppInsightsResourceObservable.next(false);
  ...
  CheckIfAppInsightsEnabled(): Observable<boolean> {
    ...
    return this.getAppInsightsResourceForWebApp().pipe(
      map((resp) => {
      ...
      .pipe(
        map((settingsResponse) => {
          if (settingsResponse && settingsResponse.properties) {
            if (
              settingsResponse.properties[this.appInsightsAppSettingName] !=
              null
            ) {
        ...
        mergeMap((instrumentationKey) => {
          if (this.isNotNullOrEmpty(instrumentationKey)) {
            ...
            return this.getAppInsightsResourceForInstrumentationKey(
              instrumentationKey,
              this.subscriptionId
            );
          } else {
  ...
  getAppInsightsResourceForInstrumentationKey(
    ...
    return this.armService
      .getResourceCollection<
        ResponseMessageCollectionEnvelope<
          ResponseMessageEnvelope<AppInsightsResponse>[]
        >
      >(url, '2015-05-01', true)
      .pipe(
        map((response: ResponseMessageEnvelope<AppInsightsResponse>[]) => {
          let appInsightsUri = response.find(
            (i) =>
              i.properties &&
              i.properties.InstrumentationKey &&
              i.properties.InstrumentationKey === instrumentationKey
          );
  ...
  getAppInsightsKeyName() {
    return (
      this.appInsightsKeyName +
      '_' +
      this.siteName +
      '_' +
      this.getRandomNumbers().toString()
    );
  }
  ...
  generateAppInsightsAccessKey(): Observable<any> {
    ...
    return this.armService
      .postResourceWithoutEnvelope<any, any>(url, body, '2015-05-01')
      .pipe(
        catchError((err) => {
          return throwError(
            'Failed while generating AppInsights ApiKey - ' + err
          );
        })
      );
  }
  ...
  ExecuteQuery(query: string): Observable<any> {
    ...
    const resourceUri: string = `${
      this.appInsightsSettings.resourceUri
  ...
  ExecuteQuerywithPostMethod(query: string): Observable<any> {
    ...
    return this.armService.postResource<any, any>(
      resourceUri,
      body,
      '2015-05-01',
      true,
  ...
  public openAppInsightsFailuresBlade() {
    this.portalActionService.openAppInsightsFailuresBlade(
      this.appInsightsSettings.resourceUri
    );
  }

  public openAppInsightsPerformanceBlade() {
    this.portalActionService.openAppInsightsPerformanceBlade(
      this.appInsightsSettings.resourceUri
    );
  }
  ...
  public openAppInsightsExtensionBlade(detailBlade?: string) {
    return this.portalService
      .getAppInsightsResourceInfo()
      .subscribe((aiResource: string) => {
        if (this.isNotNullOrEmpty(aiResource)) {
          this.portalActionService.openAppInsightsExtensionBlade(
            detailBlade,
            aiResource
          );
        } else if (
          this.isNotNullOrEmpty(this.appInsightsSettings.resourceUri)
        ) {
          this.portalActionService.openAppInsightsExtensionBlade(
            detailBlade,
            this.appInsightsSettings.resourceUri
          ...
          this.getAppInsightsResourceFromAppSettings().subscribe(
            (aiResourceAppSettings) => {
              this.portalActionService.openAppInsightsExtensionBlade(
                detailBlade,
                aiResourceAppSettings
              );
  ...
  public logAppInsightsError(
    resourceUri: string,
    telmetryEvent: string,
    error: any
  ) {
      ...
      mergeMap((apiKey) => {
        if (apiKey) {
          const additionalHeaders = new HttpHeaders({
            'appinsights-key': apiKey
          });
          return this._backendService
            .get(`api/appinsights/encryptkey`, additionalHeaders)
            .pipe(
              catchError((err) => {
                return throwError(
                  'Failed while encrypting the AppInsights API Key'
                );
              })
            )
      ...
      mergeMap((encryptedKey) => {
        if (encryptedKey) {
          if (this.useAppSettingsForAppInsightEncryption) {
            return this.updateAppInsightsEncryptedAppSettings(
              encryptedKey,
              appId
            );
          } else {
            return this.updateAppInsightsEncryptedArmTag(
              resourceUri,
              encryptedKey,
              appId
            );
    ...
    if (this.useAppSettingsForAppInsightEncryption) {
      return this.getAppInsightsEncryptedAppSettings().pipe(
        map((settingsResponse) => {
          if (
            settingsResponse &&
            settingsResponse.ApiKey != null &&
            settingsResponse.AppId != null
          ) {
            return true;
          }
          return false;
  ...
  public updateAppInsightsEncryptedAppSettings(
    encryptedKey: string,
    appId: string
  ): Observable<boolean> {
    ...
    return this.siteService
      ...
      .pipe(
        map((settingsResponse) => {
          settingsResponse.properties[this.appInsightsEncryptedAppSettingName] =
            settingValue;
          return settingsResponse;
        }),
        catchError((err) => {
          return throwError(
            'Failed while getting App Settings for the resource - ' + err
          );
        }),
        mergeMap((settingsResponse) => {
          return this.siteService
            ...
            .pipe(
              ...
              catchError((err) => {
                return throwError(
                  'Failed while updating App Settings for the resource - ' + err
                );
              })
            );
        })
  ...
  private checkAppInsightsConnectedViaArmTag(
    resourceId: string
  ): Observable<boolean> {
    return this.getAppInsightsArmTag(resourceId).pipe(
      map((appInsightsTag) => {
        if (
          appInsightsTag != null &&
          appInsightsTag.AppId != null &&
          appInsightsTag.ApiKey != null
        ) {
          return true;
        }
        return false;
  ...
  private updateAppInsightsEncryptedArmTag(
    resourceUri: string,
    encryptedKey: string,
    appId: string
  ): Observable<any> {
      ...
      mergeMap((updatedTags) => {
        return this.armService
          ...
          .pipe(
            ...
            catchError((err) => {
              return throwError(
                'Failed while updating ARM tags for the resource - ' + err
              );
            })
          );
      })
  ...
  private getExistingTags(
    ...
    return this.armService
      .getResourceFullResponse(resourceUri, true, apiVersion)
      .pipe(
        map((response) => {
        ...
        catchError((err) => {
          return throwError(
            'Failed while getting ARM tags for the resource - ' + err
          );
        })
      );
  }

  private getUpdatedTags(
    resourceUri: string,
    encryptedKey: string,
    appId: string
  ) {
    return this.getExistingTags(resourceUri).map((existingTags) => {
      if (existingTags != null) {
        existingTags[this.appInsightsTagName] = JSON.stringify({
          ApiKey: encryptedKey,
          AppId: appId
        });
        return existingTags;
        ...
        let newTags = {};
        newTags[this.appInsightsTagName] = JSON.stringify({
          ApiKey: encryptedKey,
          AppId: appId
        });
        return newTags;
      ...
      .pipe(
        map((settingsResponse) => {
          if (
            ...
            var appInsightsSettingsJson = JSON.parse(
              settingsResponse.properties[
                this.appInsightsEncryptedAppSettingName
              ]
            );
  ...
  public getAppInsightsStoredConfiguration(
    resourceUri: string
  ): Observable<any> {
    return this.getAppInsightsEncryptedAppSettings().pipe(
      map((appInsightsSettingsJson) => {
        if (
          appInsightsSettingsJson != null &&
          this.useAppSettingsForAppInsightEncryption
        ) {
          return appInsightsSettingsJson;
        }
      }),
  ...
  public getAppInsightsArmTag(resourceUri: string): Observable<any> {
    return this.getExistingTags(resourceUri).pipe(
      map((existingTags) => {
        if (
          existingTags != null &&
          existingTags[this.appInsightsTagName] != null
        ) {
          var appInsightsTag = JSON.parse(
            existingTags[this.appInsightsTagName]
          );
  ...
  public checkAppInsightsAccess(
    appInsightsResourceUri: string
  ): Observable<boolean> {
    return this._rbacService.hasPermission(appInsightsResourceUri, [
      this._rbacService.writeScope
    ]);
  }
