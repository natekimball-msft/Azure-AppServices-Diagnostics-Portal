...
export class PortalAppInsightsTelemetryService implements ITelemetryProvider {
  ...
  constructor(
    private _backendCtrlService: BackendCtrlService,
    private _versionTestService: VersionTestService,
    private _portalService: PortalService
  ) {
    const appInsightsRequest = this._backendCtrlService
      .get<string>(`api/appsettings/ApplicationInsights:InstrumentationKey`)
      .pipe(
        map((value: string) => {
    ...
    const envConfigRequest = this._backendCtrlService
      .get<string>('api/appsettings/ASD_ENVIRONMENT')
      .pipe(
        map((value: string) => {
    ...
    const hostnameConfigRequest = this._backendCtrlService
      .get<string>(`api/appsettings/ASD_HOST`)
      .pipe(
        map((value: string) => {
    ...
    appInsightsRequest.subscribe(() => {
      envConfigRequest.subscribe(() => {
        hostnameConfigRequest.subscribe(() => {
          ...
          this.appInsights.addTelemetryInitializer(
            (envelop: ITelemetryItem) => {
              ...
              try {
                ...
                envelop.data['initalPortalVersion'] =
                  this._versionTestService.initializedPortalVersion.value;
              } catch (e) {
                this.logException(e);
              }
            }
          ...
          this.logEvent(
            'Application Insights initialized for diagnostics client'
          );
        });
      });
    });
  ...
  public logPageView(
    ...
    properties.duration =
      duration === undefined || duration === null ? 0 : duration;
  ...
  public logTrace(
    ...
    severityLevel =
      severityLevel == undefined || severityLevel == null
