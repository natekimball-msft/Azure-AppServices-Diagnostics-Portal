...
export class LoggingService {
  ...
  constructor(
    private _portalServiceInstance: PortalService,
    private _authServiceInstance: AuthService,
    private _armServiceInstance: ArmService
  ) {
    this._authServiceInstance.getStartupInfo().subscribe((data) => {
      ...
      if (this._startUpInfo && this._startUpInfo.resourceId) {
        ...
        this._subscriptionId =
          subscriptionIndex !== -1 ? parts[subscriptionIndex + 1] : '';

        const resourceGroupIndex = parts.indexOf('resourcegroups');
        this._resourceGroup =
          resourceGroupIndex !== -1 ? parts[resourceGroupIndex + 1] : '';
        ...
        this._resourceName =
          resourceIndex !== -1
            ? parts[resourceIndex + 1]
            : siteIndex !== -1
            ? parts[siteIndex + 1]
            : hostingEnvironmentIndex !== -1
            ? parts[hostingEnvironmentIndex + 1]
            : '';

        const providerIndex = parts.indexOf('providers');
        this._resourceType =
          providerIndex !== -1
            ? parts[providerIndex + 1] + '/' + parts[providerIndex + 2]
            : '';

        if (this._startUpInfo.workflowId) {
          this._ticketBladeWorkflowId = this._startUpInfo.workflowId;
          this._diagnosticsVersion = VersioningHelper.isV2Subscription(
            this._subscriptionId
          )
            ? 'v2'
            : 'v1';
        ...
        if (siteIndex !== -1) {
          const siteSpecificDataRequests = forkJoin(
            this._armServiceInstance.getResource<IDiagnosticProperties>(
              this._startUpInfo.resourceId + '/diagnostics/properties'
            ),
            this._armServiceInstance.getResource<Site>(
              this._startUpInfo.resourceId
            )
          );

          siteSpecificDataRequests.subscribe((partialObserver) => {
            const propertiesEnvelope: ResponseMessageEnvelope<IDiagnosticProperties> =
              <ResponseMessageEnvelope<IDiagnosticProperties>>(
                partialObserver[0]
              );
            const resourceEnvelope: ResponseMessageEnvelope<Site> = <
              ResponseMessageEnvelope<Site>
            >partialObserver[0];
            ...
            if (resourceEnvelope && resourceEnvelope.properties) {
              this.platform =
                SiteExtensions.operatingSystem(resourceEnvelope.properties) ===
                OperatingSystem.windows
                  ? 'windows'
                  : 'linux';
              this._appType =
                resourceEnvelope.properties.kind &&
                resourceEnvelope.properties.kind
                  .toLowerCase()
                  .indexOf('functionapp') >= 0
                  ? 'functionapp'
                  : 'webapp';
            }
  ...
  protected _log(id: string, category: string, args: any = null): void {
    ...
    const combinedArgs = {};
    Object.keys(commonArgs).forEach(
      (key: string) => (combinedArgs[key] = commonArgs[key])
    );
    if (args) {
      Object.keys(args).forEach(
        (key: string) => (combinedArgs[key] = args[key])
      );
    }
  ...
  LogMessage(message: string, category: string = 'Availability') {
    this._log(
      CommonLogEventType[CommonLogEventType.Message].toString(),
      category,
      { message: message }
    );
  }
  ...
  LogError(errorMessage: string, category: string = 'Availability') {
    this._log(
      CommonLogEventType[CommonLogEventType.Error].toString(),
      category,
  ...
  LogClickEvent(
    ...
    this._log(
      CommonLogEventType[CommonLogEventType.Click].toString(),
      category,
      {
        name: name,
  ...
  LogTabOpened(name: string) {
    this._log(
      CommonLogEventType[CommonLogEventType.TabOpened].toString(),
      'Support Home',
      {
        name: name
      }
  ...
  LogTabClosed(name: string) {
    this._log(
      CommonLogEventType[CommonLogEventType.TabClosed].toString(),
      'Support Home',
      {
        name: name
      }
  ...
  LogStartUpInfo(startupInfo: StartupInfo, category: string = 'Availability') {
    this._log(
      CommonLogEventType[CommonLogEventType.StartUp].toString(),
      category,
      {
  ...
  LogMissingSolution(solutionId: number, category: string = 'Availability') {
    this._log(
      CommonLogEventType[CommonLogEventType.MissingSolution],
      category,
      {
  ...
  LogFeedbackMessage(
    ...
    this._log(
      CommonLogEventType[CommonLogEventType.FeedbackMessage],
      category,
      {
        source: source,
        message: message
  ...
  LogIncidentNotification(hasActiveIncident: boolean) {
    this._log(
      CommonLogEventType[CommonLogEventType.IncidentNotification],
      'Incidents',
      {
  ...
  LogIncidentIncidentDetails(incident: IncidentNotification) {
    this._log(
      CommonLogEventType[CommonLogEventType.IncidentDetails],
      'Incidents',
      {
  ...
  LogIncidentDismissed(hasActiveIncident: boolean) {
    this._log(
      CommonLogEventType[CommonLogEventType.IncidentDismissed],
      'Incidents',
      {
        active: hasActiveIncident
      }
