...
export class PortalSettingsService {
  ...
  constructor(
    ...
    this.authService.getStartupInfo().subscribe((data) => {
      ...
      this.versionTestService.isLegacySub.subscribe((isLegacy) => {
        this.settingsUrl = isLegacy
          ? `/resource${data.resourceId}/settings`
          : `/resource${data.resourceId}/categories/AvailabilityAndPerformanceWindows/settings`;
      });
    });
  }
  ...
  public getScanEnabled(): Observable<boolean> {
    ...
    return this.armService
      .getResource<ResponseMessageEnvelope<any>>(url, '2018-02-01', true)
      .pipe(
        map((data: ResponseMessageEnvelope<any>) => {
          let resource = data;
          let scanEnabled = false;
          if (
            resource.tags &&
            resource.tags[
              'hidden-related:diagnostics/changeAnalysisScanEnabled'
            ]
          ) {
            scanEnabled =
              resource.tags[
