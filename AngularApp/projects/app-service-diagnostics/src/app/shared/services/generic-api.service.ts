...
export class GenericApiService {
  ...
  public getDetectors(
    ...
    let resourceId = overrideResourceUri
      ? overrideResourceUri
      : this.resourceId;
    let queryParams = this.isLocalizationApplicable()
      ? [{ key: 'l', value: this.effectiveLocale }]
      : [];
    if (this.useLocal) {
      ...
      return this._armService
        .getResourceCollection<DetectorResponse[]>(
          path,
          null,
          false,
          queryParams
        ...
        .pipe(
          map((response: ResponseMessageEnvelope<DetectorResponse>[]) => {
            this.detectorList = response.map(
              (listItem) => listItem.properties.metadata
            );
  ...
  public getDetectorsSearch(searchTerm): Observable<DetectorMetaData[]> {
    if (this.useLocal) {
      const path =
        `v4${this.resourceId}/detectors?stampName=waws-prod-bay-085&hostnames=netpractice.azurewebsites.net&text=` +
        ...
        .pipe(
          map((response: ResponseMessageEnvelope<DetectorResponse>[]) => {
            var searchResults = response
              .map((listItem) => listItem.properties.metadata)
              .sort((a, b) => {
  ...
  public getDetector(
    ...
    let resourceId = overrideResourceUri
      ? overrideResourceUri
      : this.resourceId;
    let languageQueryParam = this.isLocalizationApplicable()
      ? `&l=${this.effectiveLocale}`
      : '';
    ...
    if (this.useLocal) {
      ...
      return this._armService.getArmResource<ArmResource>(resourceId).pipe(
        flatMap((resource) => {
          ...
          return this._armService
            .getResource<DetectorResponse>(path, null, refresh, requestHeaders)
            .pipe(
              map((response: ResponseMessageEnvelope<DetectorResponse>) => {
