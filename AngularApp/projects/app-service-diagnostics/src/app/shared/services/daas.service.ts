...
import {
  ...
  DaasAppInfo,
  DaasSettings,
  ValidateSasUriResponse,
  Session,
  LinuxDaasSettings,
  ValidateStorageAccountResponse,
...
export class DaasService {
  ...
  constructor(
    ...
    private _uriElementsService: UriElementsService,
    private _siteService: SiteService
  ) {}

  submitDaasSession(site: SiteDaasInfo, session: Session) {
    let resourceUri: string = this._uriElementsService.getSessionsUrl(
      site,
      false
    );
    return <Observable<string>>(
      this._armClient.postResource(resourceUri, session, null, true)
    );
  }
  ...
  submitDaasSessionOnInstance(
    ...
    let resourceUri: string =
      this._uriElementsService.getSessionsForInstanceUrl(site, instanceId);
    return <Observable<string>>(
      this._armClient.postResource(resourceUri, session, null, true)
    );
  }
  ...
  getActiveSession(
    ...
    let resourceUri: string =
      this._uriElementsService.getActiveSessionUrl(site);
    if (!isWindowsApp) {
      resourceUri = this._uriElementsService.getActiveSessionLinuxUrl(
        site,
        useDiagnosticServerForLinux
      );
    }
    return <Observable<Session>>(
      this._armClient.getResourceWithoutEnvelope<Session>(
        resourceUri,
        null,
        true
      )
  ...
  getSessions(
    ...
    const resourceUri: string = this._uriElementsService.getSessionsUrl(
      site,
      useDiagnosticServerForLinux
    );
    return <Observable<Session[]>>(
  ...
  getSession(
    ...
    const resourceUri: string = this._uriElementsService.getSessionUrl(
      site,
      sessionId,
      useDiagServerForLinux
    );
    return <Observable<Session>>(
      this._armClient.getResourceWithoutEnvelope<Session>(
        resourceUri,
  ...
  getInstances(
    ...
    return this._armClient
      .getResourceCollection<any>(resourceUri, null, true)
      .pipe(
        map((response) => {
          if (Array.isArray(response) && response.length > 0) {
            ...
            response.forEach((instance) => {
              if (
                instance &&
                instance.properties &&
                instance.properties['machineName']
              ) {
                instances.push({
                  instanceId: instance.properties['name'],
                  machineName: instance.properties['machineName']
                });
              }
            });
            return instances;
  ...
  getDiagnosers(site: SiteDaasInfo): Observable<DiagnoserDefinition[]> {
    ...
    return <Observable<DiagnoserDefinition[]>>(
      this._armClient.getResourceWithoutEnvelope<DiagnoserDefinition[]>(
        resourceUri,
        null,
        true
      )
  ...
  getDatabaseTest(
    site: SiteInfoMetaData
  ): Observable<DatabaseTestConnectionResult[]> {
    const resourceUri: string =
      this._uriElementsService.getDatabaseTestUrl(site);
    return <Observable<DatabaseTestConnectionResult[]>>(
      this._armClient.getResourceWithoutEnvelope<
        DatabaseTestConnectionResult[]
  ...
  getDaasWebjobState(site: SiteDaasInfo): Observable<string> {
    ...
    return this._armClient
      ...
      .pipe(
        map((response) => {
          if (Array.isArray(response) && response.length > 0) {
            const daasWebJob = response.filter((x) =>
              x.id.toLowerCase().endsWith('/daas')
            );
            if (
              daasWebJob != null &&
              daasWebJob.length > 0 &&
              daasWebJob[0].properties != null
            ) {
  ...
  deleteDaasSession(
    ...
    let resourceUri: string =
      this._uriElementsService.getSingleSessionDeleteUrl(
        site,
        sessionId,
        isDiagServerSession
      );
    return <Observable<any>>(
      this._armClient.deleteResource(resourceUri, null, true)
    );
  }

  getAppInfo(site: SiteDaasInfo): Observable<DaasAppInfo> {
    const resourceUri: string = this._uriElementsService.getAppInfoUrl(site);
    return <Observable<DaasAppInfo>>(
      this._armClient.getResourceWithoutEnvelope<DaasAppInfo>(
        resourceUri,
        null,
        true
      )
  ...
  getAllMonitoringSessions(
    site: SiteDaasInfo
  ): Observable<MonitoringSession[]> {
    const resourceUri: string =
      this._uriElementsService.getMonitoringSessionsUrl(site);
    return <Observable<MonitoringSession[]>>(
      this._armClient.getResourceWithoutEnvelope<MonitoringSession[]>(
        resourceUri,
        null,
        true
      )
  ...
  getMonitoringSession(
    ...
    const resourceUri: string =
      this._uriElementsService.getMonitoringSessionUrl(site, sessionId);
    return <Observable<MonitoringSession>>(
      this._armClient.getResourceWithoutEnvelope<MonitoringSession>(
        resourceUri,
        null,
  ...
  analyzeMonitoringSession(
    site: SiteDaasInfo,
    sessionId: string
  ): Observable<any> {
    const resourceUri: string =
      this._uriElementsService.getAnalyzeMonitoringSessionUrl(site, sessionId);
    return <Observable<any>>(
      this._armClient.postResource(resourceUri, null, null, true)
    );
  }
  ...
  getActiveMonitoringSession(
    ...
    const resourceUri: string =
      this._uriElementsService.getActiveMonitoringSessionUrl(site);
    return <Observable<MonitoringSession>>(
      this._armClient.getResourceWithoutEnvelope<MonitoringSession>(
        resourceUri,
        null,
  ...
  getActiveMonitoringSessionDetails(
    site: SiteDaasInfo
  ): Observable<ActiveMonitoringSession> {
    const resourceUri: string =
      this._uriElementsService.getActiveMonitoringSessionDetailsUrl(site);
    return <Observable<ActiveMonitoringSession>>(
      this._armClient.getResourceWithoutEnvelope<ActiveMonitoringSession>(
        resourceUri,
        null,
        true
      )
  ...
  stopMonitoringSession(site: SiteDaasInfo): Observable<string> {
    const resourceUri: string =
      this._uriElementsService.stopMonitoringSessionUrl(site);
    return <Observable<string>>(
      this._armClient.postResource(resourceUri, null, null, true)
    );
  }

  submitMonitoringSession(
    site: SiteDaasInfo,
  ...
  setBlobSasUri(
    ...
    return <Observable<boolean>>(
      this._armClient.postResource(resourceUri, settings, null, true)
    );
  }
  ...
  getStorageConfiguration(
    site: SiteDaasInfo,
    useDiagServerForLinux: boolean
  ): Observable<DaasStorageConfiguration> {
    let settingName = 'WEBSITE_DAAS_STORAGE_SASURI';
    if (useDiagServerForLinux) {
      settingName = 'WEBSITE_DAAS_STORAGE_CONNECTIONSTRING';
    }
      ...
      .pipe(
        map((settingsResponse) => {
          if (
            ...
            settingsResponse.properties[settingName] != null
          ) {
            let daasStorageConfiguration: DaasStorageConfiguration =
              new DaasStorageConfiguration();
            daasStorageConfiguration.IsAppSetting = true;
            if (useDiagServerForLinux) {
              daasStorageConfiguration.ConnectionString =
                settingsResponse.properties[settingName];
            } else {
              daasStorageConfiguration.SasUri =
                settingsResponse.properties[settingName];
            }
        ...
        mergeMap((daasStorageConfiguration: DaasStorageConfiguration) => {
          if (
            ...
            let daasStorageConfig: DaasStorageConfiguration =
              new DaasStorageConfiguration();
  ...
  validateSasUri(site: SiteDaasInfo): Observable<ValidateSasUriResponse> {
    const resourceUri: string =
      this._uriElementsService.getValidateBlobSasUriUrl(site);
    return this._armClient
      .getResourceWithoutEnvelope<ValidateSasUriResponse>(
        resourceUri,
        null,
        true
      )
  ...
  validateStorageAccount(
    site: SiteDaasInfo
  ): Observable<ValidateStorageAccountResponse> {
    const resourceUri: string =
      this._uriElementsService.getValidateStorageAccountUrl(site);
    return this._armClient
      .getResourceWithoutEnvelope<ValidateStorageAccountResponse>(
        resourceUri,
        null,
        true
      )
  ...
  isStorageAccountConfiguredForDiagServer(
    site: SiteDaasInfo
  ): Observable<boolean> {
  ...
  isDiagServerEnabledForLinux(site: SiteDaasInfo): Observable<boolean> {
    ...
    const resourceUri: string =
      this._uriElementsService.getLinuxCommandUrl(site);
    let command: LinuxCommand = {
      command: 'printenv WEBSITE_USE_DIAGNOSTIC_SERVER'
    };
    return this._armClient
      .postResourceWithoutEnvelope<LinuxCommandOutput, LinuxCommand>(
        resourceUri,
        command,
        null,
        true
      ...
      .pipe(
        map((resp: LinuxCommandOutput) => {
          if (resp && resp.Output) {
            this.linuxDiagServerEnabled =
              resp.Output.toLowerCase().startsWith('true');
            this.linuxDiagServerEnabledChecked = true;
            return this.linuxDiagServerEnabled;
          } else {
            this.linuxDiagServerEnabled = false;
  ...
  putStdoutSetting(
    ...
    const resourceUri: string =
      this._uriElementsService.getStdoutSettingUrl(resourceUrl);
    return <Observable<{ Stdout: string }>>(
      this._armClient.putResourceWithoutEnvelope<{ Stdout: string }, any>(
        resourceUri,
