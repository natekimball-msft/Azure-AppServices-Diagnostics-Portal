...
const publicOrigins = ['portal.azure.com'];
...
export class PortalService {
  ...
  initializeIframe(): void {
    ...
    window.addEventListener(
      Verbs.message,
      this.iframeReceivedMsg.bind(this),
      false
    );

    this._broadcastService.subscribe<ErrorEvent>(
  ...
  private iframeReceivedMsg(event: Event): void {
    if (
      !event ||
      !event.data ||
      event.data.signature !== this.portalSignature
    ) {
      return;
    }
    ...
    console.log(
      '[iFrame] Received validated mesg: ' + methodName,
      event,
      event.srcElement,
      event.srcElement.location,
      event.srcElement.location.host
    );
    ...
    this._checkAcceptOrigins(event).subscribe((foundOrigin) => {
      ...
      const isIFrameForCaseSubmissionSolution = event.srcElement.location.host
        .toString()
        .includes('appservice-diagnostics-am2');

      if (methodName === Verbs.sendStartupInfo) {
        const info = <StartupInfo>data;
        this.sessionId = info.sessionId;
        info.isIFrameForCaseSubmissionSolution =
          isIFrameForCaseSubmissionSolution;
        this.startupInfoObservable.next(info);
        this.isIFrameForCaseSubmissionSolution.next(
          isIFrameForCaseSubmissionSolution
        );
        this.logMessage(
          LogEntryLevel.Verbose,
          TelemetryEventNames.PortalIFrameLoadingSuccess,
          {
    ...
    if (event.data.kind === Verbs.sendStartupInfo) {
      ...
      const request = this._http
        ...
        .pipe(
          ...
          catchError((error) => {
            this.logException(
              'cannot get origin list from backend appsetting',
  ...
  private _checkAcceptOrigins(event: Event): Observable<boolean> {
    if (event.data.kind === Verbs.sendStartupInfo) {
      ...
      this.logMessage(
        LogEntryLevel.Verbose,
        TelemetryEventNames.PortalIFrameLoadingStart,
        {
          portalSessionId: this.sessionId
        }
      );
    ...
    if (
      publicOrigins.findIndex((o) =>
        event.origin.toLocaleLowerCase().endsWith(o.toLowerCase())
      ) > -1
    ) {
      return of(true);
    }

    return this._getAcceptOrigins(event).pipe(
      map((originsSuffix) => {
        const originIndex = originsSuffix.findIndex((o) =>
          event.origin.toLowerCase().endsWith(o.toLowerCase())
        );
        if (originIndex === -1) {
