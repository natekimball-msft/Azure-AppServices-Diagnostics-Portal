...
export class AuthService {
  ...
  public get hasLocalStartupInfo() {
    return (
      this.localStartUpInfo &&
      this.localStartUpInfo.token &&
      this.localStartUpInfo.resourceId
    );
  }
  ...
  getStartupInfo(): Observable<StartupInfo> {
    ...
    if (this.inIFrame) {
      ...
      if (this.localStartUpInfo.token.startsWith('Bearer ')) {
        this.localStartUpInfo.token = this.localStartUpInfo.token.replace(
          'Bearer ',
          ''
        );
      }
        ...
        if (info && info.resourceId) {
          ...
          if (!this.resourceType) {
            this.resourceType =
              info.resourceId
                .toLowerCase()
                .indexOf('/providers/microsoft.web/hostingenvironments/') > 0
                ? ResourceType.HostingEnvironment
                : info.resourceId
                    .toLowerCase()
                    .includes('/providers/microsoft.web/sites/')
                ? ResourceType.Site
                : ResourceType.Other;
          }
