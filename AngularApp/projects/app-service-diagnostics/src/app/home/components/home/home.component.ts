...
import {
  HomePageText,
  RiskAlertConfig
} from '../../../shared/models/arm/armResourceConfig';
...
import {
  allowV3PResourceTypeList,
  VersionTestService
} from '../../../fabric-ui/version-test.service';
...
export class HomeComponent implements OnInit, AfterViewInit {
  ...
  get showSwitchBanner(): boolean {
    const typeSwitchItem = allowV3PResourceTypeList.find(
      (item) =>
        this._resourceService.resource &&
        this._resourceService.resource.type &&
        this._resourceService.resource.type.toLowerCase() ===
          item.type.toLowerCase()
    );
    const allowResourceTypeSwitch =
      typeSwitchItem === undefined ? false : typeSwitchItem.allowSwitchBack;
    return allowResourceTypeSwitch && this._showSwitchBanner;
  }
  initializedPortalVersion = 'v3';
  get inputAriaLabel(): string {
    return this.searchValue !== ''
      ? `${this.searchResultCount} Result` +
          (this.searchResultCount !== 1 ? 's' : '')
      : '';
  }
  ...
  constructor(
    ...
    private _riskAlertService: RiskAlertService,
    public abTestingService: ABTestingService
  ) {
    this.subscriptionId =
      this._activatedRoute.snapshot.params['subscriptionid'];
    this.versionTestService.isLegacySub.subscribe(
      (isLegacy) => (this.useLegacy = isLegacy)
    );
    this.versionTestService.initializedPortalVersion.subscribe(
      (v) => (this.initializedPortalVersion = v)
    );
    this.resourceName = this._resourceService.resource
    ...
    if (
      _resourceService.armResourceConfig &&
      _resourceService.armResourceConfig.homePageText &&
      _resourceService.armResourceConfig.homePageText.title &&
      _resourceService.armResourceConfig.homePageText.title.length > 1 &&
      _resourceService.armResourceConfig.homePageText.description &&
      _resourceService.armResourceConfig.homePageText.description.length > 1 &&
      _resourceService.armResourceConfig.homePageText.searchBarPlaceHolder &&
      _resourceService.armResourceConfig.homePageText.searchBarPlaceHolder
        .length > 1
    ) {
      ...
      if (
        ...
        this.homePageText = {
          ...
          description:
            "Use App Service Environment Diagnostics to investigate how your App Service Environment is performing, diagnose issues, and discover how to\
        ...
        if (
          ...
          (this._resourceService as WebSitesService).appType ===
            AppType.WorkflowApp
        ) {
          this.homePageText = {
            title: 'Logic App Diagnostics (Preview)',
            description:
              'Troubleshoot run and connector issues, investigate app performance and discover how to improve its reliability.',
          ...
          this.homePageText = {
            ...
            description:
              'Investigate how your app is performing, diagnose issues and discover how to improve your application.',
    ...
    if (this.useStaticAksText) {
      this.homePageText = {
        ...
        description:
          'Explore ways to diagnose and troubleshoot the common problems of your cluster from CRUD operations to connection problems. Click on any of the documents below to start troubleshooting.',
        searchBarPlaceHolder: 'Search a keyword that best describes your issue'
      };
    }

    if (_resourceService.armResourceConfig) {
      this._categoryService.initCategoriesForArmResource(
        _resourceService.resource.id
      );
      this._quickLinkService.initQuickLinksForArmResource(
        _resourceService.resource.id
      );
      this._riskAlertService.initRiskAlertsForArmResource(
        _resourceService.resource.id
      );
    }
    ...
    this._authService.getStartupInfo().subscribe((startupInfo) => {
      if (
        startupInfo.additionalParameters &&
        Object.keys(startupInfo.additionalParameters).length > 0
      ) {
        let path = 'resource' + startupInfo.resourceId.toLowerCase();
        path = this._updateRouteBasedOnAdditionalParameters(
          path,
          startupInfo.additionalParameters
        );
        if (path) {
    ...
    this._featureService.featureSub.subscribe((features) => {
      this._quickLinkService.quickLinksSub.subscribe((quickLinks) => {
        if (features !== null && quickLinks !== null) {
          this.quickLinkFeatures = this._filterFeaturesWithQuickLinks(
            quickLinks,
            features
          );
  ...
  ngOnInit() {
    ...
    if (
      this._resourceService &&
      !!this._resourceService.resource &&
      this._resourceService.resource.type === 'Microsoft.Web/sites'
    ) {
      if (locationPlacementId.toLowerCase() !== 'geos_2020-01-01') {
        // Register Change Analysis Resource Provider.
        this.armService
          .postResourceFullResponse(
            this.providerRegisterUrl,
            {},
            true,
            '2018-05-01'
          )
          .subscribe(
            (response: HttpResponse<{}>) => {
              ...
              this._telemetryService.logEvent(
                'Change Analysis Resource Provider registered',
                eventProps
              );
            },
            (error: any) => {
    ...
    this._riskAlertService.getRiskAlertNotificationResponse().subscribe(() => {
      this._riskAlertService.riskPanelContentsSub.next(
        this._riskAlertService.risksPanelContents
      );
    });
  ...
  ngAfterViewInit() {
    this._telemetryService.logPageView(TelemetryEventNames.HomePageLoaded, {
      numCategories: this.categories.length.toString()
    });
  ...
  public get useStaticAksText(): boolean {
    return (
      this.armService.isMooncake &&
      ResourceDescriptor.parseResourceUri(
        this._resourceService.resourceIdForRouting
      ).provider.toLowerCase() == 'microsoft.containerservice'
    );
  }

  public get isAKSOnNationalCloud(): boolean {
    return (
      this.armService.isNationalCloud &&
      ResourceDescriptor.parseResourceUri(
        this._resourceService.resourceIdForRouting
      ).provider.toLowerCase() == 'microsoft.containerservice'
    );
  ...
  public get isWebApp(): boolean {
    return (
      this._resourceService &&
      this._resourceService instanceof WebSitesService &&
      (this._resourceService as WebSitesService).appType === AppType.WebApp
    );
  }
  ...
  private _updateRouteBasedOnAdditionalParameters(
    route: string,
  ...
  private logHTTPError(error: any, methodName: string): void {
    ...
    this._telemetryService.logTrace(
      'HTTP error in ' + methodName,
      errorLoggingProps
    );
  }
  ...
  private _checkIsWindowsWebApp(): boolean {
    ...
    if (
      ...
      this._resourceService instanceof WebSitesService &&
      (this._resourceService as WebSitesService).appType === AppType.WebApp &&
      (this._resourceService as WebSitesService).platform ===
  ...
  private _filterFeaturesWithQuickLinks(
    quickLinks: string[],
    features: Feature[]
  ): Feature[] {
    let res: Feature[] = [];
  ...
  refreshPage() {
    ...
    this._riskAlertService
      .getRiskAlertNotificationResponse(false, true)
      .subscribe(() => {
        this._riskAlertService.riskPanelContentsSub.next(
          this._riskAlertService.risksPanelContents
        );
  ...
  private initalABTestingBanner() {
    ...
    this.enableABTesting =
      this.abTestingService.enableABTesting &&
      !isContainerApps &&
      !isArcApplianceApps;

    if (this.isPreview) {
      this.abTestingBannerText =
        "Welcome to the new and improved version of Diagnose and Solve Problems. If you'd like to switch back to the old experience";
    } else {
      this.abTestingBannerText =
        'You are currently on the old version of the Diagnose and Solve Problems. To explore the latest UI improvements in the new version';
    }
  }
