...
import {
  NotificationService,
  Notification
} from '../../../shared-v2/services/notification.service';
...
export class SupportTopicRedirectComponent implements OnInit {
  constructor(
    ...
    private portalKustoLogging: PortalKustoTelemetryService,
  ...
  ngOnInit() {
    this._supportTopicService
      .getPathForSupportTopic(
        this._activatedRoute.snapshot.queryParams.supportTopicId,
        this._activatedRoute.snapshot.queryParams.pesId,
        this._activatedRoute.snapshot.queryParams.caseSubject,
        this._activatedRoute.snapshot.queryParams.sapSupportTopicId,
        this._activatedRoute.snapshot.queryParams.sapProductId
      )
      .subscribe((res) => {
        this._router.navigate([`../${res.path}`], {
        ...
        this._riskAlertService
          .getRiskAlertNotificationResponse(true)
          .subscribe((res) => {
      ...
      .subscribe((response) => {
        if (response.body['subscriptionPolicies']) {
          this.portalKustoLogging.logEvent('SubscriptionProperties', {
            ...
            subscriptionLocationPlacementId:
