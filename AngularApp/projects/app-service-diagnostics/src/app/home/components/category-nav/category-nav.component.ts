...
export class CategoryNavComponent implements OnInit {
    ...
    return siteFilteredItems
      .filter(
        (item) =>
          ...
          (item.sku === Sku.All ||
            (item.sku === Sku.NotDynamic &&
              this._webSiteService.sku != Sku.Dynamic) ||
            item.sku & this._webSiteService.sku) > 0 &&
          (item.hostingEnvironmentKind &
            this._webSiteService.hostingEnvironmentKind) >
            0 &&
  ...
  constructor(
    public siteFeatureService: SiteFeatureService,
    ...
    public _detectorCategorization: DetectorCategorizationService,
  ...
  getCurrentItemId() {
    if (
      ...
      this._route.url.split('?')[0].endsWith('/overview')
    ) {
    ...
    if (
      ...
      if (
        this._activatedRoute.firstChild.snapshot.routeConfig.path.startsWith(
          'tools/'
        )
      ) {
        ...
        !this._activatedRoute.firstChild.snapshot.params['analysisId']
      ) {
        if (this._activatedRoute.firstChild.snapshot.params['detectorName']) {
          this.currentDetectorId =
            this._activatedRoute.firstChild.snapshot.params['detectorName'];
        }
      } else {
        this.currentDetectorId =
          this._activatedRoute.firstChild.snapshot.params['analysisId'];
  ...
  ngOnInit() {
    ...
    this.toolCategories.push(<SiteFilteredItem<any>>{
      ...
      item: {
        ...
        tools: this.siteFeatureService.proactiveTools
          .filter((tool) => this.stackMatchedForTools(tool))
          .map((tool) => {
            let isSelected = () => {
            ...
            return new CollapsibleMenuItem(
              tool.item.name,
              onClick,
              isSelected,
              icon
    ...
    this.toolCategories.push(<SiteFilteredItem<any>>{
      ...
      item: {
        ...
        tools: this.siteFeatureService.diagnosticTools
          .filter((tool) => this.stackMatchedForTools(tool))
          .map((tool) => {
            let onClick = () => {
    ...
    let supportTools = this.siteFeatureService.supportTools
      .filter((tool) => this.stackMatchedForTools(tool))
      .map((tool) => {
        let onClick = () => {
    ...
    const resourceType: string =
      this._activatedRoute.parent.snapshot.data.data.type;
    ...
    if (resourceType.toLowerCase().startsWith('microsoft.web/sites')) {
      ...
      const diagnosticToolsForNonWeb =
        this.siteFeatureService.diagnosticToolsForNonWeb
          .filter((t) => t.type.toLowerCase() === resourceType.toLowerCase())
          .map((tool) => {
            let onClick = () => {
              this.logCategoryNavClicked(tool.item.name, 'Diagnostic Tools');
              tool.item.clickAction();
    ...
    this.categoryService.categories.subscribe((categories) => {
      ...
      this._activatedRoute.params.subscribe((params) => {
        ...
        this.category = categories.find(
          (category) =>
            category.id.toLowerCase() === params.category.toLowerCase() ||
        ...
        this.isDiagnosticTools = this.category.id
          .toLowerCase()
          .startsWith('diagnostictools');

        this.orphanDetectorList = this._detectorCategorization
          .detectorlistCategories[this.category.id]
          ? this._detectorCategorization.detectorlistCategories[
              this.category.id
            ]
          : [];
        ...
        this.siteFeatureService
          .getFeaturesForCategorySub(this.category)
          .subscribe((features) => {
        ...
        this._route.events.subscribe((evt) => {
          if (evt instanceof NavigationEnd) {
            ...
            if (
              !(
                evt.url.split('/').length > 14 &&
                evt.url.split('/')[12].toLowerCase() === 'analysis' &&
                (evt.url.split('/')[14].toLowerCase() === 'detectors' ||
                  evt.url.split('/')[14].toLowerCase() === 'analysis')
              )
            ) {
              if (evt.url.split('/').length > 12) {
                if (evt.url.split('/')[12].toLowerCase() === 'detectors') {
                  itemId = decodeURI(
                    evt.url.split('detectors/')[1].split('?')[0]
                  );
                } else if (
                  evt.url.split('/')[12].toLowerCase() === 'analysis'
                ) {
                  itemId = decodeURI(
                    evt.url.split('analysis/')[1].split('?')[0]
                  );
                  routePath = 'analysis';
                }
              }

              let item = this.detectorDataLocalCopy.find(
                (metadata) => metadata.id.toLowerCase() === itemId.toLowerCase()
              );
              ...
              if (
                ...
                (item.category == undefined ||
                  item.category == '' ||
                  this.checkIsFromAnotherCategory(
                    categories,
                    item,
                    decodedCategoryName
                  )) &&
                !this.detectorList.find(
                  (detector) => detector.label === item.name
                )
              ) {
                if (
                  !this.orphanDetectorList ||
                  !this.orphanDetectorList.find(
                    (orphan) => orphan.label === item.name
                  )
                ) {
                  let isSelected = () => {
                    return (
                      this._route.url.includes(`detectors/${item.id}`) ||
                      this._route.url.includes(`analysis/${item.id}`)
                    );
                  };
                  ...
                  let orphanMenuItem = new CollapsibleMenuItem(
                    item.name,
                    onClick,
                    isSelected,
                    icon
                  ...
                  if (
                    !this.orphanDetectorList ||
                    !this.orphanDetectorList.find(
                      (item1) => item1.label === orphanMenuItem.label
                    )
                  ) {
                    this._detectorCategorization.detectorlistCategories[
                      this.category.id
  ...
  private checkIsFromAnotherCategory(
    categories: Category[],
    detector: DetectorMetaData,
    currentCategoryId: string
  ): boolean {
    const detectorCategory = categories.find(
      (c) => c.name.toLowerCase() === detector.category.toLowerCase()
    );
    return (
      detectorCategory === undefined ||
      detectorCategory.id.toLowerCase() !== currentCategoryId.toLowerCase()
    );
  }

  private logCategoryNavClicked(name: string, category: string) {
    this._telemetryService.logEvent(
      TelemetryEventNames.CategoryNavItemClicked,
      {
        DetectorName: name,
