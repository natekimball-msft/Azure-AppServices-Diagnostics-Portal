...
@Component({
  ...
  template: ` <div #genieDynamicComponentContainer></div> `
})
export class GenieDynamicComponent {
  currentComponent = null;

  @ViewChild('genieDynamicComponentContainer', {
  ...
  @Input() set componentData(message: Message) {
    ...
    const inputProviders = Object.keys(message.parameters).map((inputName) => ({
      provide: inputName,
      useValue: message.parameters[inputName]
    }));
    ...
    this.currentComponent.instance.onComplete
