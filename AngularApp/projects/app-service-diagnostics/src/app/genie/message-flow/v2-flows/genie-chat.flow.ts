...
export class GenieChatFlow extends IMessageFlowProvider {
  ...
  constructor(
    ...
    private _diagnosticApiService: DiagnosticService,
    ...
    let welcomeMessage: string =
      'Hello, my name is Genie. I am here to help answer any questions you may have about diagnosing and solving problems with your resource. Please describe the issue of your resource.';
    ...
    welcomeMessageGroup.messages.push(
      new TextMessage(welcomeMessage, MessageSender.System, 200)
    );

    const feedbackMessageGroup: MessageGroup = new MessageGroup(
      'feedback',
      [],
      () => ''
    );
    feedbackMessageGroup.messages.push(
      new FeedbackButtonListMessage(
        'Did this information help you solve the issue?',
        this._getButtonListDidYouFindHelpfulinNewGenie(),
        'Availability and Performance'
      )
    );
    const helpfulGroup: MessageGroup = new MessageGroup(
      'feedback-helpful',
      [],
      () => 'feedback-textbox'
    );
    helpfulGroup.messages.push(new TextMessage('Yes', MessageSender.User, 200));
    helpfulGroup.messages.push(
      new TextMessage(
        'Good to hear! Could you let us know how helpful was this?',
        MessageSender.System,
        500
      )
    );
    helpfulGroup.messages.push(
      new GenieFeedbackMessage([], 'Submit', 'Feedback', 'Support Home')
    );
    helpfulGroup.messages.push(
      new TextMessage(
        'Thank you so much for your feedback! If you need more help, type in your question below. Otherwise, exit the session by using the close button in the top right corner.'
      )
    );

    const notHelpfulGroup: MessageGroup = new MessageGroup(
      'feedback-not-helpful',
      [],
      () => 'feedback-textbox'
    );
    notHelpfulGroup.messages.push(
      new TextMessage('No', MessageSender.User, 200)
    );
    notHelpfulGroup.messages.push(
      new TextMessage(
        'Sorry to hear! Could you let us know how we can improve?',
        MessageSender.System,
        500
      )
    );
    ...
    notHelpfulGroup.messages.push(
      new TextMessage(
        'Thank you for your feedback! Could you help describe your issue again so we can further assist you?'
      )
    );
    const feedbackText: MessageGroup = new MessageGroup(
      'feedback-textbox',
      [],
      () => ''
    );
  ...
  createMessageFlowForAnaysisResult(
    data: any,
  ...
  createMessageFlowForAnaysis(
    ...
    let systemResponseTextMessage = new TextMessage(
      'Okay give me a moment while I analyze your app for any issues related to this.',
      MessageSender.System,
      500
    );
    resourceId = this.resourceId;
    let dynamicAnalysisMessage = new DynamicAnalysisMessage(
      keyword,
      resourceId
    );

    const analysisMessageGroup: MessageGroup = new MessageGroup(
      `${messageGroupId}`,
      [],
      () => ''
    );
  ...
  createMessageFlowForCategory(category: Category): Observable<MessageGroup[]> {
    if (
      !category.createFlowForCategory ||
      this.categoriesCreated.indexOf(category) >= 0
    ) {
      return of([]);
    }
    ...
    return this._diagnosticApiService.getDetectors().pipe(
      map((detectors: DetectorMetaData[]) => {
        ...
        let welcomeMessage =
          'genie-Hello, my name is Genie. I am here to help answer any questions you may have about diagnosing and solving problems with your resource. Please describe the issue of your resource.';
        ...
        const welcomeCategory: MessageGroup = new MessageGroup(
          `welcome`,
          [],
          () => mainMenuId
        );
        welcomeCategory.messages.push(
          new TextMessage(welcomeMessage, MessageSender.System, 200)
        );
